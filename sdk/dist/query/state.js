 ...
 */
export async function getMilestonePhaseFilter(projectDir, workstream) {
    ...
    if (milestonePhaseNums.size === 0) {
        const passAllFn = (_dirName) => true;
        const passAll = passAllFn;
 ...
 */
export const stateJson = async (_args, projectDir, workstream) => {
 ...
 */
export const stateGet = async (args, projectDir, workstream) => {
    const statePath = planningPaths(projectDir, workstream).state;
    let content;
    try {
        content = await readFile(statePath, 'utf-8');
 ...
 */
export const stateSnapshot = async (_args, projectDir, workstream) => {
    const statePath = planningPaths(projectDir, workstream).state;
    let content;
    try {
        content = await readFile(statePath, 'utf-8');
