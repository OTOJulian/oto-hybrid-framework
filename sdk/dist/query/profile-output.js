...
function cmdWriteProfileLogic(cwd, options) {
    ...
    if (!isAbsolute(analysisPath))
        analysisPath = join(cwd, analysisPath);
    ...
    for (const dimKey of Object.keys(dimensions)) {
        ...
        if (dim.evidence && Array.isArray(dim.evidence)) {
            for (const ev of dim.evidence) {
                if (ev.quote)
                    ev.quote = redactSensitive(String(ev.quote));
                if (ev.example)
                    ev.example = redactSensitive(String(ev.example));
                if (ev.signal)
                    ev.signal = redactSensitive(String(ev.signal));
            }
        }
    }
    if (redactedCount > 0) {
    ...
    const summaryInstructions = summaryLines.length > 0 ? summaryLines.join('\n') : '- No high or medium confidence dimensions scored yet.';
    const projectsList = (analysis.projects_list ?? analysis.projects_analyzed);
    ...
    for (const dimKey of DIMENSION_KEYS) {
        ...
        const instruction = String(dim.claude_instruction ??
...
export const writeProfile = async (args, projectDir) => {
...
export const generateDevPreferences = async (args, projectDir) => {
...
export const generateClaudeProfile = async (args, projectDir) => {
    const analysisIdx = args.indexOf('--analysis');
    const analysisPath = analysisIdx >= 0 ? args[analysisIdx + 1] : null;
    const outputIdx = args.indexOf('--output');
    const outputPathOpt = outputIdx >= 0 ? args[outputIdx + 1] : null;
...
export const generateClaudeMd = async (args, projectDir) => {
    const outputIdx = args.indexOf('--output');
    const outputPathOpt = outputIdx >= 0 ? args[outputIdx + 1] : null;
...
//# sourceMappingURL=profile-output.js.map