...
import { normalizePhaseName, toPosixPath, resolveAgentsDir, detectRuntime, planningRootName } from './helpers.js';
 ...
 */
async function getPhaseInfoForVerifyWork(phase, projectDir) {
...
export const initPlanPhase = async (args, projectDir, workstream) => {
    ...
    if (phaseInfo?.directory) {
        const phaseDirFull = join(projectDir, phaseInfo.directory);
        try {
            const files = readdirSync(phaseDirFull);
            const contextFile = files.find(f => f.endsWith('-CONTEXT.md') || f === 'CONTEXT.md');
            if (contextFile)
                result.context_path = toPosixPath(join(phaseInfo.directory, contextFile));
            const researchFile = files.find(f => f.endsWith('-RESEARCH.md') || f === 'RESEARCH.md');
            if (researchFile)
                result.research_path = toPosixPath(join(phaseInfo.directory, researchFile));
            const verificationFile = files.find(f => f.endsWith('-VERIFICATION.md') || f === 'VERIFICATION.md');
            if (verificationFile)
                result.verification_path = toPosixPath(join(phaseInfo.directory, verificationFile));
            const uatFile = files.find(f => f.endsWith('-UAT.md') || f === 'UAT.md');
            if (uatFile)
                result.uat_path = toPosixPath(join(phaseInfo.directory, uatFile));
            const reviewsFile = files.find(f => f.endsWith('-REVIEWS.md') || f === 'REVIEWS.md');
            if (reviewsFile)
                result.reviews_path = toPosixPath(join(phaseInfo.directory, reviewsFile));
            const patternsFile = files.find(f => f.endsWith('-PATTERNS.md') || f === 'PATTERNS.md');
            if (patternsFile)
                result.patterns_path = toPosixPath(join(phaseInfo.directory, patternsFile));
        }
 ...
 */
export const initVerifyWork = async (args, projectDir) => {
    const phase = args[0];
    if (!phase) {
 ...
 */
export const initMilestoneOp = async (_args, projectDir) => {
    const config = await loadConfig(projectDir);
    const planningDir = join(projectDir, planningRootName(projectDir));
    const milestone = await getMilestoneInfo(projectDir);
    const phasesDir = join(planningDir, 'phases');
