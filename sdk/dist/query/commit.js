 ...
 */
export function sanitizeCommitMessage(text) {
 ...
 */
export const commit = async (args, projectDir, workstream) => {
    const allArgs = [...args];
    ...
    const filePaths = filesIndex !== -1 ? allArgs.slice(filesIndex + 1).filter(a => !a.startsWith('--')) : [];
    if (!message && !hasAmend) {
 ...
 */
export const checkCommit = async (_args, projectDir, workstream) => {
    const paths = planningPaths(projectDir, workstream);
    let commitDocs = true;
    try {
        const raw = await readFile(paths.config, 'utf-8');
...
// ─── commitToSubrepo ─────────────────────────────────────────────────────
export const commitToSubrepo = async (args, projectDir, workstream) => {
    ...
    try {
        ...
        const commitResult = spawnSync('git', ['-C', projectDir, 'commit', '-m', sanitized], { stdio: 'pipe', encoding: 'utf-8' });
        if (commitResult.status !== 0) {
            return { data: { committed: false, reason: commitResult.stderr || 'commit failed' } };
        }
        const hashResult = spawnSync('git', ['-C', projectDir, 'rev-parse', '--short', 'HEAD'], { encoding: 'utf-8' });
