 ...
 */
export async function determinePhaseStatus(plans, summaries, phaseDir, defaultWhenNoPlans = 'Pending') {
    if (plans === 0)
        return defaultWhenNoPlans;
 ...
 */
export const progressJson = async (_args, projectDir, workstream) => {
 ...
 */
export const progressBar = async (_args, projectDir, workstream) => {
 ...
 */
export const progressTable = async (_args, projectDir, workstream) => {
    const json = await progressJson([], projectDir, workstream);
    const d = json.data;
 ...
 */
export const statsJson = async (args, projectDir, workstream) => {
 ...
 */
export const statsTable = async (_args, projectDir, workstream) => {
 ...
 */
export const todoMatchPhase = async (args, projectDir) => {
    const phase = args[0];
    if (!phase) {
    ...
    const phaseKeywords = new Set(phaseText
        .split(/[\s\-_/.,;:()\[\]{}|]+/)
        .map(w => w.replace(/[^a-z0-9]/g, ''))
        .filter(w => w.length > 2 && !stopWords.has(w)));
    const fp = await findPhase([phase], projectDir);
    const phaseInfoDisk = fp.data;
    const phasePlans = [];
    if (phaseInfoDisk && phaseInfoDisk.found) {
        try {
            const phaseDir = join(projectDir, phaseInfoDisk.directory);
 ...
 */
export const listTodos = async (args, projectDir) => {
 ...
 */
export const todoComplete = async (args, projectDir) => {
