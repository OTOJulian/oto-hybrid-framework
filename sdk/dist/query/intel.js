...
function intelFilePath(projectDir, filename) {
...
export function searchJsonEntries(data, term, depth = 0) {
    ...
    if (!data || typeof data !== 'object')
        return results;
    function matchesInValue(value, d) {
        if (d > MAX_JSON_SEARCH_DEPTH)
            return false;
        if (typeof value === 'string')
            return value.toLowerCase().includes(lowerTerm);
        if (Array.isArray(value))
            return value.some(v => matchesInValue(v, d + 1));
        if (value && typeof value === 'object')
            return Object.values(value).some(v => matchesInValue(v, d + 1));
        return false;
    }
    if (Array.isArray(data)) {
        for (const entry of data) {
            if (matchesInValue(entry, depth + 1))
                results.push(entry);
        }
    }
    else {
...
export const intelStatus = async (_args, projectDir, _workstream) => {
...
export const intelDiff = async (_args, projectDir, _workstream) => {
    if (!isIntelEnabled(projectDir)) {
        return { data: { disabled: true, message: INTEL_DISABLED_MSG } };
    }
...
export const intelSnapshot = async (_args, projectDir, _workstream) => {
    if (!isIntelEnabled(projectDir)) {
        return { data: { disabled: true, message: INTEL_DISABLED_MSG } };
    }
...
export const intelValidate = async (_args, projectDir, _workstream) => {
    if (!isIntelEnabled(projectDir)) {
        return { data: { disabled: true, message: INTEL_DISABLED_MSG } };
    }
    const errors = [];
    ...
    for (const [, filename] of Object.entries(INTEL_FILES)) {
        ...
        if (!filename.endsWith('.md')) {
            ...
            if (!meta?.updated_at)
                ...
                if (age > STALE_MS)
                    warnings.push(`${filename}: stale (${Math.round(age / 3600000)}h old)`);
            }
        }
    }
    return { data: { valid: errors.length === 0, errors, warnings } };
};
export const intelQuery = async (args, projectDir, _workstream) => {
    const term = args[0] || '';
    if (!isIntelEnabled(projectDir)) {
        return { data: { disabled: true, message: INTEL_DISABLED_MSG } };
    }
    ...
    for (const [, filename] of Object.entries(INTEL_FILES)) {
        if (filename.endsWith('.md')) {
            ...
            if (archMatches.length > 0) {
                matches.push({ source: filename, entries: archMatches });
 ...
 */
export const intelExtractExports = async (args, projectDir, _workstream) => {
    const raw = args[0];
    if (!raw) {
...
export const intelPatchMeta = async (args, projectDir, _workstream) => {
    const raw = args[0];
    if (!raw) {
 ...
 */
export const intelUpdate = async (_args, projectDir, _workstream) => {
    if (!isIntelEnabled(projectDir)) {
        return { data: { disabled: true, message: INTEL_DISABLED_MSG } };
    }
    return {
