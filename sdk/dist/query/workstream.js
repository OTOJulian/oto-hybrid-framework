 ...
 */
export const workstreamGet = async (_args, projectDir) => {
...
export const workstreamList = async (_args, projectDir) => {
    const dir = workstreamsDir(projectDir);
    if (!existsSync(dir))
        return { data: { mode: 'flat', workstreams: [], message: 'No workstreams — operating in flat mode' } };
    try {
...
export const workstreamCreate = async (args, projectDir) => {
    const rawName = args[0];
    if (!rawName)
        return { data: { created: false, reason: 'name required' } };
...
export const workstreamStatus = async (args, projectDir) => {
    const name = args[0];
    if (!name) {
...
export const workstreamComplete = async (args, projectDir) => {
    const name = args[0];
    if (!name)
        return { data: { completed: false, reason: 'workstream name required' } };
    if (/[/\\]/.test(name) || name === '.' || name === '..') {
    ...
    try {
        ...
        for (const fname of filesMoved) {
            try {
                renameSync(join(archivePath, fname), join(wsDir, fname));
            }
            catch { /* rollback */ }
        }
        try {
 ...
 */
export const workstreamProgress = async (_args, projectDir) => {
    const wsRoot = workstreamsDir(projectDir);
