    ...
    for (const line of lines) {
        ...
        if (keyMatch) {
            ...
            if (isObjItem && Array.isArray(current.obj)) {
                stack.push({ obj: itemValue, key: null, indent });
            }
        }
    }
    return frontmatter;
 ...
 */
export function extractFrontmatterLeading(content) {
...
export function parseMustHavesBlock(content, blockName) {
    ...
    if (items.length === 0 && blockLines.length > 0) {
        ...
        if (nonEmptyLines > 0) {
            warnings.push(`must_haves.${blockName} block has ${nonEmptyLines} content lines but parsed 0 items. ` +
 ...
 */
export const frontmatterGet = async (args, projectDir) => {
