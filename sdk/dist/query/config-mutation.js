 ...
 */
export function isValidConfigKey(keyPath) {
    ...
    if (DYNAMIC_KEY_PATTERNS.some((p) => p.test(keyPath)))
        return { valid: true };
...
function setConfigValue(obj, dotPath, value) {
    const keys = dotPath.split('.');
    let current = obj;
 ...
 */
export const configSet = async (args, projectDir, workstream) => {
 ...
 */
export const configSetModelProfile = async (args, projectDir, workstream) => {
    const profileName = args[0];
    if (!profileName) {
        throw new GSDError(`Usage: config-set-model-profile <${VALID_PROFILES.join('|')}>`, ErrorClassification.Validation);
    }
    const normalized = profileName.toLowerCase().trim();
    if (!VALID_PROFILES.includes(normalized)) {
        throw new GSDError(`Invalid profile '${profileName}'. Valid profiles: ${VALID_PROFILES.join(', ')}`, ErrorClassification.Validation);
    }
 ...
 */
export const configNewProject = async (args, projectDir, workstream) => {
    const paths = planningPaths(projectDir, workstream);
    ...
    const config = {
        ...
        agent_skills: {
            ...
            ...(globalDefaults.agent_skills || {}),
            ...(userChoices.agent_skills || {}),
        },
        features: {
            ...(defaults.features || {}),
            ...(globalDefaults.features || {}),
 ...
 */
export const configEnsureSection = async (args, projectDir, workstream) => {
