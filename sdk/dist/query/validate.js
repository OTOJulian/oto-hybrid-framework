 ...
 */
export function regexForKeyLinkPattern(pattern) {
 ...
 */
export const verifyKeyLinks = async (args, projectDir) => {
 ...
 */
export const validateConsistency = async (_args, projectDir, workstream) => {
 ...
 */
export const validateHealth = async (args, projectDir, workstream) => {
            ...
            for (const ref of phaseRefs) {
                ...
                if (!normalizedValid.has(ref) && !normalizedValid.has(padded)) {
                    if (normalizedValid.size > 0) {
                        addIssue('warning', 'W002', `STATE.md references phase ${ref}, but only phases ${[...validPhases].sort().join(', ')} are declared`, 'Review STATE.md manually');
                    }
                }
            }
        }
    ...
    if (existsSync(configPath)) {
        try {
            ...
            const bs = configParsed.branching_strategy;
            ...
            const pbt = configParsed.phase_branch_template;
            ...
            const mbt = configParsed.milestone_branch_template;
 ...
 */
function getAgentsDirForValidateAgents() {
 ...
 */
export const validateAgents = async (_args, _projectDir) => {
