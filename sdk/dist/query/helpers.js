    ...
    switch (runtime) {
        ...
        case 'opencode':
            ...
            if (process.env.OPENCODE_CONFIG)
                return dirname(expandTilde(process.env.OPENCODE_CONFIG));
            if (process.env.XDG_CONFIG_HOME)
                return join(expandTilde(process.env.XDG_CONFIG_HOME), 'opencode');
            return join(homedir(), '.config', 'opencode');
        case 'kilo':
            if (process.env.KILO_CONFIG_DIR)
                return expandTilde(process.env.KILO_CONFIG_DIR);
            if (process.env.KILO_CONFIG)
                return dirname(expandTilde(process.env.KILO_CONFIG));
            if (process.env.XDG_CONFIG_HOME)
                return join(expandTilde(process.env.XDG_CONFIG_HOME), 'kilo');
...
export function detectRuntime(config) {
    ...
    if (envValue && SUPPORTED_RUNTIMES.includes(envValue)) {
        return envValue;
    }
 ...
 */
export function comparePhaseNum(a, b) {
 ...
 */
export function extractPhaseToken(dirName) {
 ...
 */
export function phaseTokenMatches(dirName, normalized) {
 ...
 */
export function normalizeMd(content) {
 ...
 */
export async function resolvePathUnderProject(projectDir, userPath) {
