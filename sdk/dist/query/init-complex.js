 ...
 */
function extractCheckboxStates(content) {
 ...
 */
function deriveStatusFromCheckbox(phaseNum, checkboxStates) {
    const stripped = phaseNum.replace(/^0+/, '') || '0';
    if (checkboxStates.get(phaseNum) === true)
        return 'complete';
    if (checkboxStates.get(stripped) === true)
        return 'complete';
    return 'not_started';
}
// ─── initNewProject ───────────────────────────────────────────────────────
 ...
 */
export const initNewProject = async (_args, projectDir, workstream) => {
    ...
    const hasExaSearch = !!(process.env.EXA_API_KEY ||
    ...
    function findCodeFiles(dir, depth) {
        ...
        for (const entry of entries) {
            if (entry.isFile()) {
                ...
                if (findCodeFiles(join(dir, entry.name), depth + 1))
                    return true;
            }
        }
        return false;
    ...
    const hasPackageFile = pathExists(projectDir, 'package.json') ||
 ...
 */
export const initProgress = async (_args, projectDir, workstream) => {
    const config = await loadConfig(projectDir, workstream);
    ...
    try {
        ...
        for (const dir of dirs) {
            ...
            let status = summaries.length >= plans.length && plans.length > 0 ? 'complete' :
            ...
            const roadmapComplete = checkboxStates.get(phaseNumber) === true ||
 ...
 */
export const initManager = async (_args, projectDir, workstream) => {
    const config = await loadConfig(projectDir, workstream);
    const milestone = await getMilestoneInfo(projectDir, workstream);
    const paths = planningPaths(projectDir, workstream);
    let rawContent;
    ...
    for (const phase of phases) {
        const dependsOnStr = phase.depends_on;
    ...
    for (const phase of phases) {
        const status = phase.disk_status;
    ...
    function reaches(from, to, visited = new Set()) {
        ...
        const depPhases = p?.dep_phases;
        if (!depPhases || depPhases.length === 0)
            return false;
        if (depPhases.includes(to))
            return true;
        return depPhases.some(dep => reaches(dep, to, visited));
    }
    const activeExecuting = phases.filter(p => {
        const status = p.disk_status;
    ...
    let queuedPhases = [];
    let queuedMilestoneVersion = null;
    let queuedMilestoneName = null;
    try {
...
//# sourceMappingURL=init-complex.js.map