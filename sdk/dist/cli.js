...
/**
 * Parse `oto-sdk query …` without rejecting unknown flags (query argv is forwarded to the registry).
 */
...
// ─── Usage ───────────────────────────────────────────────────────────────────
export const USAGE = `
Usage: oto-sdk <command> [args] [options]
...
async function readStdin() {
    ...
    if (stdin.isTTY) {
        throw new Error('No input provided. Usage:\n' +
            '  oto-sdk init @path/to/prd.md\n' +
            '  oto-sdk init "build a todo app"\n' +
            '  cat prd.md | oto-sdk init');
    }
...
/** When false, unknown `oto-sdk query` commands error instead of shelling out to gsd-tools.cjs. */
function queryFallbackToCjsEnabled() {
    const v = process.env.OTO_QUERY_FALLBACK?.toLowerCase();
...
function execGsdToolsCjsQuery(projectDir, gsdToolsPath, normCmd, normArgs, ws) {
    const cjsArgv = dottedCommandToCjsArgv(normCmd, normArgs);
    const wsSuffix = ws ? ['--ws', ws] : [];
    const fullArgv = [gsdToolsPath, ...cjsArgv, ...wsSuffix];
    return new Promise((resolve, reject) => {
        execFile(process.execPath, fullArgv, { cwd: projectDir, maxBuffer: 10 * 1024 * 1024, env: { ...process.env } }, (err, stdout, stderr) => {
            if (err)
                reject(err);
            else
...
export async function main(argv = process.argv.slice(2)) {
    ...
    //
    // When the user launches `oto-sdk` from inside a `sub_repos`-listed child repo,
    ...
    if (args.command === 'query') {
        ...
        if (queryArgs.length === 0 || !queryArgs[0]) {
            console.error('Error: "oto-sdk query" requires a command');
            process.exitCode = 10;
            return;
        }
        try {
            ...
            if (!matched) {
                if (!queryFallbackToCjsEnabled()) {
                    throw new GSDError(`Unknown command: "${tokens.join(' ')}". Use a registered \`oto-sdk query\` subcommand (see sdk/src/query/QUERY-HANDLERS.md) or invoke \`node …/gsd-tools.cjs\` for CJS-only operations. Set OTO_QUERY_FALLBACK=registered (default) to allow automatic fallback.`, ErrorClassification.Validation);
                }
                const { resolveGsdToolsPath } = await import('./gsd-tools.js');
                const gsdPath = resolveGsdToolsPath(args.projectDir);
                console.error(`[oto-sdk] '${tokens.join(' ')}' not in native registry; falling back to gsd-tools.cjs.`);
                console.error('[oto-sdk] Transparent bridge — prefer adding a native handler when parity matters.');
                const { stdout, stderr } = await execGsdToolsCjsQuery(args.projectDir, gsdPath, normCmd, normArgs, args.ws);
                if (stderr.trim())
                    console.error(stderr.trimEnd());
                let output = await parseCliQueryJsonOutput(stdout, args.projectDir);
                if (pickField) {
    ...
    if (args.command !== 'run' && args.command !== 'init' && args.command !== 'auto') {
        console.error('Error: Expected "oto-sdk run <prompt>", "oto-sdk auto", "oto-sdk init [input]", or "oto-sdk query <command>"');
        console.error(USAGE);
        process.exitCode = 1;
        return;
    }
    if (args.command === 'run' && !args.prompt) {
        console.error('Error: "oto-sdk run" requires a prompt');
        console.error(USAGE);
        process.exitCode = 1;
        return;
    }
