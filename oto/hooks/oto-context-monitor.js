...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Quick sentinel check: skip config read entirely for non-OTO projects (#P2.5).
    ...
    if (fs.existsSync(planningDir)) {
      try {
        ...
        // Ignore config read/parse errors (config may not exist in .oto/)
      }
    }
    ...
    // Detect if OTO is active (has .oto/STATE.md in working directory)
    const isGsdActive = fs.existsSync(path.join(cwd, '.oto', 'STATE.md'));

    // On CRITICAL with active OTO project, auto-record session state as a
    // breadcrumb for /oto-resume-work (#1974). Fire-and-forget subprocess —
    ...
    if (isCritical && isGsdActive && !warnData.criticalRecorded) {
      try {
        ...
        // and oto-tools.cjs lives at <runtime-config>/oto/bin/.
        // Using __dirname makes this work on Claude Code, OpenCode, Gemini,
        // Kilo, etc. without hardcoding ~/.claude/.
        const otoTools = path.join(__dirname, '..', 'oto', 'bin', 'oto-tools.cjs');
        ...
        spawn(
          process.execPath,
          [otoTools, 'state', 'record-session', '--stopped-at', stoppedAt],
    ...
    if (isCritical) {
      message = isGsdActive
        ? `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
          ...
          'OTO state is already tracked in STATE.md. Inform the user so they can run ' +
          '/oto-pause-work at the next natural stopping point.'
        : `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
