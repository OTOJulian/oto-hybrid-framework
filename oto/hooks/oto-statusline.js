...
// Claude Code Statusline - OTO Edition
// Shows: model | current task (or OTO state) | directory | context usage
...
/**
 * Walk up from dir looking for .oto/config.json and return its parsed contents.
...
function readGsdConfig(dir) {
  ...
  let current = dir;
  for (let i = 0; i < 10; i++) {
    const candidate = path.join(current, '.oto', 'config.json');
...
function readLastSlashCommand(transcriptPath) {
  ...
  // Command names in Claude Code transcripts are plain identifiers like "oto-plan-phase"
...
/**
 * Walk up from dir looking for .oto/STATE.md.
...
function readGsdState(dir) {
  ...
  let current = dir;
  for (let i = 0; i < 10; i++) {
    const candidate = path.join(current, '.oto', 'STATE.md');
    if (fs.existsSync(candidate)) {
      try {
...
/**
 * Format OTO state into display string.
...
function runStatusline() {
  ...
  try {
    ...
    // OTO state (milestone · status · phase) — shown when no todo task
    const otoStateStr = task ? '' : formatGsdState(readGsdState(dir) || {});

    // OTO update available?
    // Check shared cache first (#1421), fall back to runtime-specific cache for
    // backward compatibility with older oto-check-update.js versions.
    let otoUpdate = '';
    const sharedCacheFile = path.join(homeDir, '.cache', 'oto', 'oto-update-check.json');
    const legacyCacheFile = path.join(claudeDir, 'cache', 'oto-update-check.json');
    const cacheFile = fs.existsSync(sharedCacheFile) ? sharedCacheFile : legacyCacheFile;
    if (fs.existsSync(cacheFile)) {
      try {
        const cache = JSON.parse(fs.readFileSync(cacheFile, 'utf8'));
        if (cache.update_available) {
          otoUpdate = '\x1b[33m⬆ /oto-update\x1b[0m │ ';
        }
        if (cache.stale_hooks && cache.stale_hooks.length > 0) {
          // If installed version is ahead of npm latest, this is a dev install.
          // Running /oto-update would downgrade — show a contextual warning instead.
          const isDevInstall = (() => {
          ...
          if (isDevInstall) {
            otoUpdate += '\x1b[33m⚠ dev install — re-run installer to sync hooks\x1b[0m │ ';
          } else {
            otoUpdate += '\x1b[31m⚠ stale hooks — run /oto-update\x1b[0m │ ';
          }
        }
      } catch (e) {}
    }
    ...
    const middle = task
      ? `\x1b[1m${task}\x1b[0m`
      : otoStateStr
        ? `\x1b[2m${otoStateStr}\x1b[0m`
        : null;

    if (middle) {
      process.stdout.write(`${otoUpdate}\x1b[2m${model}\x1b[0m │ ${middle} │ \x1b[2m${dirname}\x1b[0m${ctx}${lastCmdSuffix}`);
    } else {
      process.stdout.write(`${otoUpdate}\x1b[2m${model}\x1b[0m │ \x1b[2m${dirname}\x1b[0m${ctx}${lastCmdSuffix}`);
    }
  } catch (e) {
...
function renderStatusline(data) {
  ...
  const otoStateStr = formatGsdState(readGsdState(dir) || {});
  const middle = otoStateStr ? `\x1b[2m${otoStateStr}\x1b[0m` : null;
  if (middle) {
