...
| `git.phase_branch_template` | `"oto/phase-{phase}-{slug}"` | Branch template for phase strategy |
| `git.milestone_branch_template` | `"oto/{milestone}-{slug}"` | Branch template for milestone strategy |
...
| `manager.flags.discuss` | `""` | Flags passed to `/oto-discuss-phase` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
oto-sdk query commit "docs: update state" --files .oto/STATE.md
...
INIT=$(oto-sdk query init.execute-phase "1")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
...
- Standard OTO behavior

**When `git.branching_strategy: "phase"`:**
- `execute-phase` creates/switches to a branch before execution
- Branch name from `phase_branch_template` (e.g., `oto/phase-03-authentication`)
...
- Branch name from `milestone_branch_template` (e.g., `oto/v1.0-mvp`)
...
| `commit_docs` | boolean | `true` | `true`, `false` | Commit .oto/ artifacts to git (auto-false if .oto/ is gitignored) |
...
| `review.models.<cli>` | string\|null | `null` | Any model ID string | Per-CLI model override for /oto-review (e.g., `review.models.gemini`). Falls back to CLI default when null. |
...
| `workflow.ai_integration_phase` | boolean | `true` | `true`, `false` | Run /oto-ai-integration-phase before planning AI system phases |
...
| `workflow.security_enforcement` | boolean | `true` | `true`, `false` | Enable threat-model-anchored security verification via `/oto-secure-phase`. When `false`, security checks are skipped entirely |
...
| `git.phase_branch_template` | string | `"oto/phase-{phase}-{slug}"` | Template with `{phase}`, `{slug}` | Branch naming template for `phase` strategy |
| `git.milestone_branch_template` | string | `"oto/{milestone}-{slug}"` | Template with `{milestone}`, `{slug}` | Branch naming template for `milestone` strategy |
...
| `features.global_learnings` | boolean | `false` | `true`, `false` | Enable injection of global learnings from `~/.oto/learnings/` into agent prompts |
...
Set via `intel.*` namespace (e.g., `"intel": { "enabled": true }`). Controls the queryable codebase intelligence system consumed by `/oto-intel`.
...
| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `manager.flags.discuss` | string | `""` | Any CLI flags string | Flags passed to `/oto-discuss-phase` from manager (e.g., `"--auto --analyze"`) |
...
5. **Search API keys and flags** -- `brave_search`, `firecrawl`, and `exa_search` are auto-set to `true` during project creation if the corresponding API key is detected (environment variable or `~/.oto/<name>_api_key` file). Setting them to `true` without the API key has no effect.
...
8. **`sub_repos` auto-sync** -- On every config load, OTO scans for child directories with `.git` and updates the `sub_repos` array if the filesystem has changed. Legacy `multiRepo: true` is automatically migrated to a detected `sub_repos` array.
