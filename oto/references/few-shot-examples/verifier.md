...
**Input:** Must-have artifact: `oto/workflows/context-bridge.md: >50 lines`

**Output:**

> **L1 (Existence):** EXISTS -- `oto/workflows/context-bridge.md` found, 187 lines.
...
> **L3 (Wired):** WIRED -- Referenced by `oto/workflows/execute-phase.md` at step 3. Called by `agents/oto-executor.md` via @-reference include. Arguments verified: receives planning directory path matching the expected parameter.
> **Status: PASSED**
...
> **Method:** Ran `grep -n "scope" oto/bin/validate-commit.js` to locate scope validation logic.
...
> **Ran:** `echo "feat(: test" | node oto/bin/validate-commit.js` -- exit code 0 (accepted).
...
> **Method:** Ran `grep -n "feedback-loop\|extractFeedback" oto/bin/check-subagent-output.js`.
...
> **Cross-check:** `grep -rn "require.*feedback-loop" oto/bin/` returns zero results across all scripts.
...
> **Method:** Ran `grep "architecture_graph\|architecture_guard" oto/bin/lib/config-schema.json`.
> **Evidence:** Zero matches in schema. However, `grep "architecture" oto/bin/lib/config.cjs` shows both flags in CONFIG_DEFAULTS (lines 426, 428).
...
> **Method:** Ran `grep -rn "\/\[a-zA-Z\]\/" oto/bin/` to find inline MSYS regex.
> **Evidence:** 2 matches in `commands/oto/misc.md` (lines 26, 487). These were NOT in the original plan scope -- Phase 148 extracted code into misc.md AFTER Phase 149's plan was written.
...
**Input:** Must-have artifact: `oto/bin/lib/audit-checks/index.cjs: >100 lines`

**Output:**

> **L1 (Existence):** EXISTS -- File found at `oto/bin/lib/audit-checks/index.cjs`, 142 lines.
> **Status: PASSED**
