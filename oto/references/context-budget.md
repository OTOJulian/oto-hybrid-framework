...
3. **Read depth scales with context window** -- check `context_window` in `.oto/config.json`:
...
**How to check:** Read `.oto/config.json` and inspect `context_window`. If the field is absent, treat as 200k (conservative default).
