...
Shared conflict-detection contract for workflows that ingest external content into `.oto/` (e.g., `/oto-import`, `/oto-ingest-docs`). Defines the report format, severity semantics, and safety-gate behavior. The specific checks that populate each severity bucket are workflow-specific and defined by the calling workflow.
...
Display:
```
OTO > BLOCKED: {N} blockers must be resolved before {operation} can proceed.
```
...
Proceed silently or display `OTO > No conflicts detected.` Either is acceptable; workflows choose based on verbosity context.
