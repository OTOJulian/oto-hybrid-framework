...
3. **Read depth scales with context window** -- check `context_window` in `.oto/config.json`. At < 500000: read only frontmatter, status fields, or summaries. At >= 500000 (1M model): full body reads permitted when content is needed for inline decisions. See `references/context-budget.md` for the complete table.
...
8. **Never** read `.oto/logs/` files -- only the health workflow reads these.
...
10. **NEVER** use non-OTO agent types (`general-purpose`, `Explore`, `Plan`, `Bash`, `feature-dev`, etc.) -- ALWAYS use `subagent_type: "oto-{agent}"` (e.g., `oto-phase-researcher`, `oto-executor`, `oto-planner`). OTO agents have project-aware prompts, audit logging, and workflow context. Generic agents bypass all of this.
...
15. **No direct Write/Edit to STATE.md or ROADMAP.md for mutations.** Always use `oto-sdk query` for registered state/roadmap handlers (e.g. `state.update`, `state.advance-plan`, `roadmap.update-plan-progress`), or legacy `node …/oto-tools.cjs` for CLI-only commands. Direct Write tool usage bypasses safe update logic and is unsafe in multi-session environments. Exception: first-time creation of STATE.md from template is allowed.
...
24. **Do not** check for `mode === 'auto'` or `mode === 'autonomous'` -- OTO uses `yolo` config flag. Check `yolo: true` for autonomous mode, absence or `false` for interactive mode.
25. **Prefer `oto-sdk query`** for orchestration when a handler exists; when shelling out to the legacy CLI, use **`oto-tools.cjs`** (not `oto-tools.js` or any other filename) — OTO ships the programmatic API as CommonJS for Node.js CLI compatibility.
26. **Plan files MUST follow `{padded_phase}-{NN}-PLAN.md` pattern** (e.g., `01-01-PLAN.md`). Never use `PLAN-01.md`, `plan-01.md`, or any other variation -- oto-tools detection depends on this exact pattern.
