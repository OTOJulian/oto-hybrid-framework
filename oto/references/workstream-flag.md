...
The `--ws <name>` flag scopes OTO operations to a specific workstream, enabling
...
OTO now prefers a session-scoped pointer keyed by runtime/session identity
(`OTO_SESSION_KEY`, `CODEX_THREAD_ID`, `CLAUDE_CODE_SSE_PORT`, terminal session IDs,
...
When OTO resolves the session-scoped pointer in step 3 above, it uses this order:

1. Explicit runtime/session env vars such as `OTO_SESSION_KEY`, `CODEX_THREAD_ID`,
...
- If that was the last pointer for the repo, OTO also removes the now-empty
  per-project temp directory
- If sibling session pointers still exist, the temp directory is left in place
- When a pointer refers to a workstream directory that no longer exists, OTO
...
OTO does not currently run a background garbage collector for historical temp
...
All workflow routing commands include `${OTO_WS}` which:
...
```bash
# All oto-sdk query commands accept --ws
oto-sdk query state.json --ws feature-a
oto-sdk query find-phase 3 --ws feature-b

# Session-local switching without --ws on every command
OTO_SESSION_KEY=my-terminal-a oto-sdk query workstream.set feature-a
OTO_SESSION_KEY=my-terminal-a oto-sdk query state.json
OTO_SESSION_KEY=my-terminal-b oto-sdk query workstream.set feature-b
OTO_SESSION_KEY=my-terminal-b oto-sdk query state.json

# Workstream CRUD
oto-sdk query workstream.create <name>
oto-sdk query workstream.list
oto-sdk query workstream.status <name>
oto-sdk query workstream.complete <name>
```
