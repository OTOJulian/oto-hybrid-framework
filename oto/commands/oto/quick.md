---
name: oto:quick
description: Execute a quick task with OTO guarantees (atomic commits, state tracking) but skip optional agents
...
---
<objective>
Execute small, ad-hoc tasks with OTO guarantees (atomic commits, STATE.md tracking).
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  oto-sdk query frontmatter.get .oto/quick/{dir}/SUMMARY.md status
  ```
...
```bash
dir=$(ls -d .oto/quick/*-{SLUG}/ 2>/dev/null | head -1)
```
...
Quick Task: {slug}
─────────────────────────────────────
Plan file: .oto/quick/{dir}/PLAN.md
...
─────────────────────────────────────
Resume with: /oto-quick resume {slug}
```
...
Execute the quick workflow from @~/.claude/oto/workflows/quick.md end-to-end.
...
- Status fields read via `oto-sdk query frontmatter.get` — never eval'd or shell-expanded
</security_notes>
