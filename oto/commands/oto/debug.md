...
**Orchestrator role:** Gather symptoms, spawn oto-debugger agent, handle checkpoints, spawn continuations.
...
```bash
ls .oto/debug/*.md 2>/dev/null | grep -v resolved | head -5
```
</context>
...
```bash
debugger_model=$(oto-sdk query resolve-model oto-debugger 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Read TDD mode from config:
```bash
TDD_MODE=$(oto-sdk query config-get workflow.tdd_mode 2>/dev/null | jq -r 'if type == "boolean" then tostring else . end' 2>/dev/null || echo "false")
```
...
─────────────────────────────────────────────
Run `/oto-debug continue <slug>` to resume a session.
No sessions? `/oto-debug <description>` to start.
```

If no files exist or the glob returns nothing: print "No active debug sessions. Run `/oto-debug <issue description>` to start one."
...
<session_params>
slug: {SLUG}
debug_file_path: .oto/debug/{SLUG}.md
