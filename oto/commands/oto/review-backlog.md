...
1. **List backlog items:**
   ```bash
   ls -d .oto/phases/999* 2>/dev/null || echo "No backlog items found"
   ```
...
4. **For items to PROMOTE:**
   ...
   - Rename the directory from `999.x-slug` to `{new_num}-slug`:
     ```bash
     NEW_NUM=$(oto-sdk query phase.add "${DESCRIPTION}" --raw)
     ```
...
6. **Commit changes:**
   ```bash
   oto-sdk query commit "docs: review backlog — promoted N, removed M" --files .oto/ROADMAP.md
   ```
