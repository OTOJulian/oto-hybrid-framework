...
2. **Find next backlog number:**
   ```bash
   NEXT=$(oto-sdk query phase.next-decimal 999 --raw)
   ```
...
3. **Add to ROADMAP.md** under a `## Backlog` section. If the section doesn't exist, create it at the end.
   ...
   **Plans:** 0 plans
   ...
   Plans:
   - [ ] TBD (promote with /oto-review-backlog when ready)
   ```

4. **Create the phase directory:**
   ```bash
   SLUG=$(oto-sdk query generate-slug "$ARGUMENTS" --raw)
   mkdir -p ".oto/phases/${NEXT}-${SLUG}"
   touch ".oto/phases/${NEXT}-${SLUG}/.gitkeep"
   ```

5. **Commit:**
   ```bash
   oto-sdk query commit "docs: add backlog item ${NEXT} — ${ARGUMENTS}" --files .oto/ROADMAP.md ".oto/phases/${NEXT}-${SLUG}/.gitkeep"
   ```
...
6. **Report:**
   ...
   Directory: .oto/phases/{NEXT}-{slug}/

   This item lives in the backlog parking lot.
   Use /oto-discuss-phase {NEXT} to explore it further.
   Use /oto-review-backlog to promote items to active milestone.
   ```
...
- Phase directories are created immediately, so /oto-discuss-phase and /oto-plan-phase work on them
