...
const OTO_CODEX_MARKER = '# OTO Agent Configuration \u2014 managed by oto installer';
const OTO_CODEX_HOOKS_OWNERSHIP_PREFIX = '# OTO codex_hooks ownership: ';

// Copilot instructions marker constants
const OTO_COPILOT_INSTRUCTIONS_MARKER = '<!-- OTO Configuration \u2014 managed by oto installer -->';
const OTO_COPILOT_INSTRUCTIONS_CLOSE_MARKER = '<!-- /OTO Configuration -->';
...
// Hoisted to top with absolute __dirname-based paths so `oto install codex` works
// when invoked via npm global install (cwd is the user's project, not the oto repo
// root). Inline `require('../oto/...')` from inside install functions
...
const _otoLibDir = path.join(__dirname, '..', 'oto', 'bin', 'lib');
const { MODEL_PROFILES: OTO_MODEL_PROFILES } = require(path.join(_otoLibDir, 'model-profiles.cjs'));
const {
  RUNTIME_PROFILE_MAP: OTO_RUNTIME_PROFILE_MAP,
  resolveTierEntry: otoResolveTierEntry,
} = require(path.join(_otoLibDir, 'core.cjs'));

const {
  MINIMAL_SKILL_ALLOWLIST,
  isMinimalMode,
  stageSkillsForMode,
} = require(path.join(_otoLibDir, 'install-profiles.cjs'));
...
const hasPortableHooks = args.includes('--portable-hooks') || process.env.OTO_PORTABLE_HOOKS === '1';
 ...
 * @param {string} hookName - Hook filename (e.g. 'oto-statusline.js')
...
/**
 * Read model_overrides from ~/.oto/defaults.json at install time.
...
function readGsdGlobalModelOverrides() {
  try {
    const defaultsPath = path.join(os.homedir(), '.oto', 'defaults.json');
 ...
 *
 * Merges `~/.oto/defaults.json` (global) with per-project
 * `<project>/.oto/config.json`. Per-project keys win on conflict so a
 ...
 * described — a per-project override for `oto-codebase-mapper` in
...
function readGsdEffectiveModelOverrides(targetDir = null) {
  ...
  if (targetDir) {
    ...
    for (let depth = 0; depth < 8; depth += 1) {
      const candidate = path.join(probeDir, '.oto', 'config.json');
...
/**
 * #2517 — Read a single OTO config file (defaults.json or per-project
...
function _readGsdConfigFile(absPath, label) {
  ...
  try {
    ...
    process.stderr.write(`oto: warning — could not read ${label} (${absPath}): ${err.message}\n`);
    return null;
  }
  try {
    return JSON.parse(raw);
  } catch (err) {
    process.stderr.write(`oto: warning — invalid JSON in ${label} (${absPath}): ${err.message}\n`);
    return null;
  }
}
 ...
 *
 * Probes BOTH per-project `<targetDir>/.oto/config.json` AND
 * `~/.oto/defaults.json`, with per-project keys winning over global. This
 * matches `loadConfig`'s precedence and is the only way the PR's headline claim
 * — "set runtime in .oto/config.json and the Codex TOML emit picks it up"
...
function readGsdRuntimeProfileResolver(targetDir = null) {
  const homeDefaults = _readGsdConfigFile(
    path.join(os.homedir(), '.oto', 'defaults.json'),
    '~/.oto/defaults.json'
  );
  ...
  if (targetDir) {
    ...
    for (let depth = 0; depth < 8; depth += 1) {
      const candidate = path.join(probeDir, '.oto', 'config.json');
      if (fs.existsSync(candidate)) {
        projectConfig = _readGsdConfigFile(candidate, '.oto/config.json');
        break;
      }
      const parent = path.dirname(probeDir);
      if (parent === probeDir) break;
  ...
  // Per-project wins. Only fall back to ~/.oto/defaults.json when the project
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all oto: references → oto-)
  c = c.replace(/oto:/g, 'oto-');
...
/**
 * Map a skill directory name (oto-<cmd>) to the frontmatter `name:` used
 * by Claude Code as the skill identity. Workflows emit `Skill(skill="oto:<cmd>")`
 ...
 *
 * Codex must NOT use this helper: its adapter invokes skills as `$oto-<cmd>`
...
function skillFrontmatterName(skillDirName) {
  ...
  // Only rewrite the first hyphen after the `oto` prefix.
  return skillDirName.replace(/^oto-/, 'oto:');
}
 ...
 * Emits `name: oto:<cmd>` (colon) so Skill(skill="oto:<cmd>") calls in
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agent/');
  c = c.replace(/\.claude\//g, '.agent/');
  // Command name conversion (all oto: references → oto-)
  c = c.replace(/oto:/g, 'oto-');
  // Runtime-neutral agent name replacement (#766)
...
function convertSlashCommandsToCursorSkillMentions(content) {
  // Keep leading "/" for slash commands; only normalize oto: -> oto-.
  // This preserves rendered "next step" commands like "/oto-execute-phase 17".
  return content.replace(/oto:/gi, 'oto-');
}
...
function convertClaudeToCursorMarkdown(content) {
  ...
  converted = converted.replace(/\$ARGUMENTS\b/g, '{{OTO_ARGS}}');
...
- Treat all user text after the skill mention as \`{{OTO_ARGS}}\`.
- If no arguments are present, treat \`{{OTO_ARGS}}\` as empty.
...
## C. Tool Usage
Use these Cursor tools when executing OTO workflows:
...
## C. Tool Usage
Use these Windsurf tools when executing OTO workflows:
- \`Shell\` for running commands (terminal operations)
- \`StrReplace\` for editing existing files
- \`Read\`, \`Write\`, \`Glob\`, \`Grep\`, \`Task\`, \`WebSearch\`, \`WebFetch\`, \`TodoWrite\` as needed
...
## C. Tool Usage
Use these Augment tools when executing OTO workflows:
...
function copyCommandsAsAugmentSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous OTO Augment skills to avoid stale command skills
...
function convertSlashCommandsToTraeSkillMentions(content) {
  return content.replace(/\/oto:([a-z0-9-]+)/g, (_, commandName) => {
    return `/oto-${commandName}`;
  });
}
...
function convertSlashCommandsToCodexSkillMentions(content) {
  ...
  let converted = content.replace(/\/oto:([a-z0-9-]+)/gi, (_, commandName) => {
    return `$oto-${String(commandName).toLowerCase()}`;
  });
  // Convert hyphen-style command references (workflow output) to Codex $ prefix.
  // Negative lookbehind excludes file paths like bin/oto-tools.cjs where
  // the slash is preceded by a word char, dot, or another slash.
  converted = converted.replace(/(?<![a-zA-Z0-9./])\/oto-([a-z0-9-]+)/gi, (_, commandName) => {
    return `$oto-${String(commandName).toLowerCase()}`;
  });
  return converted;
}
...
- Treat all user text after \`${invocation}\` as \`{{OTO_ARGS}}\`.
- If no arguments are present, treat \`{{OTO_ARGS}}\` as empty.
...
function generateCodexAgentToml(agentName, agentContent, modelOverrides = null, runtimeResolver = null) {
  ...
  // Embed model override when configured in ~/.oto/defaults.json so that
...
/**
 * Generate the OTO config block for Codex config.toml.
...
/**
 * Strip any managed OTO agent sections from a TOML string.
 *
 * Handles BOTH shapes so reinstall self-heals configs from all OTO versions:
 *   - Current (#2727): `[agents.oto-*]` struct tables (Codex 0.120.0+).
 *   - Legacy (#2645): `[[agents]]` array-of-tables whose `name = "oto-*"`.
 *
...
function stripCodexGsdAgentSections(content) {
  ...
  const sections = getTomlTableSections(content).filter((section) => {
    // Current `[agents.oto-<name>]` struct tables (#2727, Codex 0.120.0+).
    if (!section.array && /^agents\.oto-/.test(section.path)) {
      return true;
    }

    // Legacy `[[agents]]` array-of-tables (#2645) — only strip blocks whose
    // `name = "oto-..."`, preserving user-authored [[agents]] entries.
    if (section.array && section.path === 'agents') {
      ...
      return Boolean(nameMatch && /^oto-/.test(nameMatch[1]));
    }
...
function stripGsdFromCodexConfig(content) {
  ...
  // No marker but may have OTO-injected feature keys
  let cleaned = content;
  ...
  // Remove [agents.oto-*] sections (from header to next section or EOF)
...
function getManagedCodexHooksOwnership(content) {
  ...
  const afterMarker = content.slice(markerIndex + OTO_CODEX_MARKER.length);
  const match = afterMarker.match(/^\r?\n# OTO codex_hooks ownership: (section|root_dotted)\r?\n/);
...
function setManagedCodexHooksOwnership(content, ownership) {
  ...
  const eol = detectLineEnding(content);
  const markerEnd = markerIndex + OTO_CODEX_MARKER.length;
  ...
  return content.slice(0, markerEnd) +
    eol +
    `${OTO_CODEX_HOOKS_OWNERSHIP_PREFIX}${ownership}${eol}` +
    remainder;
}
...
function stripLeakedGsdCodexSections(content) {
  const leakedSections = getTomlTableSections(content)
    .filter((section) => {
      // Legacy [agents.oto-<name>] map tables (pre-#2645).
      if (!section.array && section.path.startsWith('agents.oto-')) return true;
      ...
      // Current [[agents]] array-of-tables whose name is oto-*. Preserve
      // user-authored [[agents]] entries (other names) untouched.
      if (section.array && section.path === 'agents') {
        const body = content.slice(section.headerEnd, section.end);
        const nameMatch = body.match(/^[ \t]*name[ \t]*=[ \t]*["']([^"']+)["']/m);
        if (nameMatch && /^oto-/.test(nameMatch[1])) return true;
      }
...
/**
 * Merge OTO config block into an existing or new config.toml.
 * Three cases: new file, existing with OTO marker, existing without marker.
 */
function mergeCodexConfig(configPath, otoBlock) {
  // Case 1: No config.toml — create fresh
  if (!fs.existsSync(configPath)) {
    fs.writeFileSync(configPath, otoBlock + '\n');
    return;
  }

  const existing = fs.readFileSync(configPath, 'utf8');
  const eol = detectLineEnding(existing);
  const normalizedGsdBlock = otoBlock.replace(/\r?\n/g, eol);
  const markerIndex = existing.indexOf(OTO_CODEX_MARKER);

  // Case 2: Has OTO marker — truncate and re-append
  if (markerIndex !== -1) {
    let before = existing.substring(0, markerIndex).trimEnd();
    if (before) {
      // Strip any OTO-managed sections that leaked above the marker from previous installs
  ...
  // Case 3: No marker — append OTO block
...
/**
 * Repair config.toml files corrupted by pre-#1346 OTO installs.
...
/**
 * Merge OTO instructions into copilot-instructions.md.
 ...
 * @param {string} otoContent - Template content (without markers)
 */
function mergeCopilotInstructions(filePath, otoContent) {
  const otoBlock = OTO_COPILOT_INSTRUCTIONS_MARKER + '\n' +
    otoContent.trim() + '\n' +
    OTO_COPILOT_INSTRUCTIONS_CLOSE_MARKER;

  // Case 1: No file — create fresh
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, otoBlock + '\n');
    return;
  }

  const existing = fs.readFileSync(filePath, 'utf8');
  const openIndex = existing.indexOf(OTO_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = existing.indexOf(OTO_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  // Case 2: Has OTO markers — replace between markers
  if (openIndex !== -1 && closeIndex !== -1) {
    const before = existing.substring(0, openIndex).trimEnd();
    const after = existing.substring(closeIndex + OTO_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
    let newContent = '';
...
/**
 * Strip OTO section from copilot-instructions.md content.
 * Returns cleaned content, or null if file should be deleted (was OTO-only).
 * @param {string} content - File content
 * @returns {string|null} - Cleaned content or null if empty
 */
function stripGsdFromCopilotInstructions(content) {
  const openIndex = content.indexOf(OTO_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = content.indexOf(OTO_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  if (openIndex !== -1 && closeIndex !== -1) {
    const before = content.substring(0, openIndex).trimEnd();
    const after = content.substring(closeIndex + OTO_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  const agentEntries = fs.readdirSync(agentsSrc).filter(f => f.startsWith('oto-') && f.endsWith('.md'));
  const agents = [];

  // Compute the Codex OTO install path (absolute, so subagents with empty $HOME work — #820)
  const codexGsdPath = `${path.resolve(targetDir, 'oto').replace(/\\/g, '/')}/`;

  for (const file of agentEntries) {
    let content = fs.readFileSync(path.join(agentsSrc, file), 'utf8');
    // Replace full .claude/oto prefix so path resolves to the Codex
    // OTO install before generic .claude → .codex conversion rewrites it.
    content = content.replace(/~\/\.claude\/oto\//g, codexGsdPath);
    content = content.replace(/\$HOME\/\.claude\/oto\//g, codexGsdPath);
    ...
    // $ARGUMENTS → {{OTO_ARGS}}, /clear removal, anchored and bare .claude/
    ...
    // `~/.oto/defaults.json` (project wins on conflict) so Codex TOML files
    ...
    // per-project override the reporter had set for oto-codebase-mapper.
    ...
    // Pass `targetDir` so per-project .oto/config.json wins over global
    // ~/.oto/defaults.json — without this, the PR's headline claim that
  ...
  const otoBlock = generateCodexConfigBlock(agents, targetDir);
  mergeCodexConfig(configPath, otoBlock);
...
function convertClaudeToGeminiAgent(content) {
  ...
  // when they can't be resolved. OTO agents use ${PHASE}, ${PLAN}, etc. as
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
  ...
  // Replace /oto-command colon variant with /oto-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/oto:/g, '/oto-');
  ...
  if (isAgent) {
    ...
    // Embed model override from ~/.oto/defaults.json so model_overrides is
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false } = {}) {
  ...
  convertedContent = convertedContent.replace(/\bSlashCommand\b/g, 'skill');
  convertedContent = convertedContent.replace(/\bTodoWrite\b/g, 'todowrite');
  // Replace /oto-command colon variant with /oto-command for Kilo (flat command structure)
  convertedContent = convertedContent.replace(/\/oto:/g, '/oto-');
 ...
 * OpenCode expects: command/oto-help.md (invoked as /oto-help)
 * Source structure: commands/oto/help.md
 * 
 * @param {string} srcDir - Source directory (e.g., commands/oto/)
 * @param {string} destDir - Destination directory (e.g., command/)
 * @param {string} prefix - Prefix for filenames (e.g., 'oto')
...
function copyFlattenedCommands(srcDir, destDir, prefix, pathPrefix, runtime) {
  ...
  // Remove old oto-*.md files before copying new ones
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // e.g., commands/oto/debug/start.md -> command/oto-debug-start.md
      copyFlattenedCommands(srcPath, destDir, `${prefix}-${entry.name}`, pathPrefix, runtime);
    } else if (entry.name.endsWith('.md')) {
      // Flatten: help.md -> oto-help.md
      const baseName = entry.name.replace('.md', '');
...
function listCodexSkillNames(skillsDir, prefix = 'oto-') {
...
function copyCommandsAsCodexSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous OTO Codex skills to avoid stale command skills.
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyCommandsAsCursorSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous OTO Cursor skills to avoid stale command skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyCommandsAsWindsurfSkills(srcDir, skillsDir, prefix, pathPrefix, runtime) {
  ...
  // Remove previous OTO Windsurf skills to avoid stale command skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function copyCommandsAsCopilotSkills(srcDir, skillsDir, prefix, isGlobal = false) {
  ...
  // Remove previous OTO Copilot skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
 ...
 * Claude Code 2.1.88+ uses skills/xxx/SKILL.md instead of commands/oto/xxx.md.
 ...
 * @param {string} prefix - Skill name prefix (e.g. 'oto')
 * @param {string} pathPrefix - Path prefix for file references
...
function copyCommandsAsClaudeSkills(srcDir, skillsDir, prefix, pathPrefix, runtime, isGlobal = false) {
  ...
  // Remove previous OTO Claude skills to avoid stale command skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
/**
 * Recursively install OTO commands as Antigravity skills.
...
function copyCommandsAsAntigravitySkills(srcDir, skillsDir, prefix, isGlobal = false) {
  ...
  // Remove previous OTO Antigravity skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
/**
 * Single source of truth for user-owned artifacts inside oto/.
 *
 * These files are created/refreshed by user-facing workflows (e.g.
 * /oto-profile-user) and must be preserved across reinstalls. Critically, they
 * MUST be excluded from oto-file-manifest.json — otherwise saveLocalPatches()
 * will compare a refreshed file against a stale manifest hash and emit a
 * spurious "locally modified OTO file" warning (bug #2771).
 *
 ...
 *
 * Paths are relative to the oto/ directory.
 */
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, isCommand = false, isGlobal = false) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      jsContent = jsContent.replace(/oto:/gi, 'oto-');
      ...
      let jsContent = fs.readFileSync(srcPath, 'utf8');
      jsContent = jsContent.replace(/\/oto:([a-z0-9-]+)/g, (_, commandName) => {
        return `/oto-${commandName}`;
      });
...
/**
 * Clean up orphaned files from previous OTO versions
 */
function cleanupOrphanedFiles(configDir) {
  const orphanedFiles = [
    'hooks/oto-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to oto-statusline.js in v1.9.0
  ];
...
function cleanupOrphanedHooks(settings) {
  const orphanedHookPatterns = [
    'oto-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to oto-statusline.js in v1.9.0
    'oto-intel-index.js',  // Removed in v1.9.2
    'oto-intel-session.js',  // Removed in v1.9.2
    'oto-intel-prune.js',  // Removed in v1.9.2
  ];
  ...
  // Fix #330: Update statusLine if it points to old OTO statusline.js path
  // Only match the specific old OTO path pattern (hooks/statusline.js),
  ...
  if (settings.statusLine && settings.statusLine.command &&
    ...
    settings.statusLine.command = settings.statusLine.command.replace(
      /hooks([\/\\])statusline\.js/,
      'hooks$1oto-statusline.js'
    );
    console.log(`  ${green}✓${reset} Updated statusline path (hooks/statusline.js → hooks/oto-statusline.js)`);
  }
...
/**
 * Uninstall OTO from the specified directory for a specific runtime
 * Removes only OTO-specific files/directories, preserves user content
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  console.log(`  Uninstalling OTO from ${cyan}${runtimeLabel}${reset} at ${cyan}${locationLabel}${reset}\n`);
  ...
  // 1. Remove OTO commands/skills
  if (isOpencode || isKilo) {
    // OpenCode/Kilo: remove command/oto-*.md files
    const commandDir = path.join(targetDir, 'command');
    if (fs.existsSync(commandDir)) {
      const files = fs.readdirSync(commandDir);
      for (const file of files) {
        if (file.startsWith('oto-') && file.endsWith('.md')) {
          fs.unlinkSync(path.join(commandDir, file));
          removedCount++;
        }
      }
      console.log(`  ${green}✓${reset} Removed OTO commands from command/`);
    }
  } else if (isCodex || isCursor || isWindsurf || isTrae || isCodebuddy) {
    // Codex/Cursor/Windsurf/Trae/CodeBuddy: remove skills/oto-*/SKILL.md skill directories
    ...
    // Codex-only: remove OTO agent .toml config files and config.toml sections
    if (isCodex) {
      ...
      if (fs.existsSync(codexAgentsDir)) {
        ...
        for (const file of tomlFiles) {
          if (file.startsWith('oto-') && file.endsWith('.toml')) {
      ...
      // Codex: clean OTO sections from config.toml
      const configPath = path.join(targetDir, 'config.toml');
      if (fs.existsSync(configPath)) {
        ...
        if (cleaned === null) {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed config.toml (was OTO-only)`);
        } else if (cleaned !== content) {
          fs.writeFileSync(configPath, cleaned);
          removedCount++;
          console.log(`  ${green}✓${reset} Cleaned OTO sections from config.toml`);
        }
      }
    }
  } else if (isCopilot) {
    // Copilot: remove skills/oto-*/ directories (same layout as Codex skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Copilot: clean OTO section from copilot-instructions.md
    ...
    if (fs.existsSync(instructionsPath)) {
      ...
      if (cleaned === null) {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed copilot-instructions.md (was OTO-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(instructionsPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned OTO section from copilot-instructions.md`);
      }
    }
  } else if (isAntigravity) {
    // Antigravity: remove skills/oto-*/ directories (same layout as Copilot skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    const legacyCommandsDir = path.join(targetDir, 'commands', 'oto');
    if (fs.existsSync(legacyCommandsDir)) {
      ...
      removedCount++;
      console.log(`  ${green}✓${reset} Removed legacy commands/oto/`);
      restoreUserArtifacts(legacyCommandsDir, savedLegacyArtifacts);
    }
  } else if (isGemini) {
    // Gemini: still uses commands/oto/
    const otoCommandsDir = path.join(targetDir, 'commands', 'oto');
    if (fs.existsSync(otoCommandsDir)) {
      ...
      const devPrefsPath = path.join(otoCommandsDir, 'dev-preferences.md');
      const preservedDevPrefs = fs.existsSync(devPrefsPath) ? fs.readFileSync(devPrefsPath, 'utf-8') : null;

      fs.rmSync(otoCommandsDir, { recursive: true });
      removedCount++;
      console.log(`  ${green}✓${reset} Removed commands/oto/`);

      // Restore user-generated files
      if (preservedDevPrefs) {
        try {
          fs.mkdirSync(otoCommandsDir, { recursive: true });
          fs.writeFileSync(devPrefsPath, preservedDevPrefs);
          console.log(`  ${green}✓${reset} Preserved commands/oto/dev-preferences.md`);
        } catch (err) {
    ...
    // Claude Code global: remove skills/oto-*/ directories (primary global install location)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    ...
    // Also clean up legacy commands/oto/ from older global installs
    const legacyCommandsDir = path.join(targetDir, 'commands', 'oto');
    if (fs.existsSync(legacyCommandsDir)) {
    ...
    // Claude Code local: remove commands/oto/ (primary local install location since #1736)
    const otoCommandsDir = path.join(targetDir, 'commands', 'oto');
    if (fs.existsSync(otoCommandsDir)) {
      // Preserve user-generated files before wipe (#1423)
      const devPrefsPath = path.join(otoCommandsDir, 'dev-preferences.md');
  ...
  // 2. Remove oto directory
  const otoDir = path.join(targetDir, 'oto');
  if (fs.existsSync(otoDir)) {
    // Preserve user-generated files before wipe (#1423)
    const userProfilePath = path.join(otoDir, 'USER-PROFILE.md');
    const preservedProfile = fs.existsSync(userProfilePath) ? fs.readFileSync(userProfilePath, 'utf-8') : null;

    fs.rmSync(otoDir, { recursive: true });
    removedCount++;
    console.log(`  ${green}✓${reset} Removed oto/`);
    ...
    if (preservedProfile) {
      try {
        ...
        console.log(`  ${green}✓${reset} Preserved oto/USER-PROFILE.md`);
      } catch (err) {
  ...
  // 3. Remove OTO agents (oto-*.md files only)
  ...
  if (fs.existsSync(agentsDir)) {
    ...
    if (agentCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${agentCount} OTO agents`);
    }
  }
  ...
  if (fs.existsSync(hooksDir)) {
    const otoHooks = ['oto-statusline.js', 'oto-check-update.js', 'oto-context-monitor.js', 'oto-prompt-guard.js', 'oto-read-guard.js', 'oto-read-injection-scanner.js', 'oto-workflow-guard.js', 'oto-session-state.sh', 'oto-validate-commit.sh', 'oto-phase-boundary.sh'];
    let hookCount = 0;
    for (const hook of otoHooks) {
    ...
    if (hookCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${hookCount} OTO hooks`);
    }
  }

  // 5. Remove OTO package.json (CommonJS mode marker)
  ...
  if (fs.existsSync(pkgJsonPath)) {
    try {
      ...
      if (content === '{"type":"commonjs"}') {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed OTO package.json`);
      }
    } catch (e) {
      // Ignore read errors
    }
  ...
  // 6. Clean up settings.json (remove OTO hooks and statusline)
  ...
  if (fs.existsSync(settingsPath)) {
    ...
    // Remove OTO statusline if it references our hook
    if (settings.statusLine && settings.statusLine.command &&
      settings.statusLine.command.includes('oto-statusline')) {
      delete settings.statusLine;
      settingsModified = true;
      console.log(`  ${green}✓${reset} Removed OTO statusline from settings`);
    }

    // Remove OTO hooks from settings — per-hook granularity to preserve
    // user hooks that share an entry with a OTO hook (#1755 followup)
    const isGsdHookCommand = (cmd) =>
      cmd && (cmd.includes('oto-check-update') || cmd.includes('oto-statusline') ||
        cmd.includes('oto-session-state') || cmd.includes('oto-context-monitor') ||
        cmd.includes('oto-phase-boundary') || cmd.includes('oto-prompt-guard') ||
        cmd.includes('oto-read-guard') || cmd.includes('oto-read-injection-scanner') ||
        cmd.includes('oto-validate-commit') || cmd.includes('oto-workflow-guard'));
      ...
      if (settings.hooks && settings.hooks[eventName]) {
        ...
        settings.hooks[eventName] = settings.hooks[eventName]
          .map(entry => {
            ...
            // Filter out individual OTO hooks, keep user hooks
    ...
    if (settingsModified) {
      console.log(`  ${green}✓${reset} Removed OTO hooks from settings`);
    }
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        if (modified) {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed OTO permissions from ${path.basename(configPath)}`);
        }
      } catch (e) {
  ...
  if (removedCount === 0) {
    console.log(`  ${yellow}⚠${reset} No OTO files found to remove.`);
  }

  console.log(`
  ${green}Done!${reset} OTO has been uninstalled from ${runtimeLabel}.
...
/**
 * Configure OpenCode permissions to allow reading OTO reference docs
 * This prevents permission prompts when OTO accesses the oto directory
...
function configureOpencodePermissions(isGlobal = true, configDir = null) {
  ...
  // Build the OTO path using the actual config directory
  // Use ~ shorthand if it's in the default location, otherwise use full path
  const defaultConfigDir = path.join(os.homedir(), '.config', 'opencode');
  const otoPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/oto/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/oto/*`;
  ...
  if (config.permission.read[otoPath] !== 'allow') {
    config.permission.read[otoPath] = 'allow';
    modified = true;
  }
  ...
  if (config.permission.external_directory[otoPath] !== 'allow') {
    config.permission.external_directory[otoPath] = 'allow';
    modified = true;
  }
  ...
  // Write config back
  fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
  console.log(`  ${green}✓${reset} Configured read permission for OTO docs`);
}

/**
 * Configure Kilo permissions to allow reading OTO reference docs
 * This prevents permission prompts when OTO accesses the oto directory
 * @param {boolean} isGlobal - Whether this is a global or local install
 * @param {string|null} configDir - Resolved config directory when already known
 */
function configureKiloPermissions(isGlobal = true, configDir = null) {
  ...
  const otoPath = kiloConfigDir === defaultConfigDir
    ? '~/.config/kilo/oto/*'
    : `${kiloConfigDir.replace(/\\/g, '/')}/oto/*`;
...
const PATCHES_DIR_NAME = 'oto-local-patches';
const MANIFEST_NAME = 'oto-file-manifest.json';
...
function writeManifest(configDir, runtime = 'claude', options = {}) {
  ...
  const isTrae = runtime === 'trae';
  const isCline = runtime === 'cline';
  const otoDir = path.join(configDir, 'oto');
  const commandsDir = path.join(configDir, 'commands', 'oto');
  ...
  const otoHashes = generateManifest(otoDir);
  for (const [rel, hash] of Object.entries(otoHashes)) {
    ...
    manifest.files['oto/' + rel] = hash;
  }
  if (isGemini && fs.existsSync(commandsDir)) {
    const cmdHashes = generateManifest(commandsDir);
    for (const [rel, hash] of Object.entries(cmdHashes)) {
      manifest.files['commands/oto/' + rel] = hash;
    }
  }
  ...
  if (!isCodex && !isCopilot && !isCline) {
    ...
    if (fs.existsSync(hooksDir)) {
      for (const file of fs.readdirSync(hooksDir)) {
        if (file.startsWith('oto-') && (file.endsWith('.js') || file.endsWith('.sh'))) {
...
/**
 * Detect user-modified OTO files by comparing against install manifest.
 * Backs up modified files to oto-local-patches/ for reapply after update.
 * Also saves pristine copies (from manifest) to oto-pristine/ to enable
...
function saveLocalPatches(configDir) {
  ...
  if (manifest.files) {
    for (const artifact of USER_OWNED_ARTIFACTS) {
      delete manifest.files[`oto/${artifact}`];
    }
  }

  const patchesDir = path.join(configDir, PATCHES_DIR_NAME);
  const pristineDir = path.join(configDir, 'oto-pristine');
  const modified = [];
  ...
  // These represent the original OTO distribution files that the user then modified.
  ...
  if (modified.length > 0) {
    ...
    // knows which OTO version these files came from, enabling npm-based reconstruction.
    const meta = {
    ...
    console.log('  ' + yellow + 'i' + reset + '  Found ' + modified.length + ' locally modified OTO file(s) — backed up to ' + PATCHES_DIR_NAME + '/');
  ...
  if (meta.files && meta.files.length > 0) {
    const reapplyCommand = (runtime === 'opencode' || runtime === 'kilo' || runtime === 'copilot')
      ...
      : runtime === 'codex'
        ? '$oto-reapply-patches'
        : runtime === 'cursor'
          ? 'oto-reapply-patches (mention the skill name)'
          : '/oto-reapply-patches';
    console.log('');
...
function install(isGlobal, runtime = 'claude') {
  ...
  // Path prefix for file references in markdown content (e.g. oto-tools.cjs).
  // Replaces $HOME/.claude/ or ~/.claude/ so the result is <pathPrefix>oto/bin/...
  ...
  // Save any locally modified OTO files before they get wiped
  saveLocalPatches(targetDir);

  // Clean up orphaned files from previous versions
  cleanupOrphanedFiles(targetDir);

  // OpenCode/Kilo use command/ (flat), Codex uses skills/, Claude/Gemini use commands/oto/
  if (isOpencode || isKilo) {
    // OpenCode/Kilo: flat structure in command/ directory
    const commandDir = path.join(targetDir, 'command');
    fs.mkdirSync(commandDir, { recursive: true });

    // Copy commands/oto/*.md as command/oto-*.md (flatten structure)
    const otoSrc = stageSkillsForMode(path.join(src, 'commands', 'oto'), installMode);
    copyFlattenedCommands(otoSrc, commandDir, 'oto', pathPrefix, runtime);
    if (verifyInstalled(commandDir, 'command/oto-*')) {
      const count = fs.readdirSync(commandDir).filter(f => f.startsWith('oto-')).length;
      console.log(`  ${green}✓${reset} Installed ${count} commands to command/`);
    } else {
      failures.push('command/oto-*');
    }
  } else if (isCodex) {
    const skillsDir = path.join(targetDir, 'skills');
    const otoSrc = stageSkillsForMode(path.join(src, 'commands', 'oto'), installMode);
    copyCommandsAsCodexSkills(otoSrc, skillsDir, 'oto', pathPrefix, runtime);
    const installedSkillNames = listCodexSkillNames(skillsDir);
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
      failures.push('skills/oto-*');
    }
  } else if (isCopilot) {
    const skillsDir = path.join(targetDir, 'skills');
    const otoSrc = stageSkillsForMode(path.join(src, 'commands', 'oto'), installMode);
    copyCommandsAsCopilotSkills(otoSrc, skillsDir, 'oto', isGlobal);
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('oto-')).length;
      if (count > 0) {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const otoSrc = stageSkillsForMode(path.join(src, 'commands', 'oto'), installMode);
    copyCommandsAsAntigravitySkills(otoSrc, skillsDir, 'oto', isGlobal);
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('oto-')).length;
      if (count > 0) {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const otoSrc = stageSkillsForMode(path.join(src, 'commands', 'oto'), installMode);
    copyCommandsAsCursorSkills(otoSrc, skillsDir, 'oto', pathPrefix, runtime);
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const otoSrc = stageSkillsForMode(path.join(src, 'commands', 'oto'), installMode);
    copyCommandsAsWindsurfSkills(otoSrc, skillsDir, 'oto', pathPrefix, runtime);
    const installedSkillNames = listCodexSkillNames(skillsDir); // reuse — same dir structure
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const otoSrc = stageSkillsForMode(path.join(src, 'commands', 'oto'), installMode);
    copyCommandsAsAugmentSkills(otoSrc, skillsDir, 'oto', pathPrefix, runtime);
    const installedSkillNames = listCodexSkillNames(skillsDir);
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const otoSrc = stageSkillsForMode(path.join(src, 'commands', 'oto'), installMode);
    copyCommandsAsTraeSkills(otoSrc, skillsDir, 'oto', pathPrefix, runtime);
    const installedSkillNames = listCodexSkillNames(skillsDir);
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
    ...
    const skillsDir = path.join(targetDir, 'skills');
    const otoSrc = stageSkillsForMode(path.join(src, 'commands', 'oto'), installMode);
    copyCommandsAsClaudeSkills(otoSrc, skillsDir, 'oto', pathPrefix, runtime, isGlobal);
    if (fs.existsSync(skillsDir)) {
      const count = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('oto-')).length;
      if (count > 0) {
    ...
    if (fs.existsSync(legacyCommandsDir)) {
      const savedLegacyArtifacts = preserveUserArtifacts(legacyCommandsDir, ['dev-preferences.md']);
      fs.rmSync(legacyCommandsDir, { recursive: true });
      console.log(`  ${green}✓${reset} Removed legacy commands/oto/ directory`);
      restoreUserArtifacts(legacyCommandsDir, savedLegacyArtifacts);
    }
  } else if (isCodebuddy) {
    const skillsDir = path.join(targetDir, 'skills');
    const otoSrc = stageSkillsForMode(path.join(src, 'commands', 'oto'), installMode);
    copyCommandsAsCodebuddySkills(otoSrc, skillsDir, 'oto', pathPrefix, runtime);
    const installedSkillNames = listCodexSkillNames(skillsDir);
    if (installedSkillNames.length > 0) {
      console.log(`  ${green}✓${reset} Installed ${installedSkillNames.length} skills to skills/`);
    } else {
    ...
    const otoSrc = stageSkillsForMode(path.join(src, 'commands', 'oto'), installMode);
    const otoDest = path.join(commandsDir, 'oto');
    copyWithPathReplacement(otoSrc, otoDest, pathPrefix, runtime, true, isGlobal);
    if (verifyInstalled(otoDest, 'commands/oto')) {
      console.log(`  ${green}✓${reset} Installed commands/oto`);
    } else {
      failures.push('commands/oto');
    }
  } else if (isGlobal) {
    ...
    // Clean up legacy commands/oto/ from previous global installs
    ...
    // Claude Code local: commands/oto/ format — Claude Code reads local project
    // commands from .claude/commands/oto/, not .claude/skills/
    const commandsDir = path.join(targetDir, 'commands');
    fs.mkdirSync(commandsDir, { recursive: true });
    const otoSrc = stageSkillsForMode(path.join(src, 'commands', 'oto'), installMode);
    const otoDest = path.join(commandsDir, 'oto');
    copyWithPathReplacement(otoSrc, otoDest, pathPrefix, runtime, true, isGlobal);
    if (verifyInstalled(otoDest, 'commands/oto')) {
      const count = fs.readdirSync(otoDest).filter(f => f.endsWith('.md')).length;
      console.log(`  ${green}✓${reset} Installed ${count} commands to commands/oto/`);
    } else {
      failures.push('commands/oto');
    }
    ...
    if (fs.existsSync(staleSkillsDir)) {
      const staleGsd = fs.readdirSync(staleSkillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('oto-'));
      for (const e of staleGsd) {
        fs.rmSync(path.join(staleSkillsDir, e.name), { recursive: true });
      }
      if (staleGsd.length > 0) {
        console.log(`  ${green}✓${reset} Removed ${staleGsd.length} stale OTO skill(s) from skills/`);
      }
    }
  }

  // Copy oto skill with path replacement
  // Preserve user-generated files before the wipe-and-copy so they survive re-install
  const skillSrc = path.join(src, 'oto');
  const skillDest = path.join(targetDir, 'oto');
  ...
  if (verifyInstalled(skillDest, 'oto')) {
    console.log(`  ${green}✓${reset} Installed oto`);
  } else {
    failures.push('oto');
  }

  // Copy agents to agents directory.
  // Skipped under --minimal: oto-* subagent descriptions are eagerly loaded
  // into the runtime's Agent tool schema, costing ~6k tokens per turn even
  // when no OTO workflow is active. See oto-build/oto#2762.
  const agentsSrc = path.join(src, 'agents');
  const agentsDest = path.join(targetDir, 'agents');

  // Always remove stale oto-* agents first so re-installing with
  ...
  if (isMinimalMode(installMode)) {
    // Codex registers agents in `config.toml` via `[agents.oto-*]` sections.
    ...
    console.log(`  ${dim}↳${reset} Skipping agents (minimal install — run \`oto update\` without \`--minimal\` to add full surface)`);
    ...
    for (const entry of agentEntries) {
      if (entry.isFile() && entry.name.endsWith('.md')) {
        ...
        if (isOpencode) {
          ...
          // `.planning/config.json` and `~/.oto/defaults.json`, with
  ...
  const changelogDest = path.join(targetDir, 'oto', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'oto', 'VERSION');
  ...
  if (!isCodex && !isCopilot && !isCursor && !isWindsurf && !isTrae && !isCline) {
    // Write package.json to force CommonJS mode for OTO scripts
    ...
    if (fs.existsSync(hooksSrc)) {
      ...
      for (const entry of hookEntries) {
        ...
        if (fs.statSync(srcFile).isFile()) {
          ...
          // and stamp the current OTO version into the hook version header
          if (entry.endsWith('.js')) {
            ...
            content = content.replace(/\{\{OTO_VERSION\}\}/g, pkg.version);
            ...
            // .sh hooks carry a oto-hook-version header so oto-check-update.js can
      ...
      if (verifyInstalled(hooksDest, 'hooks')) {
        ...
        const expectedShHooks = ['oto-session-state.sh', 'oto-validate-commit.sh', 'oto-phase-boundary.sh'];
  ...
  // Cache lives at ~/.cache/oto/ (see hooks/oto-check-update.js line 35-36)
  const updateCacheFile = path.join(os.homedir(), '.cache', 'oto', 'oto-update-check.json');
  ...
  if (isCodex && !isMinimalMode(installMode)) {
    ...
    // oto-check-update.js in config.toml — the file must physically exist.
    ...
    try {
      ...
      // Codex 0.124.0 requires [[hooks]] array-of-tables; old OTO installs wrote
      ...
      const updateCheckScript = path.resolve(targetDir, 'hooks', 'oto-check-update.js').replace(/\\/g, '/');
      const hookBlock =
        `${eol}# OTO Hooks${eol}` +
        `[[hooks]]${eol}` +
      ...
      // Migrate legacy oto-update-check entries from prior installs (#1755 followup)
      ...
      if (configContent.includes('oto-update-check')) {
        configContent = configContent.replace(
          /(?:\r?\n|^)# OTO Hooks\r?\n\[\[hooks\]\]\r?\nevent = "SessionStart"\r?\ncommand = "node [^\r\n]*oto-update-check\.js"\r?\n/gm,
          (match) => (match.startsWith('\r\n') ? '\r\n' : match.startsWith('\n') ? '\n' : ''),
        );
      }

      if (hasEnabledCodexHooksFeature(configContent) && !configContent.includes('oto-check-update')) {
  ...
  if (isCopilot) {
    ...
    const templatePath = path.join(targetDir, 'oto', 'templates', 'copilot-instructions.md');
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
  ...
  if (isCline) {
    // Cline uses .clinerules — generate a rules file with OTO system instructions
    const clinerulesDest = path.join(targetDir, '.clinerules');
    const clinerules = [
      '# OTO — oto',
      '',
      '- OTO workflows live in `oto/workflows/`. Load the relevant workflow when',
      '  the user runs a `/oto-*` command.',
      '- OTO agents live in `agents/`. Use the matching agent when spawning subagents.',
      '- OTO tools are at `oto/bin/oto-tools.cjs`. Run with `node`.',
      '- Planning artifacts live in `.planning/`. Never edit them outside a OTO workflow.',
      '- Do not apply OTO workflows unless the user explicitly asks for them.',
      '- When a OTO command triggers a deliverable (feature, fix, docs), offer the next',
  ...
  const statuslineCommand = isGlobal
    ? buildHookCommand(targetDir, 'oto-statusline.js', hookOpts)
    : 'node ' + localPrefix + '/hooks/oto-statusline.js';
  const updateCheckCommand = isGlobal
    ? buildHookCommand(targetDir, 'oto-check-update.js', hookOpts)
    : 'node ' + localPrefix + '/hooks/oto-check-update.js';
  const contextMonitorCommand = isGlobal
    ? buildHookCommand(targetDir, 'oto-context-monitor.js', hookOpts)
    : 'node ' + localPrefix + '/hooks/oto-context-monitor.js';
  const promptGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'oto-prompt-guard.js', hookOpts)
    : 'node ' + localPrefix + '/hooks/oto-prompt-guard.js';
  const readGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'oto-read-guard.js', hookOpts)
    : 'node ' + localPrefix + '/hooks/oto-read-guard.js';
  const readInjectionScannerCommand = isGlobal
    ? buildHookCommand(targetDir, 'oto-read-injection-scanner.js', hookOpts)
    : 'node ' + localPrefix + '/hooks/oto-read-injection-scanner.js';
  ...
  if (!isOpencode && !isKilo) {
    ...
    const hasGsdUpdateHook = settings.hooks.SessionStart.some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('oto-check-update'))
    );
    ...
    const checkUpdateFile = path.join(targetDir, 'hooks', 'oto-check-update.js');
    if (!hasGsdUpdateHook && fs.existsSync(checkUpdateFile)) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped update check hook — oto-check-update.js not found at target`);
    }
    ...
    const hasContextMonitorHook = settings.hooks[postToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('oto-context-monitor'))
    );

    const contextMonitorFile = path.join(targetDir, 'hooks', 'oto-context-monitor.js');
    if (!hasContextMonitorHook && fs.existsSync(contextMonitorFile)) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped context monitor hook — oto-context-monitor.js not found at target`);
    } else {
      // Migrate existing context monitor hooks: add matcher and timeout if missing
      for (const entry of settings.hooks[postToolEvent]) {
        if (entry.hooks && entry.hooks.some(h => h.command && h.command.includes('oto-context-monitor'))) {
          let migrated = false;
          if (!entry.matcher) {
          ...
          for (const h of entry.hooks) {
            if (h.command && h.command.includes('oto-context-monitor') && !h.timeout) {
              h.timeout = 10;
              migrated = true;
            }
          }
    ...
    const hasPromptGuardHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('oto-prompt-guard'))
    );

    const promptGuardFile = path.join(targetDir, 'hooks', 'oto-prompt-guard.js');
    if (!hasPromptGuardHook && fs.existsSync(promptGuardFile)) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped prompt guard hook — oto-prompt-guard.js not found at target`);
    }
    ...
    const hasReadGuardHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('oto-read-guard'))
    );

    const readGuardFile = path.join(targetDir, 'hooks', 'oto-read-guard.js');
    if (!hasReadGuardHook && fs.existsSync(readGuardFile)) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped read guard hook — oto-read-guard.js not found at target`);
    }
    ...
    const hasReadInjectionScannerHook = settings.hooks[postToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('oto-read-injection-scanner'))
    );

    const readInjectionScannerFile = path.join(targetDir, 'hooks', 'oto-read-injection-scanner.js');
    if (!hasReadInjectionScannerHook && fs.existsSync(readInjectionScannerFile)) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped read injection scanner hook — oto-read-injection-scanner.js not found at target`);
    }

    // Community hooks — registered on install but opt-in at runtime.
    // Each hook checks .oto/config.json for hooks.community: true
    ...
    // Detects file edits outside OTO workflow context and advises using
    // /oto-quick or /oto-fast for state-tracked changes. Advisory only.
    const workflowGuardCommand = isGlobal
      ? buildHookCommand(targetDir, 'oto-workflow-guard.js', hookOpts)
      : 'node ' + localPrefix + '/hooks/oto-workflow-guard.js';
    const hasWorkflowGuardHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('oto-workflow-guard'))
    );

    const workflowGuardFile = path.join(targetDir, 'hooks', 'oto-workflow-guard.js');
    if (!hasWorkflowGuardHook && fs.existsSync(workflowGuardFile)) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped workflow guard hook — oto-workflow-guard.js not found at target`);
    }

    // Configure commit validation hook (Conventional Commits enforcement, opt-in)
    const validateCommitCommand = isGlobal
      ? buildHookCommand(targetDir, 'oto-validate-commit.sh', hookOpts)
      : 'bash ' + localPrefix + '/hooks/oto-validate-commit.sh';
    const hasValidateCommitHook = settings.hooks[preToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('oto-validate-commit'))
    );
    ...
    const validateCommitFile = path.join(targetDir, 'hooks', 'oto-validate-commit.sh');
    if (!hasValidateCommitHook && fs.existsSync(validateCommitFile)) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped commit validation hook — oto-validate-commit.sh not found at target`);
    }

    // Configure session state orientation hook (opt-in)
    const sessionStateCommand = isGlobal
      ? buildHookCommand(targetDir, 'oto-session-state.sh', hookOpts)
      : 'bash ' + localPrefix + '/hooks/oto-session-state.sh';
    const hasSessionStateHook = settings.hooks.SessionStart.some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('oto-session-state'))
    );
    const sessionStateFile = path.join(targetDir, 'hooks', 'oto-session-state.sh');
    if (!hasSessionStateHook && fs.existsSync(sessionStateFile)) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped session state hook — oto-session-state.sh not found at target`);
    }

    // Configure phase boundary detection hook (opt-in)
    const phaseBoundaryCommand = isGlobal
      ? buildHookCommand(targetDir, 'oto-phase-boundary.sh', hookOpts)
      : 'bash ' + localPrefix + '/hooks/oto-phase-boundary.sh';
    const hasPhaseBoundaryHook = settings.hooks[postToolEvent].some(entry =>
      entry.hooks && entry.hooks.some(h => h.command && h.command.includes('oto-phase-boundary'))
    );
    const phaseBoundaryFile = path.join(targetDir, 'hooks', 'oto-phase-boundary.sh');
    if (!hasPhaseBoundaryHook && fs.existsSync(phaseBoundaryFile)) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped phase boundary hook — oto-phase-boundary.sh not found at target`);
    }
  }
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true, configDir = null) {
  ...
  // For non-Claude runtimes, set resolve_model_ids: "omit" in ~/.oto/defaults.json
  ...
  // See #1156.
  if (runtime !== 'claude') {
    const otoDir = path.join(os.homedir(), '.oto');
    const defaultsPath = path.join(otoDir, 'defaults.json');
    try {
      fs.mkdirSync(otoDir, { recursive: true });
      let defaults = {};
      ...
      if (defaults.resolve_model_ids !== 'omit') {
        ...
        console.log(`  ${green}✓${reset} Set resolve_model_ids: "omit" in ~/.oto/defaults.json`);
      }
    } catch (e) {
      console.log(`  ${yellow}⚠${reset} Could not write ~/.oto/defaults.json: ${e.message}`);
    }
  }
  ...
  let command = '/oto-new-project';
  if (runtime === 'opencode') command = '/oto-new-project';
  if (runtime === 'kilo') command = '/oto-new-project';
  if (runtime === 'codex') command = '$oto-new-project';
  if (runtime === 'copilot') command = '/oto-new-project';
  if (runtime === 'antigravity') command = '/oto-new-project';
  if (runtime === 'cursor') command = 'oto-new-project (mention the skill name)';
  if (runtime === 'windsurf') command = '/oto-new-project';
  if (runtime === 'augment') command = '/oto-new-project';
  if (runtime === 'trae') command = '/oto-new-project';
  if (runtime === 'cline') command = '/oto-new-project';
  if (runtime === 'qwen') command = '/oto-new-project';
  console.log(`
...
function handleStatusline(settings, isInteractive, callback) {
  ...
  OTO includes a statusline showing:
    • Model name
    • Current task (from todo list)
    • Context window usage (color-coded)

  ${cyan}1${reset}) Keep existing
  ${cyan}2${reset}) Replace with OTO statusline
`);
...
function maybeSuggestPathExport(globalBin, homeDir) {
  ...
  if (homePathCoveredByRc(globalBin, homeDir)) {
    console.log(`  ${yellow}⚠${reset} ${bold}oto-sdk${reset}'s directory is already on your PATH via an rc file entry — try reopening your shell (or ${cyan}source ~/.zshrc${reset}).`);
    return;
  }
...
/**
 * Verify the prebuilt SDK dist is present and the oto-sdk shim is wired up.
 *
 * As of fix/2441-sdk-decouple, sdk/dist/ is shipped prebuilt inside the
 * oto-cc npm tarball. The parent package declares a bin entry
 * "oto-sdk": "bin/oto-sdk.js" so npm chmods the shim correctly when
 ...
 *       User ran `npx oto-cc@latest`. sdk/ lives under
 *       `<npm-cache>/_npx/<hash>/node_modules/oto-cc/sdk` which
 ...
 *       User ran a global install (`npm i -g oto-cc`). sdk/dist
...
function classifySdkInstall(sdkDir) {
  ...
  if (!readOnly) {
    try {
      const probe = path.join(sdkDir, `.oto-write-probe-${process.pid}`);
...
function installSdkIfNeeded(opts) {
  ...
  if (hasNoSdk && !opts.sdkDir) {
    console.log(`\n  ${dim}Skipping OTO SDK check (--no-sdk)${reset}`);
    return;
  }
  ...
  if (opts.isLocal) {
    console.warn(`\n  ${yellow}⚠${reset}  Skipping SDK check for local install — install @oto-build/sdk globally if you need /oto-* CLI support.`);
    return;
  }
  ...
  if (!fs.existsSync(sdkCliPath)) {
    ...
    console.error(`${redBold}  ✗ OTO SDK dist not found — /oto-* commands will not work${reset}`);
    console.error(`${redBold}${bar}${reset}`);
    console.error(`  ${red}Reason:${reset} sdk/dist/cli.js not found at ${sdkCliPath}`);
    console.error('');

    if (ctx.mode === 'tarball') {
      // User install (including `npx oto-cc@latest`, which stages
      ...
      console.error(`    ${cyan}npm install -g oto-cc@latest${reset}`);
      console.error(`  Or, if you prefer a one-shot run, clear the npx cache first:`);
      console.error(`    ${cyan}npx --yes oto-cc@latest${reset}`);
      console.error(`  Or build from source (git clone):`);
      console.error(`    ${cyan}git clone https://github.com/OTOJulian/oto-hybrid-framework && cd oto/sdk && npm install && npm run build${reset}`);
    } else {
  ...
  try {
    ...
    // `node sdkCliPath` invocation in bin/oto-sdk.js.
  }

  // #2775: do not assert "OTO SDK ready" until `oto-sdk` actually resolves on
  // PATH. `npx oto-cc` only links the package's primary bin; the
  // secondary `oto-sdk` shim is left dangling under the npx cache and is NOT
  ...
  // (`command -v oto-sdk` resolving), and led to a false ✓ in npx-cache
  // installs (issue #2775).
  const shimSrc = path.resolve(__dirname, 'oto-sdk.js');
  ...
  if (!onPath) {
    ...
    if (linked) {
      ...
      if (onPath) {
        console.log(`  ${dim}↪ linked oto-sdk → ${linked}${reset}`);
      }
    }
  }

  if (onPath) {
    console.log(`  ${green}✓${reset} OTO SDK ready (sdk/dist/cli.js)`);
  } else {
    console.log('');
    console.log(`  ${yellow}⚠${reset} OTO SDK files are present but ${bold}oto-sdk${reset} is not on your PATH.`);
    console.log(`    Workflows that call ${cyan}oto-sdk query …${reset} will fail with "command not found".`);
    console.log(`    Install globally to materialize the bin symlink:`);
    console.log(`      ${cyan}npm install -g oto-cc${reset}`);
...
/**
 * #2775 helper: check whether a callable `oto-sdk` exists on the current PATH.
 *
 * Pure PATH walk (no spawn) — we look for a regular file or symlink named
 * `oto-sdk` (or `oto-sdk.cmd`/`.exe` on Windows) in any directory on PATH and
...
/**
 * #2775 helper: attempt to materialize the `oto-sdk` shim at a user-writable
  ...
  for (const dir of candidates) {
    try {
      ...
      try {
        ...
        // cannot copyFileSync(shimSrc, target) — bin/oto-sdk.js resolves the
...
/**
 * Install OTO for all selected runtimes
 */
function installAllRuntimes(runtimes, isGlobal, isInteractive) {
  ...
  const finalize = (shouldInstallStatusline) => {
    ...
    // prebuilt in the tarball (fix/2441-sdk-decouple); oto-sdk reaches users via
    // the parent package's bin/oto-sdk.js shim, so no sub-install is needed.
...
if (process.env.OTO_TEST_MODE) {
  module.exports = {
    ...
    OTO_COPILOT_INSTRUCTIONS_MARKER,
    OTO_COPILOT_INSTRUCTIONS_CLOSE_MARKER,
  ...
  // Main logic
  if (hasSkillsRoot) {
    // Print the skills root directory for a given runtime (used by /oto-sync-skills).
...
} // end of else block for OTO_TEST_MODE
