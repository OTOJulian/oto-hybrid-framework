...
function cmdCommit(cwd, message, files, raw, amend, noVerify) {
  ...
  // Check if .oto is gitignored
  if (isGitIgnored(cwd, '.oto')) {
  ...
  const filesToStage = explicitFiles ? files : ['.oto/'];
  for (const file of filesToStage) {
    ...
    if (!fs.existsSync(fullPath)) {
      ...
      // Default mode (staging all of .oto/): stage the deletion so
...
function cmdCommitToSubrepo(cwd, message, files, raw) {
  ...
  if (!subRepos || subRepos.length === 0) {
    error('no sub_repos configured in .oto/config.json');
  }
...
function cmdScaffold(cwd, type, options, raw) {
  ...
  switch (type) {
    case 'context': {
      ...
      content = `---\nphase: "${padded}"\nname: "${name || phaseInfo?.phase_name || 'Unnamed'}"\ncreated: ${today}\n---\n\n# Phase ${phase}: ${name || phaseInfo?.phase_name || 'Unnamed'} — Context\n\n## Decisions\n\n_Decisions will be captured during /oto-discuss-phase ${phase}_\n\n## Discretion Areas\n\n_Areas where the executor can use judgment_\n\n## Deferred Ideas\n\n_Ideas to consider later_\n`;
      break;
    }
    case 'uat': {
 ...
 * When commit_docs is false, rejects commits that stage .oto/ files.
...
function cmdCheckCommit(cwd, raw) {
  ...
  // commit_docs is false — check if any .oto/ files are staged
  try {
    const staged = execSync('git diff --cached --name-only', { cwd, encoding: 'utf-8' }).trim();
    const planningFiles = staged.split('\n').filter(f => f.startsWith('.oto/') || f.startsWith('.planning\\'));

    if (planningFiles.length > 0) {
      error(
        `commit_docs is false but ${planningFiles.length} .oto/ file(s) are staged:\n` +
