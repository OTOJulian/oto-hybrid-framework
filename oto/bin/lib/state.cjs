...
// hasn't changed within the same oto-tools invocation.
...
function stateReplaceFieldWithFallback(content, primary, fallback, value) {
  ...
  process.stderr.write(
    `[oto-tools] WARNING: STATE.md field "${primary}"${fallback ? ` (fallback: "${fallback}")` : ''} not found — update skipped. ` +
...
function buildStateFrontmatter(bodyContent, cwd) {
    ...
    try {
      ...
      if (fs.existsSync(phasesDir)) {
        ...
        if (!cached) {
          ...
          // .oto/phases/ alongside new dirs with the same phase number,
  ...
  const fm = { oto_state_version: '1.0' };
...
/**
 * Write a WAITING.json signal file when OTO hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .oto/WAITING.json (or .oto/WAITING.json if .oto exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const otoDir = fs.existsSync(path.join(cwd, '.oto')) ? path.join(cwd, '.oto') : planningDir(cwd);
  const waitingPath = path.join(otoDir, 'WAITING.json');
...
function cmdSignalResume(cwd, raw) {
  const paths = [
    path.join(cwd, '.oto', 'WAITING.json'),
...
function cmdStateMilestoneSwitch(cwd, version, name, raw) {
  ...
  try {
    ...
    const fm = {
      oto_state_version: existingFm.oto_state_version || '1.0',
      milestone: version,
 ...
 * Implements the `oto state complete-phase` subcommand (issue #2735).
 */
