...
/**
 * oto2-import — Reverse migration from OTO-2 (.oto/) to OTO v1 (.planning/)
 *
 * Reads a OTO-2 project directory structure and produces a complete
 * .oto/ artifact tree in OTO v1 format.
 *
 * OTO-2 hierarchy:  Milestone → Slice → Task
 * OTO v1 hierarchy: Milestone (in ROADMAP.md) → Phase → Plan
 *
 * Mapping rules:
...
/**
 * Find the .oto/ directory starting from a project root.
 * Returns the absolute path or null if not found.
 */
function findGsd2Root(startPath) {
  if (path.basename(startPath) === '.oto' && fs.existsSync(startPath)) {
    return startPath;
  }
  const candidate = path.join(startPath, '.oto');
...
/**
 * Parse the ## Slices section from a OTO-2 milestone ROADMAP.md.
...
/**
 * Parse the milestone title from the first heading in a OTO-2 ROADMAP.md.
 * Format: # M001: Title
 */
...
/**
 * Parse a task title from a OTO-2 T##-PLAN.md.
 * Format: # T01: Title
 */
...
/**
 * Parse the ## Description body from a OTO-2 task plan.
 */
...
/**
 * Parse ## Must-Haves items from a OTO-2 task plan.
 */
...
/**
 * Read all task plan files from a OTO-2 tasks/ directory.
 */
...
/**
 * Parse a complete OTO-2 .oto/ directory into a structured representation.
 */
function parseGsd2(otoDir) {
  const data = {
    projectContent: readOptional(path.join(otoDir, 'PROJECT.md')),
    requirements: readOptional(path.join(otoDir, 'REQUIREMENTS.md')),
    milestones: [],
  };

  const milestonesBase = path.join(otoDir, 'milestones');
...
/**
 * Build a OTO v1 PLAN.md from a OTO-2 task.
 */
...
/**
 * Build a OTO v1 SUMMARY.md from a OTO-2 task summary.
 * Strips the OTO-2 frontmatter and preserves the body.
 */
function buildSummaryMd(task, phasePrefix, planPrefix) {
  const raw = task.summary || '';
  // Strip OTO-2 frontmatter block (--- ... ---) if present
  ...
  return [
    ...
    '---',
    '',
    body || 'Task completed (migrated from OTO-2).',
    '',
  ].join('\n');
}

/**
 * Build a OTO v1 XX-CONTEXT.md from a OTO-2 slice.
 */
function buildContextMd(slice, phasePrefix) {
  const lines = [
    `# Phase ${phasePrefix} Context`,
    '',
    `Migrated from OTO-2 slice ${slice.id}: ${slice.title}`,
  ];
...
/**
 * Build the OTO v1 ROADMAP.md with milestone-sectioned format.
 */
...
/**
 * Build the OTO v1 STATE.md reflecting the current position in the project.
 */
function buildStateMd(phaseMap) {
  ...
  return [
    ...
    `Status: ${status}`,
    `Last activity: ${today} — Migrated from OTO-2`,
    '',
    `Progress: ${bar} ${pct}%`,
    '',
    '## Accumulated Context',
    ...
    '### Decisions',
    '',
    'Migrated from OTO-2. Review PROJECT.md for key decisions.',
    '',
    '### Blockers/Concerns',
    '',
    'None.',
    ...
    '',
    `Last session: ${today}`,
    'Stopped at: Migration from OTO-2 completed',
    'Resume file: None',
    '',
  ].join('\n');
}
...
/**
 * Convert parsed OTO-2 data into a map of relative path → file content.
 * All paths are relative to the .oto/ root.
 */
function buildPlanningArtifacts(oto2Data) {
  const artifacts = new Map();

  // Passthrough files
  artifacts.set('PROJECT.md', oto2Data.projectContent || '# Project\n\n(Migrated from OTO-2)\n');
  if (oto2Data.requirements) {
    artifacts.set('REQUIREMENTS.md', oto2Data.requirements);
  }
  ...
  const phaseMap = [];
  let phaseNum = 1;
  for (const milestone of oto2Data.milestones) {
  ...
  artifacts.set('ROADMAP.md', buildRoadmapMd(oto2Data.milestones, phaseMap));
 ...
 */
function buildPreview(oto2Data, artifacts) {
  const lines = ['Preview — files that will be created in .oto/:'];

  for (const rel of artifacts.keys()) {
    lines.push(`  ${rel}`);
  }

  const totalSlices = oto2Data.milestones.reduce((s, m) => s + m.slices.length, 0);
  const doneSlices = oto2Data.milestones.reduce((s, m) => s + m.slices.filter(sl => sl.done).length, 0);
  const allTasks = oto2Data.milestones.flatMap(m => m.slices.flatMap(sl => sl.tasks));
  const doneTasks = allTasks.filter(t => t.done).length;

  lines.push('');
  lines.push(`Milestones: ${oto2Data.milestones.length}`);
  ...
  lines.push('  - OTO-2 cost/token ledger (no v1 equivalent)');
  lines.push('  - OTO-2 database state (rebuilt from files on first /oto-health)');
...
/**
 * Write all artifacts to the .oto/ directory.
 */
...
/**
 * Entry point called from oto-tools.cjs.
...
function cmdFromGsd2(args, cwd, raw) {
  ...
  const otoDir = findGsd2Root(projectDir);
  if (!otoDir) {
    return output({ success: false, error: `No .oto/ directory found in ${projectDir}` }, raw);
  }

  const planningRoot = path.join(path.dirname(otoDir), '.oto');
  ...
  const oto2Data = parseGsd2(otoDir);
  const artifacts = buildPlanningArtifacts(oto2Data);
  const preview = buildPreview(oto2Data, artifacts);
  ...
  return output({
    ...
    milestones: oto2Data.milestones.length,
    preview,
  }, raw);
}
