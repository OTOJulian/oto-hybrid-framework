...
// Track all .oto/.lock files held by this process so they can be removed
...
function findProjectRoot(startDir) {
  ...
  // as well as the common case where .git lives at the same level as .oto/.
...
/**
 * Remove stale oto-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param {string} prefix - filename prefix to match (e.g., 'oto-')
 * @param {object} opts
 ...
 */
/**
 * Dedicated OTO temp directory: path.join(os.tmpdir(), 'oto').
 * Created on first use. Keeps OTO temp files isolated from the system
 * temp directory so reap scans only OTO files (#1975).
 */
const OTO_TEMP_DIR = path.join(require('os').tmpdir(), 'oto');

function ensureGsdTempDir() {
  fs.mkdirSync(OTO_TEMP_DIR, { recursive: true });
}

function reapStaleTempFiles(prefix = 'oto-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
  try {
    ensureGsdTempDir();
    const now = Date.now();
    const entries = fs.readdirSync(OTO_TEMP_DIR);
    for (const entry of entries) {
      if (!entry.startsWith(prefix)) continue;
      const fullPath = path.join(OTO_TEMP_DIR, entry);
      try {
...
function output(result, raw, rawValue) {
  ...
  if (raw && rawValue !== undefined) {
    ...
    if (json.length > 50000) {
      reapStaleTempFiles();
      ensureGsdTempDir();
      const tmpPath = path.join(OTO_TEMP_DIR, `oto-${Date.now()}.json`);
...
const CONFIG_DEFAULTS = {
  ...
  phase_branch_template: 'oto/phase-{phase}-{slug}',
  milestone_branch_template: 'oto/{milestone}-{slug}',
  ...
  security_enforcement: true, // workflow.security_enforcement — threat-model-anchored security verification via /oto-secure-phase
  ...
  try {
    ...
    if (unknownKeys.length > 0) {
      process.stderr.write(
        `oto-tools: warning: unknown config key(s) in .oto/config.json: ${unknownKeys.join(', ')} — these will be ignored\n`
      );
    }
    ...
    _warnUnknownProfileOverrides(parsed, '.oto/config.json');
    ...
    return {
      ...
      commit_docs: (() => {
        ...
        // Auto-detection: when no explicit value and .oto/ is gitignored,
        // default to false instead of true
        if (isGitIgnored(cwd, '.oto/')) return false;
    ...
    // Fall back to ~/.oto/defaults.json only for truly pre-project contexts (#1683)
    // If .oto/ exists, the project is initialized — just missing config.json.
    // When OTO_WORKSTREAM is set and root config was loaded, the workstream config
    ...
    if (fs.existsSync(planningDir(cwd))) {
      if (rootParsed) {
        ...
        // Temporarily clear OTO_WORKSTREAM so planningDir() returns root .oto/,
        // then reload. This is safe: rootParsed is already the root config object.
        const savedWs = process.env.OTO_WORKSTREAM;
        delete process.env.OTO_WORKSTREAM;
        try {
          return loadConfig(cwd);
        } finally {
          process.env.OTO_WORKSTREAM = savedWs;
        }
      }
      return defaults;
    }
    try {
      const home = process.env.OTO_HOME || os.homedir();
      const globalDefaultsPath = path.join(home, '.oto', 'defaults.json');
...
function isGitIgnored(cwd, targetPath) {
  ...
  try {
    ...
    // .gitignore explicitly lists them — a common source of confusion when .oto/
 ...
 * Applied at write points so OTO-generated .oto/ files are IDE-friendly.
 *
 * Rules enforced:
 ...
 * In a linked worktree, .oto/ lives in the main worktree, not in the linked one.
...
/**
 * Acquire a file-based lock for .oto/ writes.
...
/**
 * Get the .oto directory path, project- and workstream-aware.
 *
 * Resolution order:
 * 1. If OTO_PROJECT is set (env var or explicit `project` arg), routes to
 ...
 * 2. If OTO_WORKSTREAM is set, routes to `.planning/workstreams/{ws}/`.
 * 3. Otherwise returns `.planning/`.
 *
 * OTO_PROJECT and OTO_WORKSTREAM can be combined:
 *   `.planning/{project}/workstreams/{ws}/`
 *
 * @param {string} cwd - project root
 * @param {string} [ws] - explicit workstream name; if omitted, checks OTO_WORKSTREAM env var
 * @param {string} [project] - explicit project name; if omitted, checks OTO_PROJECT env var
 */
function planningDir(cwd, ws, project) {
  if (project === undefined) project = process.env.OTO_PROJECT || null;
  if (ws === undefined) ws = process.env.OTO_WORKSTREAM || null;

  // Reject path separators and traversal components in project/workstream names
  const BAD_SEGMENT = /[/\\]|\.\./;
  if (project && BAD_SEGMENT.test(project)) {
    throw new Error(`OTO_PROJECT contains invalid path characters: ${project}`);
  }
...
/**
 * Get common .oto file paths, project-and-workstream-aware.
 *
 * All paths route through planningDir(cwd, ws), which honors the OTO_PROJECT
 ...
 * and config were resolved against the unrouted .oto/ root, which broke
 * `oto-tools config-get` in multi-project layouts (the CRUD writers and the
 ...
 *
 * Order:
 * 1. Explicit runtime/session env vars (`OTO_SESSION_KEY`, `CODEX_THREAD_ID`, etc.)
...
function getSessionScopedWorkstreamFile(cwd) {
  ...
  const dirPath = path.join(os.tmpdir(), 'oto-workstream-sessions', projectId);
  return {
    sessionKey,
    dirPath,
...
function findPhaseInternal(cwd, phase) {
  ...
  const milestonesDir = path.join(cwd, '.oto', 'milestones');
  ...
  try {
    ...
    for (const archiveName of archiveDirs) {
      ...
      const relBase = '.oto/milestones/' + archiveName;
    ...
    for (const archiveName of phaseDirs) {
      ...
      for (const dir of dirs) {
        results.push({
          name: dir,
          milestone: version,
          basePath: path.join('.oto', 'milestones', archiveName),
...
/**
 * Resolve the agents directory from the OTO install location.
 * oto-tools.cjs lives at <configDir>/oto/bin/oto-tools.cjs,
 * so agents/ is at <configDir>/agents/.
 *
 * OTO_AGENTS_DIR env var overrides the default path. Used in tests and for
 * installs where the agents directory is not co-located with oto-tools.cjs.
 *
...
/**
 * Check which OTO agents are installed on disk.
 * Returns an object with installation status and details.
 *
 * Recognises both standard format (oto-planner.md) and Copilot format
 * (oto-planner.agent.md). Copilot renames agent files during install (#1512).
 *
...
function _warnUnknownProfileOverrides(parsed, configLabel) {
  ...
  if (runtime && typeof runtime === 'string' && !KNOWN_RUNTIMES.has(runtime)) {
    ...
    if (!_warnedConfigKeys.has(key)) {
      ...
      try {
        process.stderr.write(
          `oto: warning — config key "runtime" has unknown value "${runtime}". ` +
  ...
  for (const [overrideRuntime, tierMap] of Object.entries(overrides)) {
    if (!KNOWN_RUNTIMES.has(overrideRuntime)) {
      ...
      if (!_warnedConfigKeys.has(key)) {
        ...
        try {
          process.stderr.write(
            `oto: warning — model_profile_overrides.${overrideRuntime}.* uses ` +
    ...
    for (const tierName of Object.keys(tierMap)) {
      if (!RUNTIME_OVERRIDE_TIERS.has(tierName)) {
        ...
        if (!_warnedConfigKeys.has(key)) {
          ...
          try {
            process.stderr.write(
              `oto: warning — model_profile_overrides.${overrideRuntime}.${tierName} ` +
