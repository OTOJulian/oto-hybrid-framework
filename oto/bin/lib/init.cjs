...
function cmdInitExecutePhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'oto-executor'),
    verifier_model: resolveModelInternal(cwd, 'oto-verifier'),
...
function cmdInitPlanPhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'oto-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'oto-planner'),
    checker_model: resolveModelInternal(cwd, 'oto-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  try {
    ...
    const skipDirs = new Set(['node_modules', '.git', '.oto', '.claude', '.codex', '__pycache__', 'target', 'dist', 'build']);
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'oto-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'oto-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'oto-roadmapper'),
    ...
    has_codebase_map: pathExistsInternal(cwd, '.oto/codebase'),
    planning_exists: pathExistsInternal(cwd, '.oto'),

    // Brownfield detection
    has_existing_code: hasCode,
    has_package_file: hasPackageFile,
    is_brownfield: hasCode || hasPackageFile,
    needs_codebase_map: (hasCode || hasPackageFile) && !pathExistsInternal(cwd, '.oto/codebase'),
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'oto-codebase-mapper'),
    ...
    // File existence
    planning_exists: pathExistsInternal(cwd, '.oto'),
    codebase_dir_exists: pathExistsInternal(cwd, '.oto/codebase'),
  };
...
function cmdInitManager(cwd, raw) {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error('No ROADMAP.md found. Run /oto-new-milestone first.');
  }
  if (!fs.existsSync(paths.state)) {
    error('No STATE.md found. Run /oto-new-milestone first.');
  }
  ...
  try {
    const waitingPath = path.join(cwd, '.oto', 'WAITING.json');
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: `/oto-execute-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/oto-plan-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/oto-discuss-phase ${phase.number}`,
      });
    }
  }
  ...
  const sanitizeFlags = (raw) => {
    ...
    if (!safe) {
      process.stderr.write(`oto-tools: warning: manager.flags contains invalid tokens, ignoring: ${val}\n`);
      return '';
    }
    return val;
  };
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'oto-workspaces');
...
function cmdInitListWorkspaces(cwd, raw) {
  ...
  if (fs.existsSync(defaultBase)) {
    ...
    for (const entry of entries) {
      ...
      hasProject = fs.existsSync(path.join(wsPath, '.oto', 'PROJECT.md'));
 ...
 * @param {string} agentType - The agent type (e.g., 'oto-executor', 'oto-planner')
 ...
 * Used by workflows: SKILLS=$(node "$TOOLS" agent-skills oto-executor 2>/dev/null)
 */
 ...
 *   installation: { oto_skills_installed: boolean, legacy_claude_commands_installed: boolean },
...
function buildSkillManifest(cwd, skillsDir = null) {
  ...
  const canonicalRoots = skillsDir ? [{
    ...
    {
      root: '.claude/oto/skills',
      path: path.join(os.homedir(), '.claude', 'oto', 'skills'),
      scope: 'import-only',
      kind: 'skills',
      deprecated: true,
    },
    {
      root: '.claude/commands/oto',
      path: path.join(os.homedir(), '.claude', 'commands', 'oto'),
  ...
  const otoSkillsInstalled = skills.some(skill => skill.name.startsWith('oto-'));

  return {
    skills,
    roots,
    installation: {
      oto_skills_installed: otoSkillsInstalled,
 ...
 *   --write              Also write to .oto/skill-manifest.json
 */
function cmdSkillManifest(cwd, args, raw) {
  ...
  // Optionally write to .oto/skill-manifest.json
