 ...
 * with OTO marker detection, doc tooling detection, monorepo awareness, and
...
const OTO_MARKER = '<!-- generated-by: oto-doc-writer -->';

const SKIP_DIRS = new Set([
  'node_modules', '.git', '.oto', '.claude', '__pycache__',
...
/**
 * Check whether a file begins with the OTO doc writer marker.
...
function hasGsdMarker(filePath) {
  try {
    ...
    fs.closeSync(fd);
    return buf.slice(0, bytesRead).toString('utf-8').includes(OTO_MARKER);
  } catch {
    return false;
  }
}
 ...
 * @returns {Array<{path: string, has_oto_marker: boolean}>}
 */
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          results.push({ path: rel, has_oto_marker: hasGsdMarker(abs) });
        }
      }
 ...
 *
 * @example
 * node oto-tools.cjs docs-init --raw
 *
...
function cmdDocsInit(cwd, raw) {
  ...
  const result = {
    doc_writer_model: resolveModelInternal(cwd, 'oto-doc-writer'),
