/**
 * Canonical OTO artifact registry.
 *
 * Enumerates the file names that oto workflows officially produce at the
 * .oto/ root level. Used by oto-health (W019) to flag unrecognized files
 * so stale or misnamed artifacts don't silently mislead agents or reviewers.
 *
 * Add entries here whenever a new workflow produces a .oto/ root file.
 */

'use strict';

// Exact-match canonical file names at .oto/ root
...
const CANONICAL_PATTERNS = [
  /^v\d+\.\d+(?:\.\d+)?-MILESTONE-AUDIT\.md$/i,  // oto-complete-milestone (pre-archive)
  /^v\d+\.\d+(?:\.\d+)?-.*\.md$/i,               // other version-stamped planning docs
];

/**
 * Return true if `filename` (basename only, no path) matches a canonical
 * .oto/ root artifact — either an exact name or a known pattern.
 *
