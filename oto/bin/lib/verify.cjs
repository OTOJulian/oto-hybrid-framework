...
function cmdValidateHealth(cwd, options, raw) {
  ...
  if (resolved === os.homedir()) {
    output({
      status: 'error',
      errors: [{ code: 'E010', message: `CWD is home directory (${resolved}) — health check would read the wrong .oto/ directory. Run from your project root instead.`, fix: 'cd into your project directory and retry' }],
      warnings: [],
  ...
  if (!fs.existsSync(planBase)) {
    addIssue('error', 'E001', '.oto/ directory not found', 'Run /oto-new-project to initialize');
    output({
      status: 'broken',
      errors,
      warnings,
  ...
  if (!fs.existsSync(projectPath)) {
    addIssue('error', 'E002', 'PROJECT.md not found', 'Run /oto-new-project to create');
  } else {
  ...
  if (!fs.existsSync(roadmapPath)) {
    addIssue('error', 'E003', 'ROADMAP.md not found', 'Run /oto-new-milestone to create roadmap');
  }

  // ─── Check 4: STATE.md exists and references valid phases ─────────────────
  if (!fs.existsSync(statePath)) {
    addIssue('error', 'E004', 'STATE.md not found', 'Run /oto-health --repair to regenerate', true);
      ...
      if (!normalizedValid.has(ref) && !normalizedValid.has(padded)) {
        ...
        if (normalizedValid.size > 0) {
          addIssue(
            ...
            'Review STATE.md manually before changing it; /oto-health --repair will not overwrite an existing STATE.md for phase mismatches'
          );
        }
      }
    }
  ...
  if (!fs.existsSync(configPath)) {
    addIssue('warning', 'W003', 'config.json not found', 'Run /oto-health --repair to create with defaults', true);
    ...
    try {
      ...
      addIssue('error', 'E005', `config.json: JSON parse error - ${err.message}`, 'Run /oto-health --repair to reset to defaults', true);
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      if (configParsed.workflow && configParsed.workflow.nyquist_validation === undefined) {
        addIssue('warning', 'W008', 'config.json: workflow.nyquist_validation absent (defaults to enabled but agents may skip)', 'Run /oto-health --repair to add key', true);
        if (!repairs.includes('addNyquistKey')) repairs.push('addNyquistKey');
      }
      if (configParsed.workflow && configParsed.workflow.ai_integration_phase === undefined) {
        addIssue('warning', 'W016', 'config.json: workflow.ai_integration_phase absent (defaults to enabled — run /oto-ai-integration-phase before planning AI system phases)', 'Run /oto-health --repair to add key', true);
    ...
    if (hasResearch && !hasValidation) {
      ...
      try {
        ...
        if (researchContent.includes('## Validation Architecture')) {
          addIssue('warning', 'W009', `Phase ${e.name}: has Validation Architecture in RESEARCH.md but no VALIDATION.md`, 'Re-run /oto-plan-phase with --research to regenerate');
        }
      } catch { /* intentionally empty */ }
    }
  }
  ...
  // Verify OTO agents are installed. Missing agents cause Task(subagent_type=...)
    ...
    if (!agentStatus.agents_installed) {
      if (agentStatus.installed_agents.length === 0) {
        addIssue('warning', 'W010',
          `No OTO agents found in ${agentStatus.agents_dir} — Task(subagent_type="oto-*") will fall back to general-purpose`,
          'Run the OTO installer: npx oto-cc@latest');
      } else {
        addIssue('warning', 'W010',
          `Missing ${agentStatus.missing_agents.length} OTO agents: ${agentStatus.missing_agents.join(', ')} — affected workflows will fall back to general-purpose`,
          'Run the OTO installer: npx oto-cc@latest');
      }
    }
        ...
        if (phaseCheckboxRe.test(roadmapContentFull)) {
          ...
          if (statusVal !== 'complete' && statusVal !== 'done') {
            addIssue('warning', 'W011',
              ...
              'Run /oto-progress to re-derive current position, or manually update STATE.md');
          }
        }
      }
      ...
      if (archivedVersions.length > 0) {
        ...
        if (missingFromRegistry.length > 0) {
          addIssue('warning', 'W018',
            ...
            'Run /oto-health --backfill to synthesize missing entries from archive snapshots',
            true);
  ...
  // ─── Check 13: Unrecognized .oto/ root files (W019) ──────────────────
  try {
    ...
    for (const entry of entries) {
      ...
      if (!isCanonicalPlanningFile(entry.name)) {
        addIssue('warning', 'W019',
          `Unrecognized .oto/ file: ${entry.name} — not a canonical OTO artifact`,
          'Move to .oto/milestones/ archive subdir or delete if stale. See templates/README.md for the canonical artifact list.',
          false);
      }
    }
      ...
      try {
        switch (repair) {
          ...
          case 'regenerateState': {
            ...
            stateContent += `- ${new Date().toISOString().split('T')[0]}: STATE.md regenerated by /oto-health --repair\n`;
          ...
          case 'backfillMilestones': {
            ...
            for (const ver of missingFromRegistry) {
              try {
                ...
                const entry = `## ${ver}${milestoneName && milestoneName !== ver ? ` ${milestoneName}` : ''} (Backfilled: ${today})\n\n**Note:** Synthesized from archive snapshot by \`/oto-health --backfill\`. Original completion date unknown.\n\n---\n\n`;
