 ...
 *   2. User-level defaults from ~/.oto/defaults.json (if present)
 *   3. userChoices — the settings the user explicitly selected during /oto-new-project
 *
...
/**
 * Command: create a fully-materialized .oto/config.json for a new project.
 *
 * Accepts user-chosen settings as a JSON string (the keys the user explicitly
 * configured during /oto-new-project). All remaining keys are filled from
 * hardcoded defaults and optional ~/.oto/defaults.json.
 *
...
function cmdConfigNewProject(cwd, choicesJson, raw) {
  ...
  try {
    ...
    output({ created: true, path: '.oto/config.json' }, raw, 'created');
  } catch (err) {
...
function ensureConfigFile(cwd) {
  ...
  try {
    atomicWriteFileSync(configPath, JSON.stringify(config, null, 2), 'utf-8');
    return { created: true, path: '.oto/config.json' };
  } catch (err) {
