...
function cmdPhaseAdd(cwd, description, raw, customId) {
  ...
  const { newPhaseId, dirName } = withPlanningLock(cwd, () => {
    ...
    if (customId || config.phase_naming === 'custom') {
      ...
      // 2. .oto/phases/ on disk (orphan directories not tracked in roadmap)
      ...
      // Also scan .oto/phases/ for orphan directories not tracked in ROADMAP.
    ...
    const phaseEntry = `\n### Phase ${_newPhaseId}: ${description}\n\n**Goal:** [To be planned]\n**Requirements**: TBD${dependsOn}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /oto-plan-phase ${_newPhaseId} to break down)\n`;
...
function cmdPhaseAddBatch(cwd, descriptions, raw) {
  ...
  const results = withPlanningLock(cwd, () => {
    ...
    for (const description of descriptions) {
      ...
      const phaseEntry = `\n### Phase ${newPhaseId}: ${description}\n\n**Goal:** [To be planned]\n**Requirements**: TBD${dependsOn}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /oto-plan-phase ${newPhaseId} to break down)\n`;
      const lastSeparator = rawContent.lastIndexOf('\n---');
...
function cmdPhaseInsert(cwd, afterPhase, description, raw) {
  ...
  const { decimalPhase, dirName } = withPlanningLock(cwd, () => {
    ...
    // Build phase entry
    const phaseEntry = `\n### Phase ${_decimalPhase}: ${description} (INSERTED)\n\n**Goal:** [Urgent work - to be planned]\n**Requirements**: TBD\n**Depends on:** Phase ${afterPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /oto-plan-phase ${_decimalPhase} to break down)\n`;
...
function cmdPhaseComplete(cwd, phaseNum, raw) {
  ...
  if (fs.existsSync(roadmapPath)) {
    withPlanningLock(cwd, () => {
      ...
      // block (the standard /oto-new-project layout). replaceInCurrentMilestone
