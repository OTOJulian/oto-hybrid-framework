/**
 * lib/intel.cjs -- Intel storage and query operations for OTO.
 *
 * Provides a persistent, queryable intelligence system for project metadata.
 * Intel files live in .oto/intel/ and store structured data about
 ...
 * @returns {string} Full path to .oto/intel/
 */
...
function intelUpdate(planningDir) {
  ...
  return {
    action: 'spawn_agent',
    message: 'Run oto-tools intel update or spawn oto-intel-updater agent for full refresh'
  };
}
