 ...
 * REQUIREMENTS.md, and phases/ into .oto/workstreams/{name}/ directories.
 *
 * When no workstreams/ directory exists, OTO operates in "flat mode" with
 * everything at .oto/ — backward compatible with pre-workstream installs.
 */
...
/**
 * Migrate flat .oto/ layout to workstream mode.
 * Moves per-workstream files (ROADMAP.md, STATE.md, REQUIREMENTS.md, phases/)
 * into .oto/workstreams/{name}/. Shared files (PROJECT.md, config.json,
...
function migrateToWorkstreams(cwd, workstreamName) {
  ...
  if (fs.existsSync(path.join(baseDir, 'workstreams'))) {
    throw new Error('Already in workstream mode — .oto/workstreams/ exists');
  }
...
function cmdWorkstreamCreate(cwd, name, options, raw) {
  ...
  if (!fs.existsSync(baseDir)) {
    error('.oto/ directory not found — run /oto-new-project first');
  }
