...
function cmdMilestoneComplete(cwd, version, options, raw) {
  ...
  const milestonesPath = path.join(cwd, '.oto', 'MILESTONES.md');
  const archiveDir = path.join(cwd, '.oto', 'milestones');
  ...
  const auditFile = path.join(cwd, '.oto', `${version}-MILESTONE-AUDIT.md`);
