 ...
 *
 * Background: every installed `oto-*` skill costs eager system-prompt
 ...
 *
 * The `minimal` profile installs the main OTO loop only:
...
/**
 * Stage a filtered copy of the source commands/oto directory when in
 ...
 *
 * @param {string} srcDir absolute path to commands/oto
...
function stageSkillsForMode(srcDir, mode) {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'oto-minimal-skills-'));
  try {
