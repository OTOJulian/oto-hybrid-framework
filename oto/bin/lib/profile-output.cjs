...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /oto-new-project to set up.',
...
function extractSectionContent(fileContent, sectionName) {
  const startMarker = `<!-- OTO:${sectionName}-start`;
  const endMarker = `<!-- OTO:${sectionName}-end -->`;
...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- OTO:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- OTO:${sectionName}-end -->`,
  ].join('\n');
}
...
function generateProjectSection(cwd) {
  const projectPath = path.join(cwd, '.oto', 'PROJECT.md');
  ...
  return { content: parts.join('\n\n'), source: 'PROJECT.md', linkPath: '.oto/PROJECT.md', hasFallback: false };
}

function generateStackSection(cwd) {
  const codebasePath = path.join(cwd, '.oto', 'codebase', 'STACK.md');
  const researchPath = path.join(cwd, '.oto', 'research', 'STACK.md');
  let content = safeReadFile(codebasePath);
  let source = 'codebase/STACK.md';
  let linkPath = '.oto/codebase/STACK.md';
  if (!content) {
    content = safeReadFile(researchPath);
    source = 'research/STACK.md';
    linkPath = '.oto/research/STACK.md';
  }
  if (!content) {
...
function generateConventionsSection(cwd) {
  const conventionsPath = path.join(cwd, '.oto', 'codebase', 'CONVENTIONS.md');
  ...
  const summary = summaryLines.length > 0 ? summaryLines.join('\n') : content.trim();
  return { content: summary, source: 'CONVENTIONS.md', linkPath: '.oto/codebase/CONVENTIONS.md', hasFallback: false };
}

function generateArchitectureSection(cwd) {
  const architecturePath = path.join(cwd, '.oto', 'codebase', 'ARCHITECTURE.md');
  ...
  const summary = summaryLines.length > 0 ? summaryLines.join('\n') : content.trim();
  return { content: summary, source: 'ARCHITECTURE.md', linkPath: '.oto/codebase/ARCHITECTURE.md', hasFallback: false };
}
...
function generateSkillsSection(cwd) {
  ...
  for (const dir of SKILL_SEARCH_DIRS) {
    ...
    for (const entry of entries) {
      ...
      // Skip OTO's own installed skills — only surface project-specific skills
      if (entry.name.startsWith('oto-')) continue;
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'oto', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/oto-profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', 'oto', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
  ...
  const result = {
    ...
    command_name: '/oto-dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by OTO from ${dataSource}. Run \`/oto-profile-user --refresh\` to update.`,
    '',
  ...
  if (fs.existsSync(targetPath)) {
    ...
    const startMarker = '<!-- OTO:profile-start -->';
    const endMarker = '<!-- OTO:profile-end -->';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  const sectionHeadings = {
    ...
    workflow: '## OTO Workflow Enforcement',
  };
  ...
  if (existingContent === null) {
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- OTO:${name}-start`) !== -1;
    ...
    if (!options.auto && fileContent.indexOf('<!-- OTO:profile-start') === -1) {
  ...
  let profileStatus;
  if (finalContent && finalContent.indexOf('<!-- OTO:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- OTO:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /oto-profile-user to unlock Developer Profile.';
