 ...
 *
 * @param {string} planningDir - Path to .oto directory
 * @returns {boolean}
 */
...
function disabledResponse() {
  return { disabled: true, message: 'graphify is not enabled. Enable with: oto-tools config-set graphify.enabled true' };
}
 ...
 * Reads graph.json from .oto/graphs/ and writes .last-build-snapshot.json
...
function writeSnapshot(cwd) {
  const graphPath = path.join(cwd, '.oto', 'graphs', 'graph.json');
  const graph = safeReadJson(graphPath);
  ...
  const snapshotPath = path.join(cwd, '.oto', 'graphs', '.last-build-snapshot.json');
