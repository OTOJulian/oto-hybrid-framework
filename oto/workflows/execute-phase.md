...
- **Claude Code:** Uses `Task(subagent_type="oto-executor", ...)` — blocks until complete, returns result
...
@~/.claude/oto/references/agent-contracts.md
@~/.claude/oto/references/context-budget.md
@~/.claude/oto/references/gates.md
</required_reading>

<available_agent_types>
These are the valid OTO subagent types registered in .claude/agents/ (or equivalent for your runtime).
Always use the exact name from this list — do not fall back to 'general-purpose' or other built-in types:

- oto-executor — Executes plan tasks, commits, creates SUMMARY.md
- oto-verifier — Verifies phase completion, checks quality gates
...
- oto-debugger — Diagnoses and fixes issues
- oto-codebase-mapper — Maps project structure and dependencies
- oto-integration-checker — Checks cross-phase integration
- oto-nyquist-auditor — Validates verification coverage
- oto-ui-researcher — Researches UI/UX approaches
- oto-ui-checker — Reviews UI implementation quality
- oto-ui-auditor — Audits UI against design requirements
</available_agent_types>
...
```bash
USE_WORKTREES=$(oto-sdk query config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @~/.claude/oto/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @~/.claude/oto/references/planner-antipatterns.md
...
Check if the current runtime is Copilot by testing for the `@oto-executor` agent pattern
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  oto-sdk query config-set workflow._auto_chain_active false || true
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/oto/workflows/execute-plan.md` **inline**
...
- Maintains OTO's planning/tracking structure
- Best for: small phases, bug fixes, verification gaps, learning OTO
...
```bash
oto-sdk query state.begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(oto-sdk query phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
CROSS_AI_ENABLED=$(oto-sdk query config-get workflow.cross_ai_execution 2>/dev/null || echo "false")
CROSS_AI_CMD=$(oto-sdk query config-get workflow.cross_ai_command 2>/dev/null || echo "")
CROSS_AI_TIMEOUT=$(oto-sdk query config-get workflow.cross_ai_timeout 2>/dev/null || echo "300")
```
...
`workflow.cross_ai_command` via `oto-sdk query config-set workflow.cross_ai_command "<command>"`.
...
tooling and `/oto-manager`'s background-completion handler can grep partial
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       For `oto-sdk query commit` (or legacy `oto-tools.cjs` commit): add --no-verify flag when needed.
       ...
       @~/.claude/oto/references/checkpoints.md
       @~/.claude/oto/references/tdd.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@~/.claude/oto/references/executor-examples.md'}
       </execution_context>

       <files_to_read>
       Read these files at execution start using the Read tool:
       - {phase_dir}/{plan_file} (Plan)
       - .oto/PROJECT.md (Project context — core value, requirements, evolution rules)
       - .oto/STATE.md (State)
       - .oto/config.json (Config, if exists)
...
5.5. **Worktree cleanup (when `isolation="worktree"` was used):**
   ...
   while IFS= read -r WT; do
     ...
     if [ -n "$WT_BRANCH" ] && [ "$WT_BRANCH" != "HEAD" ]; then
       ...
       [ -f .oto/STATE.md ] && cp .oto/STATE.md "$STATE_BACKUP" || true
       [ -f .oto/ROADMAP.md ] && cp .oto/ROADMAP.md "$ROADMAP_BACKUP" || true

       # Snapshot list of files on main BEFORE merge to detect resurrections
       PRE_MERGE_FILES=$(git ls-files .oto/)
       ...
       git merge "$WT_BRANCH" --no-ff --no-edit -m "chore: merge executor worktree ($WT_BRANCH)" 2>&1 || {
         ...
         echo "  Restore with: cp \$STATE_BACKUP .oto/STATE.md && cp \$ROADMAP_BACKUP .oto/ROADMAP.md"
         break
       }
       ...
       if [ -s "$STATE_BACKUP" ]; then
         cp "$STATE_BACKUP" .oto/STATE.md
       fi
       if [ -s "$ROADMAP_BACKUP" ]; then
         cp "$ROADMAP_BACKUP" .oto/ROADMAP.md
       fi
       rm -f "$STATE_BACKUP" "$ROADMAP_BACKUP"
       ...
       DELETED_FILES=$(git diff --diff-filter=A --name-only HEAD~1 -- .oto/ 2>/dev/null || true)
       ...
       if ! git diff --quiet .oto/STATE.md .oto/ROADMAP.md 2>/dev/null || \
          [ -n "$DELETED_FILES" ]; then
         # Only amend the commit with .oto/ files if commit_docs is enabled (#1783)
         COMMIT_DOCS=$(oto-sdk query config-get commit_docs 2>/dev/null || echo "true")
         if [ "$COMMIT_DOCS" != "false" ]; then
           git add .oto/STATE.md .oto/ROADMAP.md 2>/dev/null || true
...
5.6. **Post-merge build & test gate:**
   ...
   Read and execute `oto/workflows/execute-phase/steps/post-merge-gate.md`.
   ...
   **Only update tracking when tests passed (TEST_EXIT=0).**
   ...
   if [ "${TEST_EXIT}" -eq 0 ]; then
     ...
     for plan_id in {completed_plan_ids}; do
       oto-sdk query roadmap.update-plan-progress "${PHASE_NUMBER}" "${plan_id}" "complete"
     done

     # Only commit tracking files if they actually changed
     if ! git diff --quiet .oto/ROADMAP.md .oto/STATE.md 2>/dev/null; then
       oto-sdk query commit "docs(phase-${PHASE_NUMBER}): update tracking after wave ${N}" --files .oto/ROADMAP.md .oto/STATE.md
     fi
...
7. **Handle failures:**
   ...
   **Known Claude Code bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Claude Code runtime bug — not a OTO or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 5 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS → treat as **successful**. If spot-checks FAIL → treat as real failure below.
...
7b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    oto-sdk query verify.key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_MODE=$(oto-sdk query check auto-mode --pick active 2>/dev/null || echo "false")
```
...
```bash
TDD_MODE=$(oto-sdk query config-get workflow.tdd_mode 2>/dev/null || echo "false")
```
...
```bash
POST_PLAN_INDEX=$(oto-sdk query phase-plan-index "${PHASE_NUMBER}")
```
...
/oto-execute-phase {phase} ${OTO_WS}                # Continue remaining waves
/oto-execute-phase {phase} --wave {next} ${OTO_WS}  # Run the next wave explicitly
```
...
**Config gate:**
```bash
CODE_REVIEW_ENABLED=$(oto-sdk query config-get workflow.code_review 2>/dev/null || echo "true")
```
...
**Invoke review:**
```
Skill(skill="oto:code-review", args="${PHASE_NUMBER}")
```
...
/oto-code-review-fix ${PHASE_NUMBER}
```
...
```bash
PARENT_INFO=$(oto-sdk query find-phase "${PARENT_PHASE}" --raw)
...
```bash
mkdir -p .oto/debug/resolved
mv .oto/debug/{slug}.md .oto/debug/resolved/
```

**6. Commit updated artifacts:**
```bash
oto-sdk query commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .oto/phases/*${PARENT_PHASE}*/*-UAT.md .oto/debug/resolved/*.md
```
</step>
...
PRIOR_VERIFICATIONS=$(find .oto/phases/ -name "*-VERIFICATION.md" ! -path "*${PHASE_NUMBER}*" 2>/dev/null)
```
...
REG_TEST_CMD=$(oto-sdk query config-get workflow.test_command --default "" 2>/dev/null || true)
...
```bash
SCHEMA_DRIFT=$(oto-sdk query verify.schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
Check for override:
```bash
SKIP_SCHEMA=$(echo "${OTO_SKIP_SCHEMA_CHECK:-false}")
```
...
Display:
```
⚠ Schema drift detected but OTO_SKIP_SCHEMA_CHECK=true — bypassing gate.
...
2. Skip schema check (OTO_SKIP_SCHEMA_CHECK=true) — bypass this gate
...
- .oto/REQUIREMENTS.md (Requirement traceability)
...
Commit the file:
```bash
oto-sdk query commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` — they will appear in `/oto-progress` and `/oto-audit-uat`.

"approved" → continue | Report issues → gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/oto-verify-work` on it.
...
```bash
COMPLETION=$(oto-sdk query phase.complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/oto-progress` and `/oto-audit-uat`.
```

```bash
oto-sdk query commit "docs(phase-{X}): complete phase execution" --files .oto/ROADMAP.md .oto/STATE.md .oto/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
entries from the completed phase to the global learnings store at `~/.oto/knowledge/`.

**Check config gate:**
```bash
GL_ENABLED=$(oto-sdk query config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
oto-sdk query learnings.copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
...
PENDING_DIR=".oto/todos/pending"
COMPLETED_DIR=".oto/todos/completed"
...
if [ ${#CLOSED[@]} -gt 0 ]; then
  oto-sdk query commit "docs(phase-${PHASE_NUMBER}): auto-close ${#CLOSED[@]} todo(s) resolved by this phase" --files .oto/todos/completed/ .oto/STATE.md|| true
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/oto-plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `~/.claude/oto/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If neither `--auto` nor `AUTO_MODE` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/oto-transition` command. Never suggest it. The transition workflow is internal only.**

Check whether CONTEXT.md already exists for the next phase:

```bash
ls .oto/phases/*{next}*/{next}-CONTEXT.md 2>/dev/null || echo "no-context"
```
...
/oto-progress ${OTO_WS} — see updated roadmap
/oto-discuss-phase {next} ${OTO_WS} — start here: discuss next phase before planning  ← recommended
/oto-plan-phase {next} ${OTO_WS} — plan next phase (skip discuss)
/oto-execute-phase {next} ${OTO_WS} — execute next phase (skip discuss and plan)
```
...
/oto-progress ${OTO_WS} — see updated roadmap
/oto-plan-phase {next} ${OTO_WS} — start here: plan next phase (CONTEXT.md already present)  ← recommended
/oto-discuss-phase {next} ${OTO_WS} — re-discuss next phase
/oto-execute-phase {next} ${OTO_WS} — execute next phase (skip planning)
```
...
<failure_handling>
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Claude Code bug, not OTO. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
...
<resumption>
Re-run `/oto-execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
