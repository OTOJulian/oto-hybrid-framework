...
Example: `/oto-insert-phase 72 Fix critical auth bug`
-> after = 72
...
Usage: /oto-insert-phase <after> <description>
Example: /oto-insert-phase 72 Fix critical auth bug
```
...
```bash
INIT=$(oto-sdk query init.phase-op "${after_phase}")
...
**Delegate the phase insertion to `oto-sdk query phase.insert`:**

```bash
RESULT=$(oto-sdk query phase.insert "${after_phase}" "${description}")
```
...
1. Update STATE.md's next-phase pointer(s) to the newly inserted phase
   ...
   ```bash
   oto-sdk query state.patch '{"Current Phase":"{decimal_phase}","Next recommended run":"/oto-plan-phase {decimal_phase}"}'
   ```
...
2. Append a Roadmap Evolution entry via the dedicated handler. It creates the
   ...
   ```bash
   oto-sdk query state.add-roadmap-evolution \
     --phase {decimal_phase} \
     --action inserted \
     --after {after_phase} \
     --note "{description}" \
...
- Directory: .oto/phases/{decimal-phase}-{slug}/
- Status: Not planned yet
- Marker: (INSERTED) - indicates urgent work

Roadmap updated: .oto/ROADMAP.md
Project state updated: .oto/STATE.md
...
- Don't use this for planned work at end of milestone (use /oto-add-phase)
- Don't insert before Phase 1 (decimal 0.1 makes no sense)
- Don't renumber existing phases
- Don't modify the target phase content
- Don't create plans yet (that's /oto-plan-phase)
...
- [ ] `oto-sdk query phase.insert` executed successfully
- [ ] Phase directory created
- [ ] Roadmap updated with new phase entry (includes "(INSERTED)" marker)
- [ ] `oto-sdk query state.add-roadmap-evolution ...` returned `{ added: true }` or `{ added: false, reason: "duplicate" }`
- [ ] `oto-sdk query state.patch` returned matched next-phase pointer field(s)
