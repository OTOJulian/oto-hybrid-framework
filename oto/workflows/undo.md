<purpose>
Safe git revert workflow. Rolls back OTO phase or plan commits using the phase manifest with dependency checks and a confirmation gate. Uses git revert --no-commit (NEVER git reset) to preserve history.
</purpose>
...
```
Usage: /oto-undo --last N | --phase NN | --plan NN-MM

Modes:
  --last N      Show last N OTO commits for interactive selection
...
Filter for OTO conventional commits matching `type(scope): message` pattern (e.g., `feat(04-01):`, `docs(03):`, `fix(02-03):`).
...
Read `.oto/.phase-manifest.json` if it exists.
...
1. Check if `.oto/phases/${N}-*/` directory exists
...
Look for later plans in the same phase directory (`.oto/phases/${NN}-*/`). For each later plan (plans with number > MM):
...
```
Working tree has uncommitted changes. Commit or stash them before running /oto-undo.
```
...
**Also available:**
- `/oto-execute-phase ${PHASE}` — re-execute if needed
- `/oto-undo --last 1` — undo the revert itself if something went wrong
...
- [ ] --phase mode reads .oto/.phase-manifest.json using manifest.phases[TARGET_PHASE].commits
