<purpose>
Interactive configuration of OTO workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .oto/config.json with user preferences. Optionally saves settings as global defaults (~/.oto/defaults.json) for future projects.
</purpose>
...
INIT=$(oto-sdk query state.load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
# `state.load` returns STATE frontmatter JSON from the SDK — it does not include `config_path`. Orchestrators may set `OTO_CONFIG_PATH` from init phase-op JSON; otherwise resolve the same path oto-tools uses for flat vs active workstream (#2282).
...
(Opus/Sonnet/Haiku) to each agent. When `runtime` is set in .oto/config.json,
...
configure `model_overrides` manually in .oto/config.json to target specific
models per agent.
```
...
AskUserQuestion([
  ...
  {
    question: "Enable Code Review? (/oto-code-review and /oto-code-review-fix commands)",
    header: "Code Review",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Enable /oto-code-review commands for reviewing source files changed during a phase." },
  ...
  {
    question: "Code Review Depth? (default depth for /oto-code-review — override per-run with --depth=)",
    header: "Review Depth",
    multiSelect: false,
    options: [
  ...
  {
    question: "Enable UI Review? (visual quality audit via /oto-ui-review in autonomous mode)",
    header: "UI Review",
    multiSelect: false,
    options: [
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /oto-ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /oto-ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Run /oto-ai-integration-phase before planning AI system phases. Surfaces the right framework, researches its docs, and designs the evaluation strategy." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (oto/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (oto/{version}-{name})" }
    ]
  },
  {
  ...
  {
    question: "Commit .oto/ files to git? (controls whether plans/artifacts are tracked in your repo)",
    header: "Commit Docs",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Commit .oto/ to git. Plans, research, and phase artifacts travel with the repo." },
      { label: "No", description: "Do not commit .oto/. Keep planning local only. Automatic when .oto/ is in .gitignore." }
    ]
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /oto-autonomous — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  },
  {
  ...
  {
    question: "Enable Intel? (queryable codebase intelligence via /oto-intel — builds a JSON index in .oto/intel/)",
    header: "Intel",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip intel indexing. Use when codebase is small or intel queries are not needed." },
      { label: "Yes", description: "Enable /oto-intel commands. Builds and queries a JSON index of the codebase." }
    ]
  },
  {
    question: "Enable Graphify? (project knowledge graph via /oto-graphify — builds a graph in .oto/graphs/)",
    header: "Graphify",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip knowledge graph. Use when dependency graphs are not needed." },
      { label: "Yes", description: "Enable /oto-graphify commands. Builds and queries a project knowledge graph." }
    ]
  }
])
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.oto/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.oto/defaults.json`:

```bash
mkdir -p ~/.oto
```

Write `~/.oto/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
These settings apply to future /oto-plan-phase and /oto-execute-phase runs.

Quick commands:
- /oto-settings-integrations — configure API keys (Brave/Firecrawl/Exa), review.models CLI routing, and agent_skills injection
- /oto-set-profile <profile> — switch model profile
- /oto-plan-phase --research — force research
- /oto-plan-phase --skip-research — skip research
- /oto-plan-phase --skip-verify — skip plan check
- /oto-settings-advanced — power-user tuning (plan bounce, timeouts, branch templates, cross-AI, context window)
```
</step>
...
- [ ] User offered to save as global defaults (~/.oto/defaults.json)
