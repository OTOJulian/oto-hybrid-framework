...
[R] Resolve — stop and fix items, then re-run /oto-complete-milestone
[A] Acknowledge all — document as deferred and proceed with close
[C] Cancel — exit without closing
```

If user chooses [A] (Acknowledge):
1. Re-run `oto-sdk query audit-open --json` to get structured data
...
2. **Run audit first** — `/oto-audit-milestone` to assess gap severity
...
for summary in .oto/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  oto-sdk query summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `oto-sdk query milestone.complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to `oto-sdk query milestone.complete`:**

```bash
ARCHIVE=$(oto-sdk query milestone.complete "v[X.Y]" --name "[Milestone Name]")
```
...
Verify: `✅ Milestone archived to .oto/milestones/`
...
```bash
mkdir -p .oto/milestones/v[X.Y]-phases
# For each phase directory in .oto/phases/:
mv .oto/phases/{phase-dir} .oto/milestones/v[X.Y]-phases/
```
...
- Safety commit of archive files + updated ROADMAP.md, then `git rm .oto/REQUIREMENTS.md`
...
BACKLOG_SECTION=$(awk '/^## Backlog/{found=1} found{print}' .oto/ROADMAP.md)
```
...
```bash
oto-sdk query commit "chore: archive v[X.Y] milestone files" --files .oto/milestones/v[X.Y]-ROADMAP.md .oto/milestones/v[X.Y]-REQUIREMENTS.md .oto/milestones/v[X.Y]-MILESTONE-AUDIT.md .oto/MILESTONES.md .oto/PROJECT.md .oto/STATE.md .oto/ROADMAP.md
```
...
```bash
ls .oto/RETROSPECTIVE.md 2>/dev/null || true
```

**If exists:** Read the file, append new milestone section before the "## Cross-Milestone Trends" section.

**If doesn't exist:** Create from template at `~/.claude/oto/templates/retrospective.md`.
...
**Commit:**
```bash
oto-sdk query commit "docs: update retrospective for v${VERSION}" --files .oto/RETROSPECTIVE.md
```
...
Detect base branch:
```bash
BASE_BRANCH=$(oto-sdk query config-get git.base_branch 2>/dev/null || echo "")
...
if [ "$BRANCHING_STRATEGY" = "phase" ]; then
  for branch in $PHASE_BRANCHES; do
    ...
    # Strip .oto/ from staging if commit_docs is false
    if [ "$COMMIT_DOCS" = "false" ]; then
      git reset HEAD .oto/ 2>/dev/null || true
    fi
...
See .oto/MILESTONES.md for full details."
```
...
- [ ] User knows next step (/oto-new-milestone)
