...
```bash
CONFIG=$(oto-sdk query state.load)
```
...
```bash
REVIEW_CMD=$(oto-sdk query config-get workflow.code_review_command 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
```
...
2. **Load phase context from STATE.md:**
   ```bash
   STATE_STATUS=$(oto-sdk query state.load 2>/dev/null | head -20)
   ```
...
3. **Build review prompt and pipe to command via stdin:**
   ...
   REVIEW_OUTPUT=$(echo "${REVIEW_PROMPT}" | timeout 120 ${REVIEW_CMD} 2>/tmp/oto-review-stderr.log)
   REVIEW_EXIT=$?
   ```
...
4. **Handle timeout (120s) and failure:**
   ...
   if [ $REVIEW_EXIT -ne 0 ]; then
     REVIEW_STDERR=$(cat /tmp/oto-review-stderr.log 2>/dev/null)
...
```bash
oto-sdk query state.update "Last Activity" "$(date +%Y-%m-%d)"
oto-sdk query state.update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
oto-sdk query commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .oto/STATE.md
```
</step>
...
- /oto-complete-milestone (if last phase in milestone)
- /oto-progress (to see what's next)
...
- /oto-complete-milestone — if all phases in milestone are done
- /oto-progress — see overall project state
- /oto-execute-phase {next} — continue to next phase
</offer_next>
