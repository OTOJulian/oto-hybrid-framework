...
<required_reading>
@~/.claude/oto/references/domain-probes.md
@~/.claude/oto/references/gate-prompts.md
@~/.claude/oto/references/universal-anti-patterns.md
</required_reading>
...
1. **oto-phase-researcher** — Reads CONTEXT.md to know WHAT to research
2. **oto-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/oto-plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
AGENT_SKILLS_ADVISOR=$(oto-sdk query agent-skills oto-advisor-researcher)
```
...
Use /oto-progress ${OTO_WS} to see available phases.
```
Exit workflow.
...
if [ -f "$HOME/.claude/oto/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Check if a SPEC.md (from `/oto-spec-phase`) exists for this phase. SPEC.md locks requirements before implementation decisions.
...
**Note:** SPEC.md files named `AI-SPEC.md` (from `/oto-ai-integration-phase`) are excluded — different purpose.
</step>
...
cat .oto/REQUIREMENTS.md 2>/dev/null || true
...
RAW_SPIKES=$(ls .oto/spikes/MANIFEST.md 2>/dev/null)
RAW_SKETCHES=$(ls .oto/sketches/MANIFEST.md 2>/dev/null)
```
...
If raw spikes/sketches exist but no findings skill, note: `⚠ Unpackaged spikes/sketches detected — run /oto-spike-wrap-up or /oto-sketch-wrap-up to make findings available.`
...
```bash
TODO_MATCHES=$(oto-sdk query todo.match-phase "${PHASE_NUMBER}")
```
...
Read `@~/.claude/oto/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
1. `ls .oto/codebase/*.md` to find existing maps
...
```
Created: .oto/phases/${PADDED_PHASE}-${SLUG}/${PADDED_PHASE}-CONTEXT.md
...
`/oto-plan-phase ${PHASE} ${OTO_WS}`

---

**Also available:** `--chain` for auto plan+execute after; `/oto-plan-phase ${PHASE} --skip-research ${OTO_WS}` to plan without research; `/oto-ui-phase ${PHASE} ${OTO_WS}` for UI design contracts; review/edit CONTEXT.md before continuing.
```
</step>
...
```bash
oto-sdk query commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
oto-sdk query commit "docs(state): record phase ${PHASE} context session" --files .oto/STATE.md
```
</step>
