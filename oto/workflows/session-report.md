<purpose>
Generate a post-session summary document capturing work performed, outcomes achieved, and estimated resource usage. Writes SESSION_REPORT.md to .oto/reports/ for human review and stakeholder sharing.
</purpose>
...
```bash
ls -la .oto/reports/SESSION_REPORT*.md 2>/dev/null || echo "No previous reports"
```
</step>
...
*Generated by `/oto-session-report`*
```
</step>
...
```
💡 Run `/oto-session-report` at the end of each session to build a history of project activity.
```
</step>
