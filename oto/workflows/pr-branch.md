<purpose>
Create a clean branch for pull requests by filtering out transient .oto/ commits.
The PR branch contains only code changes and structural planning state — reviewers
don't see OTO transient artifacts (PLAN.md, SUMMARY.md, CONTEXT.md, RESEARCH.md, etc.)
...
for HASH in $CODE_AND_STRUCTURAL_COMMITS; do
  ...
  # Remove only transient .oto/ subdirectories that came along in mixed commits.
  ...
  for dir in phases quick research threads todos debug seeds codebase ui-reviews; do
    git rm -r --cached ".oto/$dir/" 2>/dev/null || true
  done
  git commit -C "$HASH"
done
```
...
Or use /oto-ship to create the PR automatically.
```
</step>
...
- [ ] No .oto/ files in PR branch diff
