<purpose>
Interactive configuration of OTO power-user knobs — plan bounce, node repair, subagent timeouts,
...
This is a companion to `/oto-settings` — the common-case prompt there covers model profile,
...
- `git.phase_branch_template` (default: `oto/phase-{phase}-{slug}`)
- `git.milestone_branch_template` (default: `oto/{milestone}-{slug}`)
...
AskUserQuestion([
  ...
  {
    question: "Phase branch template? (current: <value or oto/phase-{phase}-{slug}>)",
  ...
  {
    question: "Milestone branch template? (current: <value or oto/{milestone}-{slug}>)",
...
AskUserQuestion([
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Add --no-ignore to broad searches (includes .oto/)." }
    ]
  },
  {
...
This section lets the user inspect and override the built-in model IDs OTO resolves for each
...
`oto-sdk query config-set runtime <value>` before proceeding to Step C.
...
```bash
oto-sdk query config-set model_profile_overrides.<runtime>.<tier> "<model-id>"
```

For each tier where the user chose "Clear override", remove the key by setting it to null:
```bash
oto-sdk query config-set model_profile_overrides.<runtime>.<tier> null
```
...
Merge the new settings into the existing config at `$OTO_CONFIG_PATH`. This merge is the
core correctness invariant: **preserve every unrelated key** — do not clobber siblings.

Apply each selected value via `oto-sdk query config-set <key> <value>` so the central
...
oto-sdk query config-set workflow.plan_bounce_passes 5
oto-sdk query config-set workflow.subagent_timeout 900
oto-sdk query config-set git.base_branch main
oto-sdk query config-set context_window 1000000
# Runtime model tier examples:
oto-sdk query config-set runtime gemini
oto-sdk query config-set model_profile_overrides.gemini.opus gemini-3-ultra
oto-sdk query config-set model_profile_overrides.gemini.haiku null
```
...
route each write through `oto-sdk query config-set` so sibling preservation is handled by
the central setter.
</step>
...
These settings apply to future /oto-plan-phase, /oto-execute-phase, /oto-discuss-phase,
and /oto-ship runs.

For common-case toggles (model profile, research/plan_check/verifier, branching strategy,
UI/AI phase gates), use /oto-settings.
```
</step>
...
<success_criteria>
- [ ] Current config read from resolved `$OTO_CONFIG_PATH`
...
- [ ] Writes routed through `oto-sdk query config-set` so unrelated keys are preserved
