...
- Any planning operation when .oto/ already exists
...
<required_reading>
@~/.claude/oto/references/continuation-format.md
</required_reading>
...
```bash
INIT=$(oto-sdk query init.resume)
...
**If `planning_exists` is false:** This is a new project - route to /oto-new-project
</step>
...
- This is the primary resumption source — structured data from `/oto-pause-work`
...
📋 [N] pending todos — /oto-check-todos to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/oto-execute-phase {phase} ${OTO_WS})
   OR
1. Discuss Phase 3 context (/oto-discuss-phase 3 ${OTO_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/oto-plan-phase 3 ${OTO_WS}) [if CONTEXT.md exists or discuss option declined]
...
```bash
ls .oto/phases/XX-name/*-CONTEXT.md 2>/dev/null || true
```
...
- **Plan phase** → Show command for user to run after clearing:
  ...
  **Also available:**
  - `/oto-discuss-phase [N] ${OTO_WS}` — gather context first
  - `/oto-research-phase [N] ${OTO_WS}` — investigate unknowns

  ---
  ```
- **Advance to next phase** → ./transition.md (internal workflow, invoked inline — NOT a user command)
- **Check todos** → Read .oto/todos/pending/, present summary
...
4. Count pending todos in .oto/todos/pending/
...
- Cloning repo without full .oto/ state
  </reconstruction>
