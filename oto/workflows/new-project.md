...
- oto-project-researcher — Researches project-level technical decisions
- oto-research-synthesizer — Synthesizes findings from parallel research agents
- oto-roadmapper — Creates phased execution roadmaps
</available_agent_types>
...
- File reference: `/oto-new-project --auto @prd.md`
...
Usage:
  /oto-new-project --auto @your-idea.md
  /oto-new-project --auto [paste or write your idea here]
...
```bash
INIT=$(oto-sdk query init.new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(oto-sdk query agent-skills oto-project-researcher)
AGENT_SKILLS_SYNTHESIZER=$(oto-sdk query agent-skills oto-research-synthesizer)
AGENT_SKILLS_ROADMAPPER=$(oto-sdk query agent-skills oto-roadmapper)
```
...
Subagent spawns (oto-project-researcher, oto-research-synthesizer, oto-roadmapper) will fail
with "agent type not found". Run the installer with --global to make agents available:

  npx oto-cc@latest --global
...
**If `project_exists` is true:** Error — project already initialized. Use `/oto-progress`.
...
- options:
  - "Map codebase first" — Run /oto-map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**

```
Run `/oto-map-codebase` first, then return to `/oto-new-project`
```
...
AskUserQuestion([
  ...
  },
  {
    ...
    options: [
      ...
      { label: "No", description: "Keep .oto/ local-only (add to .gitignore)" }
    ]
  }
])
```
...
```bash
mkdir -p .oto
oto-sdk query commit "chore: add project config" --files .oto/config.json
```
...
# Check for raw spikes/sketches in .oto/
HAS_SPIKES=$(ls .oto/spikes/MANIFEST.md 2>/dev/null)
HAS_SKETCHES=$(ls .oto/sketches/MANIFEST.md 2>/dev/null)
```
...
{if HAS_SPIKES && !SPIKE_SKILL}  ◆ Raw spikes in .oto/spikes/ — consider `/oto-spike-wrap-up` to package findings
{if HAS_SKETCHES && !SKETCH_SKILL}  ◆ Raw sketches in .oto/sketches/ — consider `/oto-sketch-wrap-up` to package findings
...
**After each phase transition** (via `/oto-transition`):
...
**After each milestone** (via `/oto-complete-milestone`):
...
```bash
mkdir -p .oto
oto-sdk query commit "docs: initialize project" --files .oto/PROJECT.md
```
...
**Check for global defaults** at `~/.oto/defaults.json`. If the file exists, read and display its contents before asking:

```bash
DEFAULTS_RAW=$(cat ~/.oto/defaults.json 2>/dev/null)
```
...
```text
Your saved defaults (~/.oto/defaults.json):
  • Mode: [value]
  • Granularity: [value]
...
**If "Configure fresh" or `~/.oto/defaults.json` doesn't exist:** proceed with the questions below.
...
```bash
mkdir -p .oto
oto-sdk query config-new-project '{"mode":"[yolo|interactive]","granularity":"[selected]","parallelization":true|false,"commit_docs":true|false,"model_profile":"quality|balanced|budget|inherit","workflow":{"research":true|false,"plan_check":true|false,"verifier":true|false,"nyquist_validation":[false if granularity=coarse, true otherwise]}}'
```

**Note:** Run `/oto-settings` anytime to update model profile, workflow agents, branching strategy, and other preferences.
...
- question: "I detected separate git repos in this workspace. Which directories contain code that OTO should commit to?"
- multiSelect: true
...
Spawn 4 parallel oto-project-researcher agents with path references:
...
<output>
Write to: .oto/research/STACK.md
Use template: ~/.claude/oto/templates/research-project/STACK.md
</output>
", subagent_type="oto-project-researcher", model="{researcher_model}", description="Stack research")
...
<output>
Write to: .oto/research/FEATURES.md
Use template: ~/.claude/oto/templates/research-project/FEATURES.md
</output>
", subagent_type="oto-project-researcher", model="{researcher_model}", description="Features research")
...
<output>
Write to: .oto/research/ARCHITECTURE.md
Use template: ~/.claude/oto/templates/research-project/ARCHITECTURE.md
</output>
", subagent_type="oto-project-researcher", model="{researcher_model}", description="Architecture research")
...
<output>
Write to: .oto/research/PITFALLS.md
Use template: ~/.claude/oto/templates/research-project/PITFALLS.md
</output>
", subagent_type="oto-project-researcher", model="{researcher_model}", description="Pitfalls research")
```
...
<files_to_read>
- .oto/research/STACK.md
- .oto/research/FEATURES.md
- .oto/research/ARCHITECTURE.md
- .oto/research/PITFALLS.md
</files_to_read>
...
", subagent_type="oto-research-synthesizer", model="{synthesizer_model}", description="Synthesize research")
```
...
```bash
oto-sdk query commit "docs: define v1 requirements" --files .oto/REQUIREMENTS.md
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► CREATING ROADMAP
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning roadmapper...
```

Spawn oto-roadmapper agent with path references:
...
- .oto/REQUIREMENTS.md (v1 Requirements)
- .oto/research/SUMMARY.md (Research findings - if exists)
- .oto/config.json (Granularity and mode settings)
</files_to_read>
...
", subagent_type="oto-roadmapper", model="{roadmapper_model}", description="Create roadmap")
```
...
- Re-spawn roadmapper with revision context:
  ...
  <files_to_read>
  - .oto/ROADMAP.md (Current roadmap to revise)
  </files_to_read>
  ...
  ", subagent_type="oto-roadmapper", model="{roadmapper_model}", description="Revise roadmap")
  ```
...
**If "Review full file":** Display raw `cat .oto/ROADMAP.md`, then re-ask.

**Generate or refresh project instruction file before final commit:**

```bash
oto-sdk query generate-claude-md --output "$INSTRUCTION_FILE"
```

This ensures new projects get the default OTO workflow-enforcement guidance and current project context in `$INSTRUCTION_FILE`.

**Commit roadmap (after approval or auto mode):**

```bash
oto-sdk query commit "docs: create roadmap ([N] phases)" --files .oto/ROADMAP.md .oto/STATE.md .oto/REQUIREMENTS.md "$INSTRUCTION_FILE"
```
...
Exit skill and invoke SlashCommand("/oto-discuss-phase 1 --auto")
...
```bash
PHASE1_SECTION=$(oto-sdk query roadmap.get-phase 1 2>/dev/null)
...
/oto-discuss-phase 1 — gather context and clarify approach

---

**Also available:**
- /oto-ui-phase 1 — generate UI design contract (recommended for frontend phases)
- /oto-plan-phase 1 — skip discussion, plan directly
...
- [ ] oto-roadmapper spawned with context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] ROADMAP.md created with phases, requirement mappings, success criteria
- [ ] STATE.md initialized
- [ ] REQUIREMENTS.md traceability updated
- [ ] `$INSTRUCTION_FILE` generated with OTO workflow guidance (AGENTS.md for Codex, CLAUDE.md otherwise)
- [ ] User knows next step is `/oto-discuss-phase 1`
