...
Spawn a single `oto-codebase-mapper` agent with the selected focus area:

```
Task(
  prompt="Scan this codebase with focus: {focus}. Write results to .oto/codebase/. Produce only: {document_list}",
...
Use `/oto-map-codebase` for a comprehensive 4-area parallel scan.
```
...
- [ ] Output documents written to .oto/codebase/
</success_criteria>
