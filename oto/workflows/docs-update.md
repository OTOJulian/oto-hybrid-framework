...
- oto-doc-writer — Writes and updates project documentation files
- oto-doc-verifier — Verifies factual claims in docs against the live codebase
</available_agent_types>
...
```bash
INIT=$(oto-sdk query docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(oto-sdk query agent-skills oto-doc-writer)
```
...
- `existing_docs` — array of `{path, has_oto_marker}` objects for existing Markdown files
...
- These will be passed to oto-doc-verifier in the verify_docs step for accuracy checking
- If inaccuracies are found, they will be dispatched to oto-doc-writer in `fix` mode for surgical corrections
...
4. For each gap the user selects:
   ...
   - The oto-doc-writer will receive a `doc_assignment` with `type: "custom"` and a description of what to document, using the project's source files as content discovery targets
...
3. If no docs in the queue have `has_oto_marker: false` in the `existing_docs` array: skip to detect_runtime_capabilities.

**For each queued doc where `has_oto_marker` is false (hand-written doc detected):**

Present the following choice using `AskUserQuestion` if available, or inline prompt otherwise:

```
{filename} appears to be hand-written (no OTO marker found).
...
How should this file be handled?
  ...
  [3] regenerate  -- Overwrite with a fresh OTO-generated doc.
```
...
**Read the work manifest first:** `Read .oto/tmp/docs-work-manifest.json` — use `canonical_queue` items with `wave: 1` for this step.

Spawn 3 parallel oto-doc-writer agents for Wave 1 docs: README, ARCHITECTURE, CONFIGURATION.
...
**Read the work manifest first:** `Read .oto/tmp/docs-work-manifest.json` — update `status` to `"completed"` or `"failed"` for each Wave 1 item after collection. Write the updated manifest back to disk.
...
**Read the work manifest first:** `Read .oto/tmp/docs-work-manifest.json` — use `canonical_queue` items with `wave: 2` for this step.
...
**Read the work manifest first:** `Read .oto/tmp/docs-work-manifest.json` — update `status` to `"completed"` or `"failed"` for each Wave 2 item after collection. Write the updated manifest back to disk.
...
Spawn a `oto-doc-writer` agent with `run_in_background=true`:
...
**Fallback when Task tool is unavailable:** Generate per-package READMEs sequentially inline after the `sequential_generation` step. For each package directory with a `package.json`, construct the equivalent `doc_assignment` block and generate the README following oto-doc-writer instructions.
...
**Read the work manifest first:** `Read .oto/tmp/docs-work-manifest.json` — use `canonical_queue` items for generation order. Update `status` after each doc is generated. Write the updated manifest back to disk after all docs are complete.
...
Read `agents/oto-doc-writer.md` instructions once before beginning. Follow the create_mode or update_mode instructions from that agent for each doc, using the same doc_assignment fields as the parallel path.
...
1. **README** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase (Read, Grep, Glob, Bash) following oto-doc-writer create_mode / update_mode instructions
...
2. **ARCHITECTURE** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase following oto-doc-writer instructions
...
- Follow oto-doc-writer instructions for per_package scope
...
```
Read .oto/tmp/docs-work-manifest.json
```
...
1. Spawn the `oto-doc-verifier` agent (or invoke sequentially if Task tool is unavailable) with a `<verify_assignment>` block:
   ```xml
   <verify_assignment>
...
1. For each doc with `claims_failed > 0` in the latest verification_results:
   ...
   b. Spawn `oto-doc-writer` agent (or invoke sequentially) with a fix assignment:
      ```xml
      <doc_assignment>
...
These failures require manual correction. Review the verification output in .oto/tmp/verify-*.json for details.
```
...
Invoke the oto-doc-verifier agent in read-only mode for each file in `existing_docs` from the init JSON:

1. For each doc in `existing_docs`:
   a. Spawn `oto-doc-verifier` (or invoke sequentially if Task tool is unavailable) with:
      ```xml
      <verify_assignment>
      doc_path: {doc.path}
      project_root: {project_root from init JSON}
...
Display note:
```
To fix failures automatically: /oto-docs-update (runs generation + fix loop)
...
3. Re-run /oto-docs-update to regenerate clean docs
```
...
```bash
oto-sdk query commit "docs: generate project documentation" \
...
<step name="report">
**Read the work manifest first:** `Read .oto/tmp/docs-work-manifest.json` — use the manifest to compile the complete report covering all canonical docs, review_queue results, and gap_queue results. The manifest is the source of truth for what was processed.
...
```
Run `/oto-docs-update --verify-only` to fact-check generated docs against the codebase.
```
...
- [ ] Generated docs contain zero OTO methodology content
...
- [ ] Hand-written docs (no OTO marker) prompted for preserve/supplement/regenerate before dispatch (unless --force)
...
- [ ] --verify-only invokes oto-doc-verifier for full fact-checking (not just VERIFY marker count)
</success_criteria>
