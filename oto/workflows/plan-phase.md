<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates oto-phase-researcher, oto-planner, and oto-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
@~/.claude/oto/references/revision-loop.md
...
AGENT_SKILLS_PLANNER=$(oto-sdk query agent-skills oto-planner)
AGENT_SKILLS_CHECKER=$(oto-sdk query agent-skills oto-plan-checker)
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@~/.claude/oto/references/tdd.md` so gate enforcement rules are available during planning.
...
```bash
mkdir -p ".oto/phases/${padded_phase}-${phase_slug}"
```
...
```bash
CHUNKED_CFG=$(oto-sdk query config-get workflow.plan_chunked 2>/dev/null || echo "false")
CHUNKED_MODE=false
...
Then re-run /oto-plan-phase {N} --reviews
```
Exit workflow.
...
5. Commit:
```bash
oto-sdk query commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /oto-plan-phase {X} ${OTO_WS}:

  /oto-discuss-phase {X} ${OTO_WS}
  ```
...
AI_PHASE_CFG=$(oto-sdk query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
...
◆ Note: This phase appears to involve AI system development.
  Consider running /oto-ai-integration-phase {N} before planning to:
  - Select the right framework for your use case
  - Research its docs and best practices
  - Design an evaluation strategy

  Continue planning without AI-SPEC? (non-blocking — /oto-ai-integration-phase can be run after)
```

Use AskUserQuestion with options:
- "Continue — plan without AI-SPEC"
- "Stop — I'll run /oto-ai-integration-phase {N} first"

If "Stop": Exit with `/oto-ai-integration-phase {N}` reminder.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher...
```

### Spawn oto-phase-researcher

```bash
PHASE_DESC=$(oto-sdk query roadmap.get-phase "${PHASE}" --pick section)
```
...
**If found:**
1. Read template: `~/.claude/oto/templates/VALIDATION.md`
...
SECURITY_ASVS=$(oto-sdk query config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(oto-sdk query config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► SECURITY THREAT MODEL REQUIRED (ASVS L{SECURITY_ASVS})
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Each PLAN.md must include a <threat_model> block.
Block on: {SECURITY_BLOCK} severity threats.
Opt out: set security_enforcement: false in .oto/config.json
```
...
UI_GATE_CFG=$(oto-sdk query config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(oto-sdk query roadmap.get-phase "${PHASE}" 2>/dev/null)
...
```bash
AUTO_CHAIN=$(oto-sdk query check auto-mode --pick auto_chain_active 2>/dev/null || echo "false")
```
...
```
Skill(skill="oto-ui-phase", args="${PHASE} --auto ${OTO_WS}")
```
After `oto-ui-phase` returns, re-read:
```bash
UI_SPEC_FILE=$(ls "${PHASE_DIR}"/*-UI-SPEC.md 2>/dev/null | head -1)
...
`/oto-ui-phase {N} ${OTO_WS}` — generate UI design contract before planning
───────────────────────────────────────────────
Also available:
- `/oto-plan-phase {N} --skip-ui ${OTO_WS}` — plan without UI-SPEC (not recommended for frontend phases)
```
...
```bash
PHASE_SECTION=$(oto-sdk query roadmap.get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
```bash
_oto_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
STATE_PATH=$(_oto_field "$INIT" state_path)
ROADMAP_PATH=$(_oto_field "$INIT" roadmap_path)
REQUIREMENTS_PATH=$(_oto_field "$INIT" requirements_path)
RESEARCH_PATH=$(_oto_field "$INIT" research_path)
VERIFICATION_PATH=$(_oto_field "$INIT" verification_path)
UAT_PATH=$(_oto_field "$INIT" uat_path)
CONTEXT_PATH=$(_oto_field "$INIT" context_path)
REVIEWS_PATH=$(_oto_field "$INIT" reviews_path)
PATTERNS_PATH=$(_oto_field "$INIT" patterns_path)
...
1. Re-run: `/oto-plan-phase {PHASE} --research ${OTO_WS}`
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @~/.claude/oto/references/tdd.md to all eligible tasks:
...
<downstream_consumer>
Output consumed by /oto-execute-phase. Plans need:
...
`/oto-plan-phase {N} --chunked` resumes from the last successfully committed plan.

**Intended for new or in-progress chunked runs.** To recover plans already written by a prior
*non-chunked* run, use step 6's "Add more plans" or proceed directly to `/oto-execute-phase`
...
5. **Commit per-plan:**
   ```bash
   oto-sdk query commit "docs(${PADDED_PHASE}): plan ${plan_id} (chunked)" --files "${PHASE_DIR}/${plan_id}-PLAN.md"
   ```
...
3. **Stop** — exit; user can re-run `/oto-plan-phase {N}` to resume
...
**If "Split":** Use `/oto-insert-phase` to create the sub-phases, then replan each.
...
Task(
  ...
  subagent_type="oto-plan-checker",
  model="{checker_model}",
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count >= 2`:**
    ...
    Suggest: "Consider resolving these issues manually or running `/oto-debug` to investigate root causes."
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(oto-sdk query config-get workflow.plan_bounce_passes 2>/dev/null || echo "2")
BOUNCE_SCRIPT=$(oto-sdk query config-get workflow.plan_bounce_script 2>/dev/null | jq -r '.' 2>/dev/null || true)
```

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► BOUNCING PLANS (External Refinement)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
5. **Re-run plan checker on bounced plans:** Spawn oto-plan-checker (same as step 10) on all modified plans. If a bounced plan fails the checker, restore original from its pre-bounce.md backup:
```
...
```bash
oto-sdk query commit "refactor(${padded_phase}): bounce plans through external refinement" --files "${PHASE_DIR}/*-PLAN.md"
```
...
```bash
GATE_CFG=$(oto-sdk query config-get workflow.context_coverage_gate 2>/dev/null || echo "true")
if [ "$GATE_CFG" != "false" ]; then
  GATE_RESULT=$(oto-sdk query check.decision-coverage-plan "${PHASE_DIR}" "${CONTEXT_PATH}")
...
```bash
oto-sdk query state.planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
oto-sdk query roadmap.annotate-dependencies "${PHASE_NUMBER}"
```
...
```bash
oto-sdk query commit "docs(${PADDED_PHASE}): create phase plan" --files "${PHASE_DIR}"/*-PLAN.md .oto/STATE.md .oto/ROADMAP.md
```
...
```bash
POST_PLANNING_GAPS=$(oto-sdk query config-get workflow.post_planning_gaps --default true 2>/dev/null || echo true)
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  oto-tools gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
...
re-run `/oto-plan-phase --gaps` to add plans, or proceed to execute-phase as-is.
...
if ([[ "$ARGUMENTS" =~ --auto ]] || [[ "$ARGUMENTS" =~ --chain ]]) && [[ "$AUTO_CHAIN" != "true" ]]; then
  oto-sdk query config-set workflow._auto_chain_active true
fi
```
...
```
Skill(skill="oto-execute-phase", args="${PHASE} --auto --no-transition ${OTO_WS}")
```
...
- **PHASE COMPLETE** → Display final summary:
  ```
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   OTO ► PHASE ${PHASE} COMPLETE ✓
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Auto-advance pipeline finished.

  Next: /oto-discuss-phase ${NEXT_PHASE} --auto ${OTO_WS}
  ```
...
/oto-execute-phase {X} ${OTO_WS}

───────────────────────────────────────────────────────────────

**Also available:**
- cat .oto/phases/{phase-dir}/*-PLAN.md — review plans
- /oto-plan-phase {X} --research — re-research first
- /oto-review --phase {X} --all — peer review plans with external AIs
- /oto-plan-phase {X} --reviews — replan incorporating review feedback
...
5. **Retry:** Restart Claude Code and run `/oto-plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/oto-plan-phase N --skip-research
```
</windows_troubleshooting>

<success_criteria>
- [ ] .oto/ directory validated
- [ ] Phase validated against roadmap
- [ ] Phase directory created if needed
- [ ] CONTEXT.md loaded early (step 4) and passed to ALL agents
- [ ] Research completed (unless --skip-research or --gaps or exists)
- [ ] oto-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] oto-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] oto-plan-checker spawned with CONTEXT.md
