...
Seeds auto-surface during /oto-new-milestone when trigger conditions match the
...
EXISTING=$( (ls .oto/seeds/SEED-*.md 2>/dev/null || true) | wc -l )
NEXT=$((EXISTING + 1))
...
This seed should be presented during `/oto-new-milestone` when the milestone
...
```bash
oto-sdk query commit "docs: plant seed — {$IDEA}" --files .oto/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
Trigger: {$TRIGGER}
Scope: {$SCOPE}
File: .oto/seeds/SEED-{PADDED}-{slug}.md

This seed will surface automatically when you run /oto-new-milestone
...
<success_criteria>
- [ ] Seed file created in .oto/seeds/
