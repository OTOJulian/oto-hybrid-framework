...
- oto-executor — executes a specific, scoped code change
</available_agent_types>
...
```bash
INIT=$(oto-sdk query audit-uat 2>/dev/null || echo "{}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Read existing UAT and verification files to extract findings:
- Glob: `.oto/phases/*/*-UAT.md`
- Glob: `.oto/phases/*/*-VERIFICATION.md`
...
**b. Run tests:**
```bash
AUDIT_TEST_CMD=$(oto-sdk query config-get workflow.test_command --default "" 2>/dev/null || true)
