<purpose>
Orchestrate parallel codebase mapper agents to analyze codebase and produce structured documents in .oto/codebase/
...
post-execute codebase-drift gate in `/oto-execute-phase` or by a user running
`/oto-map-codebase --paths apps/accounting,packages/ui`), the workflow
operates in **incremental-remap mode**:

- Pass `--paths <p1>,<p2>,...` through to each spawned `oto-codebase-mapper`
...
```bash
INIT=$(oto-sdk query init.map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(oto-sdk query agent-skills oto-codebase-mapper)
```
...
Check if .oto/codebase/ already exists using `has_maps` from init context.
...
```
.oto/codebase/ already exists with these documents:
[List files found]
...
If "Refresh": Delete .oto/codebase/, continue to create_structure
...
Create .oto/codebase/ directory:

```bash
mkdir -p .oto/codebase
```
...
Spawn 4 parallel oto-codebase-mapper agents.

Use Task tool with `subagent_type="oto-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `oto-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.

**Agent 1: Tech Focus**

```
Task(
  subagent_type="oto-codebase-mapper",
  model="{mapper_model}",
...
Write these documents to .oto/codebase/:
...
Write this document to .oto/codebase/:
...
grep -E '(sk-[a-zA-Z0-9]{20,}|sk_live_[a-zA-Z0-9]+|sk_test_[a-zA-Z0-9]+|ghp_[a-zA-Z0-9]{36}|gho_[a-zA-Z0-9]{36}|glpat-[a-zA-Z0-9_-]+|AKIA[A-Z0-9]{16}|xox[baprs]-[a-zA-Z0-9-]+|-----BEGIN.*PRIVATE KEY|eyJ[a-zA-Z0-9_-]+\.eyJ[a-zA-Z0-9_-]+\.)' .oto/codebase/*.md 2>/dev/null && SECRETS_FOUND=true || SECRETS_FOUND=false
```
...
```bash
oto-sdk query commit "docs: map existing codebase" --files .oto/codebase/*.md
```
...
**Also available:**
- Re-run mapping: `/oto-map-codebase`
- Review specific file: `cat .oto/codebase/STACK.md`
...
<success_criteria>
- .oto/codebase/ directory created
- If Task tool available: 4 parallel oto-codebase-mapper agents spawned with run_in_background=true
- If Task tool NOT available: 4 sequential mapping passes performed inline (never using browser_subagent)
- All 7 codebase documents exist
- No empty documents (each should have >20 lines)
- Clear completion summary with line counts
- User offered clear next steps in OTO style
</success_criteria>
