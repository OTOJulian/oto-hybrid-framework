...
- **--from**: Import external plan → conflict detection → write PLAN.md → validate via oto-plan-checker
...
- If `--prd` is present: display message that `--prd` is not yet implemented and exit:
  ```
  OTO > --prd mode is planned for a future release. Use --from to import plan files.
  ```
- If neither flag is found: display usage and exit:

```
Usage: /oto-import --from <path>

  --from <path>   Import an external plan file into OTO format
```
...
4. Glob for all CONTEXT.md files across phase directories:
   ```bash
   find .oto/phases/ -name "*-CONTEXT.md" -o -name "CONTEXT.md" 2>/dev/null
   ```
...
- **OTO PLAN.md format**: Has YAML frontmatter with `phase:`, `plan:`, `type:` fields
...
Convert the imported content to OTO PLAN.md format.
...
If the imported plan references PBR plan naming (e.g., `PLAN-01.md`, `plan-01.md`), rename all references to OTO `{NN}-{MM}-PLAN.md` convention during conversion.

Apply OTO naming convention for the output filename:
...
Delegate validation to oto-plan-checker:

```
Task({
  subagent_type: "oto-plan-checker",
  prompt: "Validate: .oto/phases/{phase}/{plan}-PLAN.md — check frontmatter completeness, task structure, and OTO conventions. Report any issues."
})
```
...
```bash
oto-sdk query commit "docs({phase}): import plan from {basename FILEPATH}" --files .oto/phases/{phase}/{plan}-PLAN.md .oto/ROADMAP.md
```
