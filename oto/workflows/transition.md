...
There is no `/oto-transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/oto-transition`.

**Valid user commands for phase progression:**
- `/oto-discuss-phase {N}` — discuss a phase before planning
- `/oto-plan-phase {N}` — plan a phase
- `/oto-execute-phase {N}` — execute a phase
- `/oto-progress` — see roadmap progress
...
cat .oto/PROJECT.md 2>/dev/null || true
```
...
```bash
(ls .oto/phases/XX-current/*-PLAN.md 2>/dev/null || true) | sort
(ls .oto/phases/XX-current/*-SUMMARY.md 2>/dev/null || true) | sort
```
...
```bash
cat .oto/config.json 2>/dev/null || true
```
...
OUTSTANDING=""
for f in .oto/phases/XX-current/*-UAT.md .oto/phases/XX-current/*-VERIFICATION.md; do
  [ -f "$f" ] || continue
...
These will carry forward as debt. Review: `/oto-audit-uat`
```
...
```bash
ls .oto/phases/XX-current/.continue-here*.md 2>/dev/null || true
```
...
**Delegate ROADMAP.md and STATE.md updates to `oto-sdk query phase.complete`:**

```bash
TRANSITION=$(oto-sdk query phase.complete "${current_phase}")
```
...
```bash
cat .oto/phases/XX-current/*-SUMMARY.md
```
...
```text
@~/.claude/oto/workflows/graduation.md
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `oto-sdk query phase.complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(oto-sdk query progress.bar --raw)
```
...
See: .oto/PROJECT.md (updated [today])
...
**Use the transition result from `oto-sdk query phase.complete`:**
...
**Skip this check if NOT in workstream mode** (i.e., `OTO_WORKSTREAM` is not set / flat mode).
In flat mode, go directly to **Route B**.

```bash
# Only check if we're in workstream mode
if [ -n "$OTO_WORKSTREAM" ]; then
  WS_LIST=$(oto-sdk query workstream.list --raw)
fi
```
...
Filter out the current workstream (`$OTO_WORKSTREAM`) and any workstreams with
...
```bash
ls .oto/phases/*[X+1]*/*-CONTEXT.md 2>/dev/null || true
```
...
Exit skill and invoke SlashCommand("/oto-plan-phase [X+1] --auto ${OTO_WS}")
...
Exit skill and invoke SlashCommand("/oto-discuss-phase [X+1] --auto ${OTO_WS}")
...
`/oto-discuss-phase [X+1] ${OTO_WS}` — gather context and clarify approach

---

**Also available:**
- `/oto-plan-phase [X+1] ${OTO_WS}` — skip discussion, plan directly
- `/oto-research-phase [X+1] ${OTO_WS}` — investigate unknowns
...
`/oto-plan-phase [X+1] ${OTO_WS}`

---

**Also available:**
- `/oto-discuss-phase [X+1] ${OTO_WS}` — revisit context
- `/oto-research-phase [X+1] ${OTO_WS}` — investigate unknowns
...
```bash
oto-sdk query config-set workflow._auto_chain_active false
```
...
`/oto-workstreams complete {current_ws_name} ${OTO_WS}`

See overall milestone progress:

`/oto-workstreams progress ${OTO_WS}`
...
Do NOT suggest `/oto-complete-milestone` or `/oto-new-milestone`.
...
Exit skill and invoke SlashCommand("/oto-complete-milestone {version} ${OTO_WS}")
...
`/oto-complete-milestone {version} ${OTO_WS}`
