...
- oto-code-reviewer — Reviews source files for bugs, security issues, and code quality
</available_agent_types>
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► QUICK TASK (DISCUSS + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► QUICK TASK (DISCUSS + RESEARCH)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► QUICK TASK (RESEARCH + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```bash
if ! command -v oto-sdk &>/dev/null; then
  echo "⚠ oto-sdk not found in PATH — /oto-quick requires it."
  echo ""
  echo "Install the OTO SDK:"
  echo "  npm install -g @oto-build/sdk"
  echo ""
  echo "Or update OTO to get the latest packages:"
  echo "  /oto-update"
  exit 1
fi
```
...
```bash
INIT=$(oto-sdk query init.quick "$DESCRIPTION")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_PLANNER=$(oto-sdk query agent-skills oto-planner)
AGENT_SKILLS_EXECUTOR=$(oto-sdk query agent-skills oto-executor)
AGENT_SKILLS_CHECKER=$(oto-sdk query agent-skills oto-plan-checker)
AGENT_SKILLS_VERIFIER=$(oto-sdk query agent-skills oto-verifier)
```
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/oto-new-project` first.
...
```bash
QUICK_DIR=".oto/quick/${quick_id}-${slug}"
mkdir -p "$QUICK_DIR"
```
...
<files_to_read>
- .oto/STATE.md (Project state — what's already built)
...
Spawn oto-executor with plan reference:
...
1. **Worktree cleanup:** If the executor ran with `isolation="worktree"`, merge the worktree branch back and clean up:
   ...
   while IFS= read -r WT; do
     ...
     if [ -n "$WT_BRANCH" ] && [ "$WT_BRANCH" != "HEAD" ]; then
       ...
       # Pre-merge deletion guard: block merges that delete tracked .oto/ files
       ...
       if [ -s "$STATE_BACKUP" ]; then cp "$STATE_BACKUP" .oto/STATE.md; fi
       if [ -s "$ROADMAP_BACKUP" ]; then cp "$ROADMAP_BACKUP" .oto/ROADMAP.md; fi
       rm -f "$STATE_BACKUP" "$ROADMAP_BACKUP"
...
Task(
  ...
  Depth: quick",
  subagent_type="oto-code-reviewer",
  model="{executor_model}"
)
```
...
Stage and commit quick task artifacts. This step MUST always run — even if the executor already committed some files (e.g. when running without worktree isolation). The `oto-sdk query commit` command (or legacy `oto-tools.cjs` commit) handles already-committed files gracefully.
...
oto-sdk query commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
...
OTO > QUICK TASK COMPLETE (VALIDATED)
...
Ready for next task: /oto-quick ${OTO_WS}
```
