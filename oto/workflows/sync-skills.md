# sync-skills — Cross-Runtime OTO Skill Sync

**Command:** `/oto-sync-skills`

Sync managed `oto-*` skill directories from one canonical runtime's skills root to one or more destination runtime skills roots. Keeps multi-runtime installs aligned after a `oto-update` on one runtime.
...
```bash
INSTALL_JS="$(dirname "$0")/../oto/bin/install.js"
# If running from a global install, resolve relative to the OTO package
INSTALL_JS_GLOBAL="$HOME/.claude/oto/bin/install.js"
...
error: source skills root not found: <path>
       Is OTO installed globally for the '<runtime>' runtime?
       Run: node ~/.claude/oto/bin/install.js --global --<runtime>
```
Then exit.
...
```bash
# List oto-* subdirectories in source
SRC_SKILLS=$(ls -1 "$SRC_SKILLS_ROOT" 2>/dev/null | grep '^oto-')

# List oto-* subdirectories in destination (may not exist yet)
DST_SKILLS=$(ls -1 "$DEST_ROOT" 2>/dev/null | grep '^oto-')

# Diff:
# CREATE  — in SRC but not in DST
# UPDATE  — in both; content differs (compare recursively via checksums)
# REMOVE  — in DST but not in SRC (stale OTO skill no longer in source)
# SKIP    — in both; content identical (already up to date)
```

**Non-OTO preservation:** Only `oto-*` entries are ever created, updated, or removed. Entries in the destination that do not start with `oto-` are never touched.
...
CREATE: oto-help
UPDATE: oto-update
REMOVE: oto-old-command
SKIP:   oto-plan-phase (up to date)
(N changes)
...
1. **Only `oto-*` directories** are created, updated, or removed. Any directory not starting with `oto-` in a destination root is untouched.
2. **Dry-run is the default.** `--apply` must be passed explicitly to write anything.
3. **Source root must exist.** Never create the source root; it must have been created by a prior `oto-update` or installer run.
...
- Sync copies files verbatim and does not apply runtime-specific content transformations. Use the OTO installer directly for runtimes that require format conversion.
