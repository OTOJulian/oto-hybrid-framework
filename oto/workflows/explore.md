...
offers mid-conversation research when useful, then routes crystallized outputs to OTO artifacts.
</purpose>
...
- oto-phase-researcher — Researches specific questions and returns concise findings
</available_agent_types>
...
| Sketch | `/oto-sketch` (invoke) | Design direction unclear — "what should this look like?", "how should this feel?" |
...
```bash
oto-sdk query commit "docs: capture exploration — {topic_slug}" --files {file_list}
```
...
Continue exploring with `/oto-explore` or start working with `/oto-next`.
```
