...
- oto-assumptions-analyzer — Analyzes codebase to surface implementation assumptions
</available_agent_types>
...
AGENT_SKILLS_ANALYZER=$(oto-sdk query agent-skills oto-assumptions-analyzer)
```
...
Use /oto-progress to see available phases.
```
Exit workflow.
...
```bash
(find .oto/phases -name "*-CONTEXT.md" 2>/dev/null || true) | sort
```
...
```bash
cat .oto/METHODOLOGY.md 2>/dev/null || true
```
...
- When spawning the oto-assumptions-analyzer, pass the lens list so it can flag which lenses apply
...
```bash
ls .oto/codebase/*.md 2>/dev/null || true
```
...
Spawn a `oto-assumptions-analyzer` agent to deeply analyze the codebase for this phase. This
...
```
Task(subagent_type="oto-assumptions-analyzer", prompt="""
...
```bash
oto-sdk query commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
`/oto-plan-phase ${PHASE}`

---

**Also available:**
- `/oto-plan-phase ${PHASE} --skip-research` — plan without research
- `/oto-ui-phase ${PHASE}` — generate UI design contract (if frontend work)
...
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► AUTO-ADVANCING TO PLAN
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Context captured (assumptions mode). Launching plan-phase...
```

Launch: `Skill(skill="oto-plan-phase", args="${PHASE} --auto")`
