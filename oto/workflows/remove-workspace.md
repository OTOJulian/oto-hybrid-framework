<purpose>
Remove a OTO workspace, cleaning up git worktrees and deleting the workspace directory.
</purpose>
...
```bash
INIT=$(oto-sdk query init.remove-workspace "$WORKSPACE_NAME")
...
First run `/oto-list-workspaces` to show available workspaces, then ask:
