...
Scan a repo for mixed planning documents (ADR, PRD, SPEC, DOC), synthesize them into a consolidated context, and bootstrap or merge into `.oto/`.

- `[path]` — optional target directory to scan (defaults to repo root)
- `--mode new|merge` — override auto-detect (defaults: `new` if `.oto/` absent, `merge` if present)
...
If user passed `--mode new` but `.oto/` already exists: display warning and require explicit confirm via `AskUserQuestion` (approve-revise-abort from `references/gate-prompts.md`) before overwriting.
...
```
OTO > Discovered {N} docs, which exceeds the v1 cap of 50.
      Use --manifest to narrow the set to ≤ 50 files, or run
      /oto-ingest-docs again with a narrower <path>.
```
...
```bash
mkdir -p .oto/intel/classifications/
```

For each discovered doc, spawn `oto-doc-classifier` in parallel. In Claude Code, issue all Task calls in a single message with multiple tool uses so the harness runs them concurrently. For Copilot / sequential runtimes, fall back to sequential dispatch.
...
- `<required_reading>` — `agents/oto-doc-classifier.md` (the agent definition itself)
...
Spawn `oto-doc-synthesizer` once:

```
Task({
  subagent_type: "oto-doc-synthesizer",
  prompt: "
    CLASSIFICATIONS_DIR: .oto/intel/classifications/
    INTEL_DIR: .oto/intel/
    CONFLICTS_PATH: .oto/INGEST-CONFLICTS.md
    MODE: {MODE}
    EXISTING_CONTEXT: {paths to existing .oto files if MODE=merge, else empty}
...
- `.oto/intel/decisions.md`, `.oto/intel/requirements.md`, `.oto/intel/constraints.md`, `.oto/intel/context.md`
- `.oto/intel/SYNTHESIS.md`
- `.oto/INGEST-CONFLICTS.md`

</step>

<step name="conflict_gate">

Read `.oto/INGEST-CONFLICTS.md`. Count entries in each bucket (the synthesizer always writes the three-bucket header; parse the `### BLOCKERS ({N})`, `### WARNINGS ({N})`, `### INFO ({N})` lines).
...
```
OTO > BLOCKED: {N} blockers must be resolved before ingest can proceed.
```
...
On Abort: exit cleanly with "Ingest cancelled. Staged intel preserved at `.oto/intel/`."

**If BLOCKERS = 0 and WARNINGS = 0:**

Proceed to routing silently, or optionally display `OTO > No conflicts. Auto-resolved: {N}.`
...
Load existing `.oto/ROADMAP.md`, `.oto/PROJECT.md`, `.oto/REQUIREMENTS.md`, all `CONTEXT.md` files under `.oto/phases/`.
...
Plan the merge:
- **New requirements** from synthesized `.oto/intel/requirements.md` that do not overlap existing REQUIREMENTS.md entries → append to REQUIREMENTS.md
- **New decisions** from synthesized `.oto/intel/decisions.md` that do not overlap existing CONTEXT.md `<decisions>` blocks → write to a new phase's CONTEXT.md or append to the next milestone's requirements
- **New scope** → derive phase additions following the `new-milestone.md` pattern; append phases to `.oto/ROADMAP.md`
...
```bash
oto-sdk query commit "docs: ingest {N} docs from {SCAN_PATH} (#2387)" --files 2>/dev/null \
  .oto/PROJECT.md \
  .oto/REQUIREMENTS.md \
  .oto/ROADMAP.md \
  .oto/STATE.md \
  .oto/intel/ \
  .oto/INGEST-CONFLICTS.md || true
```
...
- Conflict report path (`.oto/INGEST-CONFLICTS.md`)
- Next step: `/oto-plan-phase 1` (new mode) or `/oto-plan-phase N` (merge, pointing at the first newly-added phase)
