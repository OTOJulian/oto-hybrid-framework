...
```bash
GRADUATION_ENABLED=$(oto-sdk query config-get features.graduation 2>/dev/null || echo "true")
GRADUATION_WINDOW=$(oto-sdk query config-get features.graduation_window 2>/dev/null || echo "5")
GRADUATION_THRESHOLD=$(oto-sdk query config-get features.graduation_threshold 2>/dev/null || echo "3")
```
...
```bash
find .oto/phases -name "*-LEARNINGS.md" | sort | tail -n "$GRADUATION_WINDOW"
```
