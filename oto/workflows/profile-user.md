...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing `oto-sdk query` handlers (with legacy `oto-tools.cjs` parity where needed) and the oto-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/oto/references/ui-brand.md (display patterns)
- @$HOME/.claude/agents/oto-user-profiler.md (profiler agent definition)
- @$HOME/.claude/oto/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.claude/oto/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/oto/USER-PROFILE.md" "$HOME/.claude/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.claude/oto/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /oto-profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(oto-sdk query scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(oto-sdk query profile-sample --json 2>/dev/null)
```
...
**Spawn oto-user-profiler agent using Task tool:**

Use the Task tool to spawn the `oto-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/oto/references/user-profiling.md`
...
Reference: @$HOME/.claude/oto/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(oto-sdk query profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/oto-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(oto-sdk query profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/oto-profile-analysis-XXXXXX.json)
```
...
```bash
oto-sdk query write-profile --input "$ANALYSIS_PATH" --json
```

Display: "✓ Profile written to $HOME/.claude/oto/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/oto-dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/oto/USER-PROFILE.md" and jump to step 10.
...
**For /oto-dev-preferences (if selected):**

```bash
oto-sdk query generate-dev-preferences --analysis "$ANALYSIS_PATH" --json
```

Display: "✓ Generated /oto-dev-preferences at $HOME/.claude/commands/oto/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
oto-sdk query generate-claude-profile --analysis "$ANALYSIS_PATH" --json
```
...
```bash
oto-sdk query generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any `oto-sdk query` or oto-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO > PROFILE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    $HOME/.claude/oto/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /oto-dev-preferences   $HOME/.claude/commands/oto/dev-preferences.md
...
- [ ] Artifacts generated sequentially via oto-sdk query (or oto-tools.cjs) subcommands
