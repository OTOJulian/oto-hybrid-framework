...
> `$HOME/.claude/oto/USER-PROFILE.md` exists). Skip the Read
...
```bash
ADVISOR_MODEL=$(oto-sdk query resolve-model oto-advisor-researcher --raw)
```
...
```bash
PROFILE_CONTENT=$(cat "$HOME/.claude/oto/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Task()` in parallel:
   ...
   ```
   Task(
     prompt="First, read @~/.claude/agents/oto-advisor-researcher.md for your role and instructions.
