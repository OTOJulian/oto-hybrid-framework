...
**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/oto-new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/oto-new-milestone` first."
...
```bash
DETAIL=$(oto-sdk query roadmap.get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(oto-sdk query init.phase-op ${PHASE_NUM})
```
...
```bash
oto-sdk query commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="oto:discuss-phase", args="${PHASE_NUM}")
```
...
```bash
PHASE_SECTION=$(oto-sdk query roadmap.get-phase ${PHASE_NUM} 2>/dev/null)
...
```
Skill(skill="oto-ui-phase", args="${PHASE_NUM}")
```
...
Agent(
  ...
  prompt="Run plan-phase for phase ${PHASE_NUM}: Skill(skill=\"oto:plan-phase\", args=\"${PHASE_NUM}\")"
)
```
...
```
Skill(skill="oto-plan-phase", args="${PHASE_NUM}")
```
...
Agent(
  ...
  run_in_background=true,
  prompt="Run execute-phase for phase ${PHASE_NUM}: Skill(skill=\"oto:execute-phase\", args=\"${PHASE_NUM} --no-transition\")"
)
```
...
```
Skill(skill="oto-execute-phase", args="${PHASE_NUM} --no-transition")
```
...
```
Skill(skill="oto-plan-phase", args="${PHASE_NUM} --gaps")
```
...
```bash
UI_REVIEW_CFG=$(oto-sdk query config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```
Skill(skill="oto-ui-review", args="${PHASE_NUM}")
```
...
> Full instructions are in `oto/references/autonomous-smart-discuss.md`. Read that file now and follow it exactly.

Smart discuss is an autonomous-optimized variant of `oto-discuss-phase`. It proposes grey area answers in batch tables — the user accepts or overrides per area — and writes an identical CONTEXT.md to what discuss-phase produces.

**Inputs:** `PHASE_NUM` from execute_phase.

Read and execute: `$HOME/.claude/oto/references/autonomous-smart-discuss.md`
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► AUTONOMOUS ▸ --to ${TO_PHASE} REACHED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Completed through phase ${TO_PHASE} as requested.
 Remaining phases were not executed.

 Resume with: /oto-autonomous --from ${next_incomplete_phase}
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► AUTONOMOUS ▸ PHASE ${ONLY_PHASE} COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Phase ${ONLY_PHASE}: ${PHASE_NAME} — Done
 Mode: Single phase (--only)

 Lifecycle skipped — run /oto-autonomous without --only
...
```
Skill(skill="oto-audit-milestone")
```

After audit completes, detect the result:

```bash
AUDIT_FILE=".oto/v${milestone_version}-MILESTONE-AUDIT.md"
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /oto-audit-milestone to review, then /oto-complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /oto-audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="oto-complete-milestone", args="${milestone_version}")
```

After complete-milestone returns, verify it produced output:

```bash
ls .oto/milestones/v${milestone_version}-ROADMAP.md 2>/dev/null || true
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /oto-autonomous ${ONLY_PHASE ? "--only " + ONLY_PHASE : "--from " + next_phase}${TO_PHASE ? " --to " + TO_PHASE : ""}
```
...
- [ ] Audit invoked via Skill(skill="oto-audit-milestone")
...
- [ ] `--interactive` runs discuss inline via oto:discuss-phase (asks questions, waits for user)
