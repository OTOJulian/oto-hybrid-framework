<purpose>
Generate a UI design contract (UI-SPEC.md) for frontend phases. Orchestrates oto-ui-researcher and oto-ui-checker with a revision loop. Inserts between discuss-phase and plan-phase in the lifecycle.
...
AGENT_SKILLS_UI=$(oto-sdk query agent-skills oto-ui-researcher)
AGENT_SKILLS_UI_CHECKER=$(oto-sdk query agent-skills oto-ui-checker)
```
...
```bash
UI_RESEARCHER_MODEL=$(oto-sdk query resolve-model oto-ui-researcher --raw)
UI_CHECKER_MODEL=$(oto-sdk query resolve-model oto-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(oto-sdk query config-get workflow.ui_phase 2>/dev/null || echo "true")
```

**If `UI_ENABLED` is `false`:**
```
UI phase is disabled in config. Enable via /oto-settings.
```
Exit workflow.
...
Recommended: run /oto-discuss-phase {N} first to capture design preferences.
...
⚡ Sketch findings detected: {SKETCH_FINDINGS_PATH}
   Validated design decisions from /oto-sketch will be loaded into the UI researcher.
...
## 5. Spawn oto-ui-researcher

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► UI DESIGN CONTRACT — PHASE {N}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```markdown
Read ~/.claude/agents/oto-ui-researcher.md for instructions.
...
- {SKETCH_FINDINGS_PATH} (Sketch Findings — validated design decisions, CSS patterns, visual direction from /oto-sketch, if exists)
</files_to_read>
...
Template: ~/.claude/oto/templates/UI-SPEC.md
</output>
...
Task(
  ...
  subagent_type="oto-ui-researcher",
...
## 7. Spawn oto-ui-checker

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► VERIFYING UI-SPEC
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```markdown
Read ~/.claude/agents/oto-ui-checker.md for instructions.
...
- Re-spawn oto-ui-researcher with revision context:
...
2. Edit manually — open UI-SPEC.md in editor, re-run /oto-ui-phase
...
`/clear` then: `/oto-plan-phase {N}`

{If CONTEXT.md does NOT exist:}
**Discuss Phase {N}** — gather implementation context before planning

`/clear` then: `/oto-discuss-phase {N}`

(or `/oto-plan-phase {N}` to skip discussion)
...
```bash
oto-sdk query commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
...
- [ ] oto-ui-researcher spawned with correct context and file paths
- [ ] UI-SPEC.md created in correct location
- [ ] oto-ui-checker spawned with UI-SPEC.md
