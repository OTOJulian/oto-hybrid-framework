...
```bash
INIT=$(node "$HOME/.claude/oto/bin/oto-tools.cjs" init phase-op "${PHASE}")
...
If "Skip": Exit with message: "Existing SPEC.md unchanged. Run /oto-discuss-phase [X] to continue."
...
Use the SPEC.md template from @~/.claude/oto/templates/spec.md.
...
- User directed to /oto-discuss-phase as next step
</success_criteria>
