<purpose>
Create an isolated workspace directory with git repo copies (worktrees or clones) and an independent `.oto/` directory. Supports multi-repo orchestration and single-repo feature branch isolation.
</purpose>
...
```bash
INIT=$(oto-sdk query init.new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /oto-new-workspace --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /oto-new-workspace --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd "$TARGET_PATH"
  /oto-new-project    # Initialize OTO in the workspace
```
...
**Offer to initialize OTO (if not `--auto`):**

Use AskUserQuestion:
- header: "Initialize OTO"
- question: "Would you like to initialize a OTO project in the new workspace?"
- options:
  - "Yes — run /oto-new-project" → tell user to `cd "$TARGET_PATH"` first, then run `/oto-new-project`
...
- [ ] `.oto/` directory initialized at workspace root
