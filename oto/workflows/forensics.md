...
3. {Recovery command if applicable — e.g., `/oto-resume-work`, `/oto-execute-phase N`}

---

*Report generated by `/oto-forensics`. All paths redacted for portability.*
```
...
BUG_LABEL=$(gh label list --repo oto-build/oto --search "bug" --json name -q '.[0].name' 2>/dev/null)
LABEL_FLAG=""
if [ -n "$BUG_LABEL" ]; then
...
oto-sdk query state.record-session "" \
  ...
  ".oto/forensics/report-{timestamp}.md"
```
