...
OLLAMA_HOST=$(oto-sdk query config-get review.ollama_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LM_STUDIO_HOST=$(oto-sdk query config-get review.lm_studio_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LLAMA_CPP_HOST=$(oto-sdk query config-get review.llama_cpp_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
```bash
INIT=$(oto-sdk query init.phase-op "${PHASE_ARG}")
...
Write to a temp file: `/tmp/oto-review-prompt-{phase}.md`
</step>
...
```bash
# JSON scalars from oto-sdk query; use jq -r to strip JSON string quotes (install jq if missing)
GEMINI_MODEL=$(oto-sdk query config-get review.models.gemini 2>/dev/null | jq -r '.' 2>/dev/null || true)
CLAUDE_MODEL=$(oto-sdk query config-get review.models.claude 2>/dev/null | jq -r '.' 2>/dev/null || true)
CODEX_MODEL=$(oto-sdk query config-get review.models.codex 2>/dev/null | jq -r '.' 2>/dev/null || true)
OPENCODE_MODEL=$(oto-sdk query config-get review.models.opencode 2>/dev/null | jq -r '.' 2>/dev/null || true)
```
...
if [ -n "$GEMINI_MODEL" ] && [ "$GEMINI_MODEL" != "null" ]; then
  cat /tmp/oto-review-prompt-{phase}.md | gemini -m "$GEMINI_MODEL" -p - 2>/dev/null > /tmp/oto-review-gemini-{phase}.md
else
  cat /tmp/oto-review-prompt-{phase}.md | gemini -p - 2>/dev/null > /tmp/oto-review-gemini-{phase}.md
fi
```
...
if [ -n "$CLAUDE_MODEL" ] && [ "$CLAUDE_MODEL" != "null" ]; then
  cat /tmp/oto-review-prompt-{phase}.md | claude --model "$CLAUDE_MODEL" -p - 2>/dev/null > /tmp/oto-review-claude-{phase}.md
else
  cat /tmp/oto-review-prompt-{phase}.md | claude -p - 2>/dev/null > /tmp/oto-review-claude-{phase}.md
fi
```
...
if [ -n "$CODEX_MODEL" ] && [ "$CODEX_MODEL" != "null" ]; then
  cat /tmp/oto-review-prompt-{phase}.md | codex exec --model "$CODEX_MODEL" --skip-git-repo-check - 2>/dev/null > /tmp/oto-review-codex-{phase}.md
else
  cat /tmp/oto-review-prompt-{phase}.md | codex exec --skip-git-repo-check - 2>/dev/null > /tmp/oto-review-codex-{phase}.md
fi
```
...
```bash
coderabbit review --prompt-only 2>/dev/null > /tmp/oto-review-coderabbit-{phase}.md
```

**OpenCode (via GitHub Copilot):**
```bash
if [ -n "$OPENCODE_MODEL" ] && [ "$OPENCODE_MODEL" != "null" ]; then
  cat /tmp/oto-review-prompt-{phase}.md | opencode run --model "$OPENCODE_MODEL" - 2>/dev/null > /tmp/oto-review-opencode-{phase}.md
else
  cat /tmp/oto-review-prompt-{phase}.md | opencode run - 2>/dev/null > /tmp/oto-review-opencode-{phase}.md
fi
if [ ! -s /tmp/oto-review-opencode-{phase}.md ]; then
  echo "OpenCode review failed or returned empty output." > /tmp/oto-review-opencode-{phase}.md
fi
```

**Qwen Code:**
```bash
cat /tmp/oto-review-prompt-{phase}.md | qwen - 2>/dev/null > /tmp/oto-review-qwen-{phase}.md
if [ ! -s /tmp/oto-review-qwen-{phase}.md ]; then
  echo "Qwen review failed or returned empty output." > /tmp/oto-review-qwen-{phase}.md
fi
```

**Cursor:**
```bash
cat /tmp/oto-review-prompt-{phase}.md | cursor agent -p --mode ask --trust 2>/dev/null > /tmp/oto-review-cursor-{phase}.md
if [ ! -s /tmp/oto-review-cursor-{phase}.md ]; then
  echo "Cursor review failed or returned empty output." > /tmp/oto-review-cursor-{phase}.md
fi
```
...
OLLAMA_HOST=$(oto-sdk query config-get review.ollama_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$OLLAMA_HOST" ] || [ "$OLLAMA_HOST" = "null" ]; then OLLAMA_HOST="http://localhost:11434"; fi
OLLAMA_MODEL=$(oto-sdk query config-get review.models.ollama 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
jq -n --rawfile content /tmp/oto-review-prompt-{phase}.md \
  ...
  > /tmp/oto-review-ollama-{phase}.md
if [ ! -s /tmp/oto-review-ollama-{phase}.md ]; then
  echo "Ollama review failed or returned empty output." > /tmp/oto-review-ollama-{phase}.md
fi
```
...
LM_STUDIO_HOST=$(oto-sdk query config-get review.lm_studio_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$LM_STUDIO_HOST" ] || [ "$LM_STUDIO_HOST" = "null" ]; then LM_STUDIO_HOST="http://localhost:1234"; fi
LM_STUDIO_MODEL=$(oto-sdk query config-get review.models.lm_studio 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LM_STUDIO_RESPONSE=$(jq -n --rawfile content /tmp/oto-review-prompt-{phase}.md \
...
if [ -n "$LM_STUDIO_CONTENT" ]; then
  echo "$LM_STUDIO_CONTENT" > /tmp/oto-review-lm_studio-{phase}.md
else
...
LLAMA_CPP_HOST=$(oto-sdk query config-get review.llama_cpp_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$LLAMA_CPP_HOST" ] || [ "$LLAMA_CPP_HOST" = "null" ]; then LLAMA_CPP_HOST="http://localhost:8080"; fi
LLAMA_CPP_MODEL=$(oto-sdk query config-get review.models.llama_cpp 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LLAMA_CPP_CONTENT=$(jq -n --rawfile content /tmp/oto-review-prompt-{phase}.md \
...
if [ -n "$LLAMA_CPP_CONTENT" ]; then
  echo "$LLAMA_CPP_CONTENT" > /tmp/oto-review-llama_cpp-{phase}.md
else
...
Commit:
```bash
oto-sdk query commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/oto-plan-phase --reviews)
</success_criteria>
