...
Examples:
  `/oto-edit-phase 5`       → phase = 5, force = false
  `/oto-edit-phase 5 --force` → phase = 5, force = true
  `/oto-edit-phase 12.1`    → phase = 12.1, force = false

If no argument provided:

```
ERROR: Phase number required
Usage: /oto-edit-phase <phase-number> [--force]
Example: /oto-edit-phase 5
Example: /oto-edit-phase 5 --force
```
...
Run /oto-new-project to initialize.
```
Exit.
</step>
...
```bash
PHASE_DATA=$(oto-sdk query roadmap get-phase "${target}")
```
...
Available phases can be seen with /oto-progress.
```
...
```bash
ANALYZE=$(oto-sdk query roadmap analyze)
```
...
```bash
ALL_PHASES=$(oto-sdk query roadmap analyze)
```
...
- `/oto-progress` — view updated roadmap
- `/oto-plan-phase {target}` — re-plan this phase (if needed)
- `/oto-discuss-phase {target}` — discuss implementation approach
