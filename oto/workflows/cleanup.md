...
```bash
ls -d .oto/milestones/v*-phases 2>/dev/null || true
```
...
```bash
cat .oto/milestones/v{X.Y}-ROADMAP.md
```
...
Destination: .oto/milestones/v{X.Y}-phases/
...
Destination: .oto/milestones/v{X.Z}-phases/
```
...
```bash
mv .oto/phases/{dir} .oto/milestones/v{X.Y}-phases/
```
...
```bash
oto-sdk query commit "chore: archive phase directories from completed milestones" --files .oto/milestones/ .oto/phases/
```
...
Archived:
{For each milestone}
- v{X.Y}: {N} phase directories → .oto/milestones/v{X.Y}-phases/

.oto/phases/ cleaned up.
```
