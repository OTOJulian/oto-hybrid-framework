...
oto-plan-phase N → oto-review N --codex → oto-plan-phase N --reviews → oto-review N --codex → ...
...
OTO_WS=""
echo "$ARGUMENTS" | grep -qE '\-\-ws\s+\S+' && OTO_WS=$(echo "$ARGUMENTS" | grep -oE '\-\-ws\s+\S+')
```

## 1.5. Config Gate (feature disabled by default)

```bash
CONVERGENCE_ENABLED=$(oto-sdk query config-get workflow.plan_review_convergence 2>/dev/null || echo "false")
```

**If `CONVERGENCE_ENABLED` is not `"true"`:** Display and exit:

```text
oto-plan-review-convergence is disabled (workflow.plan_review_convergence=false).
...
Then re-run: /oto-plan-review-convergence {PHASE}
```

## 2. Initialize

```bash
INIT=$(node "$HOME/.claude/oto/bin/oto-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "$HOME/.claude/oto/bin/oto-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► PLAN CONVERGENCE — Phase {phase_number}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Agent(
  ...
  prompt="Run /oto-plan-phase for Phase {PHASE}.

Execute: Skill(skill='oto-plan-phase', args='{PHASE} {OTO_WS}')
...
Agent(
  ...
  prompt="Run /oto-review for Phase {PHASE}.

Execute: Skill(skill='oto-review', args='--phase {PHASE} {REVIEWER_FLAGS} {OTO_WS}')
...
```bash
node "$HOME/.claude/oto/bin/oto-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```

Display:
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 OTO ► CONVERGENCE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
To replan manually:  /oto-plan-phase {PHASE} --reviews
To restart loop:     /oto-plan-review-convergence {PHASE} {REVIEWER_FLAGS}
```
Exit workflow.
...
Agent(
  ...
  prompt="Run /oto-plan-phase with --reviews for Phase {PHASE}.

Execute: Skill(skill='oto-plan-phase', args='{PHASE} --reviews --skip-research {OTO_WS}')
...
IMPORTANT: When oto-plan-phase outputs '## PLANNING COMPLETE', that means replanning is done. Return at that point.",
  mode="auto"
)
```
...
- [ ] Initial planning via Agent → Skill("oto-plan-phase") if no plans exist
- [ ] Review via Agent → Skill("oto-review") — isolated, not inline; {OTO_WS} forwarded
- [ ] Replan via Agent → Skill("oto-plan-phase --reviews") — isolated, not inline
