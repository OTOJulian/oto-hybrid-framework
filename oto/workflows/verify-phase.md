...
<required_reading>
@~/.claude/oto/references/verification-patterns.md
@~/.claude/oto/templates/verification-report.md
</required_reading>
...
```bash
oto-sdk query roadmap.get-phase "${phase_number}"
grep -E "^| ${phase_number}" .oto/REQUIREMENTS.md 2>/dev/null || true
ls "$phase_dir"/*-SUMMARY.md "$phase_dir"/*-PLAN.md 2>/dev/null || true
```

Load full milestone phases for deferred-item filtering (Step 9b):
```bash
oto-sdk query roadmap.analyze
```
...
Use `oto-sdk query` verify handlers (or legacy oto-tools) to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(oto-sdk query frontmatter.get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(oto-sdk query roadmap.get-phase "${phase_number}" --raw)
```
...
Use `oto-sdk query verify.artifacts` (or legacy oto-tools) for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(oto-sdk query verify.artifacts "$plan")
...
Use `oto-sdk query verify.key-links` (or legacy oto-tools) for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(oto-sdk query verify.key-links "$plan")
...
```bash
grep -E "Phase ${PHASE_NUM}" .oto/REQUIREMENTS.md 2>/dev/null || true
```
...
if [ "$GATE_CFG" != "false" ]; then
  ...
  DECISION_RESULT=$(oto-sdk query check.decision-coverage-verify "${PHASE_DIR}" "${CONTEXT_PATH}")
fi
```
...
TEST_CMD=$(oto-sdk query config-get workflow.test_command --default "" 2>/dev/null || true)
if [ -z "$TEST_CMD" ]; then
...
See ~/.claude/oto/templates/verification-report.md for complete template.
</step>
