...
```bash
AGENT_SKILLS_DEBUGGER=$(oto-sdk query agent-skills oto-debugger)
...
- truth: "Comment appears immediately after submission"
  ...
  debug_session: .oto/debug/comment-not-refreshing.md
```
...
```bash
oto-sdk query commit "docs({phase_num}): add root causes from diagnosis" --files ".oto/phases/XX-name/{phase_num}-UAT.md"
```
</step>
