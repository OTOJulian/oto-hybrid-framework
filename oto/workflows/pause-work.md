...
phase=$(( ls -lt .oto/phases/*/PLAN.md 2>/dev/null || true ) | head -1 | grep -oP 'phases/\K[^/]+' || true)

# Check for active spike
spike=$(( ls -lt .oto/spikes/*/SPIKE.md .oto/spikes/*/DESIGN.md .oto/spikes/*/README.md 2>/dev/null || true ) | head -1 | grep -oP 'spikes/\K[^/]+' || true)

# Check for active sketch
sketch=$(( ls -lt .oto/sketches/*/README.md .oto/sketches/*/index.html 2>/dev/null || true ) | head -1 | grep -oP 'sketches/\K[^/]+' || true)
...
grep -l "To be filled\|placeholder\|TBD" .oto/phases/*/*.md 2>/dev/null || true
```
...
<step name="commit">
```bash
oto-sdk query commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .oto/HANDOFF.json
```
</step>
...
```
✓ Handoff created:
  - .oto/HANDOFF.json (structured, machine-readable)
