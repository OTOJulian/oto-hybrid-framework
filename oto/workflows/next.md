<purpose>
Detect current project state and automatically advance to the next logical OTO workflow step.
...
```bash
# Get state snapshot
oto-sdk query state.json 2>/dev/null || echo "{}"
```
...
Run `/oto-health` to diagnose, or manually fix STATE.md.
Use `--force` to bypass this check.
```
Exit.
...
After passing all three hard-stop gates, scan all phases that precede the current phase in ROADMAP.md order for incomplete work. For each prior phase number `N`, use `oto-sdk query find-phase <N>` JSON (plans, summaries, incomplete_plans, etc.) to inspect that phase.
...
**Source phase:** {src}
**Deferred at:** {date} during /oto-next advancement to Phase {dest}
**Plans:**
- [ ] {N}-{M}: {slug} (ran, no SUMMARY.md)
```
2. Commit the deferral record:
```bash
oto-sdk query commit "docs: defer incomplete Phase {src} items to backlog"
```
...
PENDING_SPIKES=$(grep -rl 'verdict: PENDING' .oto/spikes/*/README.md 2>/dev/null | wc -l | tr -d ' ')

# Check for pending sketches (winner: null in any README)
PENDING_SKETCHES=$(grep -rl 'winner: null' .oto/sketches/*/README.md 2>/dev/null | wc -l | tr -d ' ')
```

If either count is > 0, display before routing:
```
⚠ Pending exploratory work:
  {PENDING_SPIKES} spike(s) with unresolved verdicts in .oto/spikes/
  {PENDING_SKETCHES} sketch(es) without a winning variant in .oto/sketches/

  Resume with `/oto-spike` or `/oto-sketch`, or continue with phase work below.
```
...
→ Next action: `/oto-discuss-phase <first-phase>`
...
▶ **Next step:** `/oto-[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/oto-next` is zero-friction advancement.
</step>
