...
```bash
oto-sdk query commit "docs(spike-wrap-up): package [N] spike findings into project skill" --files .oto/spikes/WRAP-UP-SUMMARY.md .oto/spikes/CONVENTIONS.md
```
</step>
...
`/oto-spike` (run with no argument — its frontier mode analyzes the spike landscape and proposes integration and frontier spikes)

───────────────────────────────────────────────────────────────

**Also available:**
- `/oto-plan-phase` — start planning the real implementation
- `/oto-spike [idea]` — spike a specific new idea
