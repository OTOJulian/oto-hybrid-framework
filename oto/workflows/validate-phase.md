...
AGENT_SKILLS_AUDITOR=$(oto-sdk query agent-skills oto-nyquist-auditor)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(oto-sdk query resolve-model oto-nyquist-auditor --raw)
NYQUIST_CFG=$(oto-sdk query config-get workflow.nyquist_validation --raw)
```

If `NYQUIST_CFG` is `false`: exit with "Nyquist validation is disabled. Enable via /oto-settings."

Display banner: `OTO > VALIDATE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /oto-execute-phase {N} ${OTO_WS} first."
...
## 5. Spawn oto-nyquist-auditor

```
Task(
  prompt="Read ~/.claude/agents/oto-nyquist-auditor.md for instructions.\n\n" +
    "<files_to_read>{PLAN, SUMMARY, impl files, VALIDATION.md}</files_to_read>" +
    "<gaps>{gap list}</gaps>" +
    "<test_infrastructure>{framework, config, commands}</test_infrastructure>" +
    "<constraints>Never modify impl files. Max 3 debug iterations. Escalate impl bugs.</constraints>" +
    "${AGENT_SKILLS_AUDITOR}",
  subagent_type="oto-nyquist-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `~/.claude/oto/templates/VALIDATION.md`
...
oto-sdk query commit "docs(phase-${PHASE}): add/update validation strategy"
```
...
**Compliant:**
```
OTO > PHASE {N} IS NYQUIST-COMPLIANT
All requirements have automated verification.
▶ Next: /oto-audit-milestone ${OTO_WS}
```

**Partial:**
```
OTO > PHASE {N} VALIDATED (PARTIAL)
{M} automated, {K} manual-only.
▶ Retry: /oto-validate-phase {N} ${OTO_WS}
```
