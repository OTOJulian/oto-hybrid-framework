<purpose>
Create all phases necessary to close gaps identified by `/oto-audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/oto-add-phase` per gap.
</purpose>
...
(ls -t .oto/v*-MILESTONE-AUDIT.md 2>/dev/null || true) | head -1
```
...
```
No audit gaps found. Run `/oto-audit-milestone` first.
```
...
HIGHEST=$(oto-sdk query phases.list --pick directories[-1])
```
...
grep -c "Pending" .oto/REQUIREMENTS.md
```

## 8. Create Phase Directories

```bash
mkdir -p ".oto/phases/{NN}-{name}"
```

## 9. Commit Roadmap and Requirements Update

```bash
oto-sdk query commit "docs(roadmap): add gap closure phases {N}-{M}" --files .oto/ROADMAP.md .oto/REQUIREMENTS.md
```
...
**Also available:**
- `/oto-execute-phase {N}` — if plans already exist
- `cat .oto/ROADMAP.md` — see updated roadmap

---

**After all gap phases complete:**

`/oto-audit-milestone` — re-audit to verify gaps closed
`/oto-complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/oto-plan-phase` next
</success_criteria>
