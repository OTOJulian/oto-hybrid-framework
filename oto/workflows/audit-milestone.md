...
AGENT_SKILLS_CHECKER=$(oto-sdk query agent-skills oto-integration-checker)
```
...
```bash
integration_checker_model=$(oto-sdk query resolve-model oto-integration-checker --raw)
```
...
PHASE_INFO=$(oto-sdk query find-phase 01 --raw)
...
${AGENT_SKILLS_CHECKER}",
  subagent_type="oto-integration-checker",
...
for summary in .oto/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  oto-sdk query summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(oto-sdk query config-get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/oto-validate-phase`.
...
**Report:** .oto/v{version}-MILESTONE-AUDIT.md
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/oto-validate-phase {N}` |

Phases needing validation: run `/oto-validate-phase {N}` for each flagged phase.
...
**Also available:**
- cat .oto/v{version}-MILESTONE-AUDIT.md — see full report
- /oto-complete-milestone {version} — proceed anyway (accept tech debt)
