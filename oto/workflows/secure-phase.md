...
- oto-security-auditor — Verifies threat mitigation coverage
</available_agent_types>
...
AGENT_SKILLS_AUDITOR=$(oto-sdk query agent-skills oto-security-auditor)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(oto-sdk query resolve-model oto-security-auditor --raw)
SECURITY_CFG=$(oto-sdk query config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
```

If `SECURITY_CFG` is `false`: exit with "Security enforcement disabled. Enable via /oto-settings."

Display banner: `OTO > SECURE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /oto-execute-phase {N} first."
...
## 5. Spawn oto-security-auditor

```
Task(
  prompt="Read ~/.claude/agents/oto-security-auditor.md for instructions.\n\n" +
  ...
  subagent_type="oto-security-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `~/.claude/oto/templates/SECURITY.md`
...
```
OTO > PHASE {N} SECURITY BLOCKED
{K} threats open — phase advancement blocked until threats_open: 0
▶ Fix mitigations then re-run: /oto-secure-phase {N}
...
```bash
oto-sdk query commit "docs(phase-${PHASE}): add/update security threat verification"
```
...
```
OTO > PHASE {N} THREAT-SECURE
threats_open: 0 — all threats have dispositions.
▶ /oto-validate-phase {N}    validate test coverage
▶ /oto-verify-work {N}       run UAT
```
