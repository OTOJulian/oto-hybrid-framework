...
BUILD_CMD=$(oto-sdk query config-get workflow.build_command --default "" 2>/dev/null || true)
if [ -z "$BUILD_CMD" ]; then
  ...
  if [ -n "$XCODEPROJ" ]; then
    ...
    BUILD_CMD="python -m py_compile $(find . -name '*.py' -not -path './.oto/*' -not -path './node_modules/*' | head -20 | tr '\n' ' ')"
