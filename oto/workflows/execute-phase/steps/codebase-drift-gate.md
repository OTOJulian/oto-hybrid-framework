...
```bash
DRIFT=$(oto-sdk query verify.codebase-drift 2>/dev/null || echo '{"skipped":true,"reason":"sdk-failed"}')
```
...
Run /oto-map-codebase --paths {affected_paths} to refresh planning context.
```
...
from step `init_context` is for `oto-executor`, not the mapper):

```bash
AGENT_SKILLS_MAPPER=$(oto-sdk query agent-skills oto-codebase-mapper)
```

Then spawn `oto-codebase-mapper` agents with the `--paths` hint:
