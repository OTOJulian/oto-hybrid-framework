...
Use /oto-quick for anything that needs multi-step planning or research.
</purpose>
...
```
This looks like it needs planning. Use /oto-quick instead:
  /oto-quick "{task description}"
```
...
if grep -q "Quick Tasks Completed" .oto/STATE.md 2>/dev/null; then
  # Append entry — workflow handles the format
  echo "| $(date +%Y-%m-%d) | fast | $TASK | ✅ |" >> .oto/STATE.md
fi
```
</step>
...
- If the task takes more than 3 file edits, STOP and redirect to /oto-quick
- If you're unsure how to implement it, STOP and redirect to /oto-quick
</guardrails>
