<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /oto-plan-phase --gaps.
...
```bash
INIT=$(oto-sdk query init.verify-work "${PHASE_ARG}")
...
```bash
(find .oto/phases -name "*-UAT.md" -type f 2>/dev/null || true)
```
...
Provide a phase number to start testing (e.g., /oto-verify-work 4)
```
...
```
UI_PHASE_FLAG=$(oto-sdk query config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(oto-sdk query uat.render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
oto-sdk query commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".oto/phases/XX-name/{phase_num}-UAT.md"
```
...
```
⚠ Security enforcement enabled — /oto-secure-phase {phase} has not run.
Run before advancing to the next phase.

All tests passed. Ready to continue.

- `/oto-secure-phase {phase}` — security review (required before advancing)
- `/oto-plan-phase {next}` — Plan next phase
- `/oto-execute-phase {next}` — Execute next phase
- `/oto-ui-review {phase}` — visual quality audit (if frontend files were modified)
```
...
Read and follow `~/.claude/oto/workflows/transition.md`.
...
- `/oto-plan-phase {next}` — Plan next phase
- `/oto-execute-phase {next}` — Execute next phase
- `/oto-secure-phase {phase}` — security review
- `/oto-ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
`audit-open` is CJS-only until registered on `oto-sdk query`:

```bash
oto-sdk query audit-open --json
```
...
If user declines: stop. User resolves items and re-runs `/oto-verify-work`.
...
- Follow @~/.claude/oto/workflows/diagnose-issues.md
...
Spawn oto-planner in --gaps mode:
...
<downstream_consumer>
Output consumed by /oto-execute-phase
...
Spawn oto-planner with revision context:
...
3. Abandon (exit, user runs /oto-plan-phase manually)
...
`/clear` then `/oto-execute-phase {phase} --gaps-only`
...
- [ ] If issues: oto-planner creates fix plans (gap_closure mode)
- [ ] If issues: oto-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/oto-execute-phase --gaps-only` when complete
</success_criteria>
