...
```bash
ls -d .oto/spikes/[0-9][0-9][0-9]-* 2>/dev/null | sort | tail -1
```
...
```bash
oto-sdk query commit "docs(spike-NNN): [VERDICT] — [key finding]" --files .oto/spikes/NNN-descriptive-name/ .oto/spikes/MANIFEST.md
```
...
```bash
oto-sdk query commit "docs(spikes): update conventions" --files .oto/spikes/CONVENTIONS.md
```
</step>
...
**Also available:**
- `/oto-spike` — spike more ideas (or run with no argument for frontier mode)
- `/oto-plan-phase` — start planning the real implementation
- `/oto-explore` — continue exploring the idea
