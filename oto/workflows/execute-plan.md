...
@~/.claude/oto/references/git-integration.md
</required_reading>
...
```bash
INIT=$(oto-sdk query init.execute-phase "${PHASE}")
...
(ls .oto/phases/XX-name/*-PLAN.md 2>/dev/null || true) | sort
(ls .oto/phases/XX-name/*-SUMMARY.md 2>/dev/null || true) | sort
```
...
# config settings can be fetched via oto-sdk query config-get if needed
```
...
TASK_COUNT=$(grep -cE '^\s*<task[[:space:]>]' .oto/phases/XX-name/{phase}-{plan}-PLAN.md 2>/dev/null || echo "0")
INLINE_THRESHOLD=$(oto-sdk query config-get workflow.inline_plan_threshold 2>/dev/null || echo "2")
grep -n "type=\"checkpoint" .oto/phases/XX-name/{phase}-{plan}-PLAN.md
```
...
```bash
if [ ! -f .oto/agent-history.json ]; then
  echo '{"version":"1.0","max_entries":50,"entries":[]}' > .oto/agent-history.json
fi
rm -f .oto/current-agent-id.txt
if [ -f .oto/current-agent-id.txt ]; then
  INTERRUPTED_ID=$(cat .oto/current-agent-id.txt)
...
2. Per segment:
   - Subagent route: spawn oto-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
cat .oto/phases/XX-name/{phase}-{plan}-PLAN.md
```
...
```bash
oto-sdk query phases.list --type summaries --raw
...
Apply deviation rules from the oto-executor agent definition (single source of truth):
...
See `~/.claude/oto/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/oto/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(oto-sdk query config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/oto/workflows/node-repair.md` with:
...
```bash
grep -A 50 "^user_setup:" .oto/phases/XX-name/{phase}-{plan}-PLAN.md | head -50
```

If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/oto/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Update STATE.md using oto-sdk query (or legacy oto-tools) state mutations:
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  oto-sdk query state.advance-plan

  # Recalculate progress bar from disk state
  oto-sdk query state.update-progress

  # Record execution metrics
  oto-sdk query state.record-metric \
...
oto-sdk query state.add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
oto-sdk query state.add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using oto-sdk query (or legacy oto-tools):

```bash
oto-sdk query state.record-session \
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  oto-sdk query roadmap.update-plan-progress "${PHASE}"
fi
```
...
```bash
oto-sdk query requirements.mark-complete ${REQ_IDS}
```
...
if [ "$IS_WORKTREE" = "true" ]; then
  oto-sdk query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .oto/phases/XX-name/{phase}-{plan}-SUMMARY.md .oto/REQUIREMENTS.md
else
  oto-sdk query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .oto/phases/XX-name/{phase}-{plan}-SUMMARY.md .oto/STATE.md .oto/ROADMAP.md .oto/REQUIREMENTS.md
fi
```
</step>

<step name="update_codebase_map">
If .oto/codebase/ doesn't exist: skip.
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/oto-execute-phase {phase}` + `/oto-verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/oto-plan-phase {Z+1}` + `/oto-verify-work {Z}` + `/oto-discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/oto-complete-milestone` + `/oto-verify-work` + `/oto-add-phase` |
