...
- **Project scope**: `.oto/notes/{YYYY-MM-DD}-{slug}.md` — used when `.oto/` exists in cwd
- **Global scope**: `~/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.oto/`, or when `--global` flag is present
...
**`--global` flag**: Strip `--global` from anywhere in `$ARGUMENTS` before parsing. When present, force global scope regardless of whether `.oto/` exists.

**Important**: Do NOT create `.oto/` if it doesn't exist. Fall back to global scope silently.
</step>
...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `/oto-note list of groceries` saves a note with text "list of groceries". Same for `promote` — only a subcommand when followed by exactly one number.
</step>
...
2. Ensure the notes directory exists (`.oto/notes/` or `~/.claude/notes/`)
...
1. Glob `.oto/notes/*.md` (if directory exists) — project notes
...
{count} active note(s). Use `/oto-note promote <N>` to convert to a todo.
```
...
4. **Requires `.oto/` directory** — if it doesn't exist, warn: "Todos require a OTO project. Run `/oto-new-project` to initialize one."
5. Ensure `.oto/todos/pending/` directory exists
6. Generate todo ID: `{NNN}-{slug}` where NNN is the next sequential number (scan both `.oto/todos/pending/` and `.oto/todos/completed/` for the highest existing number, increment by 1, zero-pad to 3 digits) and slug is the first ~4 meaningful words of the note text
7. Extract the note text from the source file (body after frontmatter)
8. Create `.oto/todos/pending/{id}.md`:
...
<edge_cases>
1. **"list" as note text**: `/oto-note list of things` saves note "list of things" (subcommand only when `list` is the entire arg)
2. **No `.oto/`**: Falls back to global `~/.claude/notes/` — works in any directory
3. **Promote without project**: Warns that todos require `.oto/`, suggests `/oto-new-project`
...
- [ ] Global fallback: Works when no `.oto/` exists
</success_criteria>
