...
- Check for MILESTONE-CONTEXT.md (from /oto-discuss-milestone)
...
```bash
oto-sdk query state.milestone-switch --milestone "v[X.Y]" --name "[Name]"
```
...
```bash
oto-sdk query phases.clear --confirm
```

```bash
oto-sdk query commit "docs: start milestone v[X.Y] [Name]" --files .oto/PROJECT.md .oto/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(oto-sdk query init.new-milestone)
...
- Tell the user to complete/archive the previous milestone first, then rerun `/oto-new-milestone --reset-phase-numbers ${OTO_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/oto-plan-phase` behavior. To change the default, use `/oto-settings`.
...
Spawn 4 parallel oto-project-researcher agents. Each uses this template with dimension-specific fields:
...
<output>
Write to: .oto/research/{FILE}
Use template: ~/.claude/oto/templates/research-project/{FILE}
</output>
", subagent_type="oto-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
**Commit requirements:**
```bash
oto-sdk query commit "docs: define milestone v[X.Y] requirements" --files .oto/REQUIREMENTS.md
```
...
- .oto/PROJECT.md
- .oto/REQUIREMENTS.md
- .oto/research/SUMMARY.md (if exists)
- .oto/config.json
- .oto/MILESTONES.md
</files_to_read>
...
```bash
oto-sdk query commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .oto/ROADMAP.md .oto/STATE.md .oto/REQUIREMENTS.md
```
...
```bash
PENDING_TODOS=$(ls .oto/todos/pending/*.md 2>/dev/null | head -50)
```
...
```bash
oto-sdk query commit "docs: tag [count] pending todos with resolves_phase after milestone v[X.Y] roadmap" --files .oto/todos/pending/*.md
```
...
`/oto-discuss-phase [N] ${OTO_WS}` — gather context and clarify approach

Also: `/oto-plan-phase [N] ${OTO_WS}` — skip discussion, plan directly
```
...
- [ ] oto-roadmapper spawned with phase numbering context
...
- [ ] User knows next step: `/oto-discuss-phase [N] ${OTO_WS}`
