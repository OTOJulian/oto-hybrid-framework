...
@~/.claude/oto/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@~/.claude/oto/references/few-shot-examples/verifier.md
...
oto-sdk query roadmap.get-phase "$PHASE_NUM"
grep -E "^| $PHASE_NUM" .oto/REQUIREMENTS.md 2>/dev/null
```
...
```bash
PHASE_DATA=$(oto-sdk query roadmap.get-phase "$PHASE_NUM" --raw)
```
...
Use `oto-sdk query` for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(oto-sdk query verify.artifacts "$PLAN_PATH")
```
...
Use `oto-sdk query` for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(oto-sdk query verify.key-links "$PLAN_PATH")
```
...
```bash
grep -E "Phase $PHASE_NUM" .oto/REQUIREMENTS.md 2>/dev/null
```
...
SUMMARY_FILES=$(oto-sdk query summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(oto-sdk query verify.commits $COMMIT_HASHES)
fi
...
```bash
ROADMAP_DATA=$(oto-sdk query roadmap.analyze --raw)
```
...
Structure gaps in YAML frontmatter for `/oto-plan-phase --gaps`:
...
**Status:** {passed | gaps_found | human_needed}
**Score:** {N}/{M} must-haves verified
**Report:** .oto/phases/{phase_dir}/{phase_num}-VERIFICATION.md
...
Structured gaps in VERIFICATION.md frontmatter for `/oto-plan-phase --gaps`.
