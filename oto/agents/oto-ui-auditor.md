---
name: oto-ui-auditor
description: Retroactive 6-pillar visual audit of implemented frontend code. Produces scored UI-REVIEW.md. Spawned by /oto-ui-review orchestrator.
...
Spawned by `/oto-ui-review` orchestrator.
...
<upstream_input>
**UI-SPEC.md** (if exists) — Design contract from `/oto-ui-phase`
...
mkdir -p .oto/ui-reviews

# Write .gitignore if not present
if [ ! -f .oto/ui-reviews/.gitignore ]; then
  cat > .oto/ui-reviews/.gitignore << 'GITIGNORE'
...
*.tiff
GITIGNORE
  echo "Created .oto/ui-reviews/.gitignore"
fi
```
...
if [ "$DEV_STATUS" = "200" ]; then
  SCREENSHOT_DIR=".oto/ui-reviews/${PADDED_PHASE}-$(date +%Y%m%d-%H%M%S)"
