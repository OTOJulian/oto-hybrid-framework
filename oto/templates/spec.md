...
- `oto-planner` — reads locked requirements to constrain plan scope
- `oto-verifier` — uses acceptance criteria as explicit pass/fail checks
...
*Phase: [XX-name]*
*Spec created: [date]*
*Next step: /oto-discuss-phase [X] — implementation decisions (how to build what's specified above)*
```
...
*Next step: /oto-discuss-phase 3 — implementation decisions (card layout, loading skeleton, etc.)*
```
...
A `oto backup` CLI command creates a reproducible database snapshot that can be restored by `oto restore` (a separate phase).
...
1. **Backup creation**: CLI command executes a full database backup.
   ...
   - Target: `oto backup` connects to the database (via `DATABASE_URL` env or `--db` flag), runs pg_dump, writes output to `./backups/YYYY-MM-DD_HH-MM-SS.dump`
   - Acceptance: Running `oto backup` on a test database creates a `.dump` file; running `pg_restore` on that file recreates the database without error
...
- [ ] `oto backup` creates a `.dump` file in `./backups/YYYY-MM-DD_HH-MM-SS.dump` format
- [ ] `oto backup` uses `DATABASE_URL` env var or `--db` flag for connection
...
*Spec created: 2025-01-20*
*Next step: /oto-discuss-phase 2 — implementation decisions (progress reporting, flag design, etc.)*
```
