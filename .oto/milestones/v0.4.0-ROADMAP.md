...
- [x] **Phase 12: Query registry + workflow consumption** — Rebuild the query registry to answer every oto key (`init.*`, `agent-skills`, `commit`, `state.*`, `phases.*`) against `.oto/` paths, and wire workflows to consume `oto-sdk query` output while still degrading gracefully when it is absent.
