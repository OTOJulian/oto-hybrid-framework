 ...
 *
 * Structural assertions for /oto-ingest-docs (#2387). Agents and workflows
...
const CMD_PATH = path.join(ROOT, 'oto', 'commands', 'oto', 'ingest-docs.md');
const WF_PATH = path.join(ROOT, 'oto', 'workflows', 'ingest-docs.md');
const CLASSIFIER_PATH = path.join(ROOT, 'oto', 'agents', 'oto-doc-classifier.md');
const SYNTHESIZER_PATH = path.join(ROOT, 'oto', 'agents', 'oto-doc-synthesizer.md');
const CONFLICT_ENGINE_PATH = path.join(ROOT, 'oto', 'references', 'doc-conflict-engine.md');

// ─── File Existence ────────────────────────────────────────────────────────────

describe('ingest-docs file structure (#2387)', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(CMD_PATH), 'commands/oto/ingest-docs.md should exist');
  });
  test('workflow file exists', () => {
    assert.ok(fs.existsSync(WF_PATH), 'oto/workflows/ingest-docs.md should exist');
  });
  test('classifier agent exists', () => {
    assert.ok(fs.existsSync(CLASSIFIER_PATH), 'agents/oto-doc-classifier.md should exist');
  });
  test('synthesizer agent exists', () => {
    assert.ok(fs.existsSync(SYNTHESIZER_PATH), 'agents/oto-doc-synthesizer.md should exist');
  });
...
describe('ingest-docs command frontmatter', () => {
  ...
  test('has name field', () => {
    assert.match(content, /^name:\s*oto:ingest-docs$/m);
  });
...
describe('ingest-docs command references', () => {
  ...
  test('references the ingest-docs workflow', () => {
    assert.ok(
      content.includes('@~/.claude/oto/workflows/ingest-docs.md'),
      'command must @-reference its workflow'
    );
  });
  test('references the doc-conflict-engine', () => {
    assert.ok(
      content.includes('@~/.claude/oto/references/doc-conflict-engine.md'),
      'command must load the shared conflict-engine contract'
    );
  });
  test('references gate-prompts', () => {
    assert.ok(
      content.includes('@~/.claude/oto/references/gate-prompts.md'),
...
describe('ingest-docs workflow content', () => {
  ...
  test('auto-detects MODE from .oto/ presence', () => {
    assert.ok(
  ...
  test('spawns oto-doc-classifier and oto-doc-synthesizer', () => {
    assert.ok(
      content.includes('oto-doc-classifier'),
      'workflow must spawn oto-doc-classifier'
    );
    assert.ok(
      content.includes('oto-doc-synthesizer'),
      'workflow must spawn oto-doc-synthesizer'
    );
  });
  ...
  test('routes to oto-roadmapper in new mode', () => {
    assert.ok(
      content.includes('oto-roadmapper'),
      'new mode must delegate to oto-roadmapper'
    );
  });
...
describe('oto-doc-classifier agent', () => {
...
describe('oto-doc-synthesizer agent', () => {
