...
const GSD_LICENSE = fs.readFileSync(path.join(REPO_ROOT, 'tests/fixtures/rebrand-corpus/get-shit-done-main/LICENSE'), 'utf8');
const SUPERPOWERS_LICENSE = fs.readFileSync(path.join(REPO_ROOT, 'tests/fixtures/rebrand-corpus/superpowers-main/LICENSE'), 'utf8');

test('CI-06: THIRD-PARTY-LICENSES embeds GSD MIT license verbatim', () => {
  assert.ok(
    THIRD_PARTY.includes(GSD_LICENSE.trim()),
    'tests/fixtures/rebrand-corpus/get-shit-done-main/LICENSE body is not embedded verbatim',
  );
});
...
test('CI-06: THIRD-PARTY-LICENSES embeds Superpowers MIT license verbatim', () => {
  assert.ok(
    ...
    'tests/fixtures/rebrand-corpus/superpowers-main/LICENSE body is not embedded verbatim',
  );
});
